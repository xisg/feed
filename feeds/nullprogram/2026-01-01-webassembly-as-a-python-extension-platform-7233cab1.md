...
feed: nullprogram
guid: urn:uuid:91e7555d-950f-47c6-84b8-bee0070f61a9
---
...
selling points. We can also use Wasm to access _embeddable capabilities_
...
store. Quite sensible, honestly. What’s _not_ sensible is how often I have
...
copying plain Python values in and out. The [`struct` package](https://docs.python.org/3/library/struct.html) is
quite handy here:
...
It fills a similar role to [JavaScript `DataView`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/DataView). If you’re copying
...
argument, _suggesting_ use of a different store for instantiation. That
would solve this problem, but as of this writing it _must_ be the same
...
I’ve [discussed `__heap_base` before](/blog/2025/04/19/), which is part of the ABI.
...
keys, which we’ll do using [the `secrets` built-in package](https://docs.python.org/3/library/secrets.html):
