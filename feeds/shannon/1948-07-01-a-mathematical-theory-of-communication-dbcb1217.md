...
S1;:::;  Sn are allowed and these symbols have durations t1        ;:::;
...
C = 0 539.
    A very general type of restriction which may be placed on allowed sequences is the following: We
                                        ; ;:::;
imagine a number of possible states a1 a2      am . For each state only certain symbols from the set S1 ;:::;
                                                                                                            Sn
...
probability .02, but there is no influence between adjacent letters and no tendency to form the preferred
    5 Letter, digram and trigram frequencies are given in Secret and Urgent by Fletcher Pratt, Blue Ribbon Books, 1939. Word frequen-
     ...
     2. The remainder, all of whose members have probabilities satisfying the inequality
                                                 log p,1
                                                    N
                                                         ,H          < :
                                                 log p,1
    In other words we are almost certain to have          very close to H when N is large.
                                                    N
...
with positive. This contradicts the definition of C as the maximum of H (x) , Hy (x).
    Actually more has been proved than was statedpin the theorem. If p      the average of a set of numbers is
         
within of of their maximum, a fraction of at most      can be more than            
...
binary number      then gives the subscript of the Xi that is incorrect (if 0 there was no error).
...
where K must be positive to satisfy (2).
                                                                                                    ni
    Now suppose we have a choice from n possibilities with commeasurable probabilities pi =              where
                                                                                                   ∑ ni
...
So that if   i satisfy
                                                ∑ iC, i j = j  `
                                                     ...
                                                     Pi = Bi i         :
Both the sets of equations for Bi and   i can be satisfied since C is such that
...
If a function of time f (t ) is limited to the band from 0 to W cycles per second it is completely determined
                                                               1
by giving its ordinates at a series of discrete points spaced 2W seconds apart in the manner indicated by the
following result. 5
...
the functions whosep total energy is less than or equal to E correspond to points in a 2TW dimensional sphere
with radius r = 2W E.
...
8. There is one important difference between the continuous and discrete entropies. In the discrete case
   ...
   in general change. In fact if we change to coordinates y1  yn the new entropy is given by
                               Z         Z                                                             
                     H (y) =              p(x1   ;:::; xn )J xy log p(x1;:::; xn )J xy dy1  dyn
           , 
   where J xy is the Jacobian of the coordinate transformation. On expanding the logarithm and chang-
   ing the variables to x1  xn , we obtain:
                                                    Z             Z                               
                             H (y) = H (x) ,                        p(x1   ;:::; xn ) logJ xy dx1 ::: dxn :
...
logarithmic sense).
    In the white noise case the distribution function is given by
                                   ...
                                   p(x1   ;:::; xn ) = (2N1 )n 2 exp , 2N1 ∑ x2i :
                                                                           =

Since this depends only on ∑ x2i the surfaces of equal probability density are spheresp  and the entire distri-
bution has spherical symmetry. The region ofp     high probability is a sphere of radius nN. As n ! ∞ the
                                      p                                   
...
density function for the sum is given by the convolution:
                                     Z          Z
                    r(x1   ;:::; xn) =           p(y1      ;:::; yn )q(x1 , y1;:::; xn , yn) dy1  dyn:
...
This can be written                            ZZ
                      ...
                      ZZ                                Z
using the fact that           ;
                           P(x y) log P(x) dx dy =              P(x) log P(x) dx. The channel capacity is thus expressed as
...
and denominator in log
                            P(x y);  will be multiplied by the same factors when x and y are transformed in
                           P(x)P(y)
...
average transmitter power is limited to P is given by
   ...
   This means that by sufficiently involved encoding systems we can transmit binary digits at the rate
       P+N
W log2         bits per second, with arbitrarily small frequency of errors. It is not possible to transmit at a
          N
...
so that no matter how small is chosen, we can, by taking T sufficiently large, transmit as near as we wish
           P+N
to TW log         binary digits in the time T .
             N
                                       P+N
...
original conditions. This altered problem can be solved easily. The maximum entropy    p occursp if the different
...
Therefore, if we can find an ensemble of functions such that they correspond to a rate nearly W log 1 +
                                                                                                          N
                                                                     p
and are limited to band W and peak S the result will be proved. Consider      p ensemble of functions of the
                                                                              the
following type. A series of t samples have the same value, either + S or , S, then the p next t samplesphave
...
of possible values of x and y:                 ZZ
                                                ,
                                                       ; 
                                               v P(x y) =                     ;  ;
                                                                         P(x y) (x y) dx dy    :
...
getting the pair in question, provided the duration T of the messages be taken sufficiently large.
     The following are simple examples of evaluation functions:
                                                                                                                                  
   9 It is not a “metric” in the strict sense, however, since in general it does not satisfy either (x; y) = (y; x) or (x; y) + (y; z)            (x z).
                                                                                                                                                      ;
   ...
   2. Frequency weighted R.M.S. criterion. More generally one can apply different weights to the different
      ...
      then                                                            Z T
                                                  (x; y) = T1 0 f (t )2 dt :
   3. Absolute error criterion.                                 Z T
                                          (x; y) = T1 0 x(t ) , y(t ) dt :
...
or solving for    and  and replacing in the equations
                                         Z
                                    H1        q(xi , si ) log r(xi ) dxi = ,H3 log p(si )
                                         Z
                                    H2        p(xi , si ) log r(xi ) dxi = ,H3 logq(si )                 :
...
This should equal                                                                               
                                             ...
                                             H1 2    2        
                         H1                            H1
which requires Ai j =       Ci j . In this case Ai j =    Bi j and both equations reduce to identities.
                         H3                            H2
