...
feed: karpathy
guid: http://karpathy.github.io/2026/02/12/microgpt/
---
...
FileWhat it adds`train0.py`Bigram count table — no neural net, no gradients`train1.py`MLP + manual gradients (numerical & analytic) + SGD`train2.py`Autograd (Value class) — replaces manual gradients`train3.py`Position embeddings + single-head attention + rmsnorm + residuals`train4.py`Multi-head attention + layer loop — full GPT architecture`train5.py`Adam optimizer — this is `train.py`
