...
feed: travisdowns
guid: https://travisdowns.github.io/blog/2020/05/18/icelake-zero-opt
---
...
If you made it down to the [hardware survey](https://travisdowns.github.io/blog/2020/05/13/intel-zero-opt.html#hardware-survey) on the last post, you might have [wondered](https://twitter.com/tarlinian/status/1260629853000265728) where Intel’s newest mainstream architecture was. _Ice Lake was missing!_

Well good news: it’s here… and it’s interesting. We’ll jump right into the same analysis we did last time for Skylake client. If you haven’t read the [first article](https://travisdowns.github.io/blog/2020/05/13/intel-zero-opt.html) you’ll probably want to start there, because we’ll refer to concepts introduced there without reexplaining them here.

As usual, you can skip to the [summary](https://travisdowns.github.io/blog/2020/05/18/icelake-zero-opt.html/#summary) for the bite sized version of the findings.
...
[\[link to this chart\]](https://travisdowns.github.io/blog/2020/05/18/icelake-zero-opt.html/#fig7a) [\[data table\]](https://travisdowns.github.io/misc/tables//intel-zero-opt/fig7a.html) [\[raw data\]](https://github.com/travisdowns/zero-fill-bench/tree/master/results/icl512/overall-warm.csv)

[![Figure 7a](https://travisdowns.github.io/assets/intel-zero-opt/fig7a.svg)](https://travisdowns.github.io/misc/tables//intel-zero-opt/fig7a.html)

Actually, I lied. _This_ is the right plot for Ice Lake:

**Figure 7b**

[\[link to this chart\]](https://travisdowns.github.io/blog/2020/05/18/icelake-zero-opt.html/#fig7b) [\[data table\]](https://travisdowns.github.io/misc/tables//intel-zero-opt/fig7b.html) [\[raw data\]](https://github.com/travisdowns/zero-fill-bench/tree/master/results/icl/overall-warm.csv)

[![Figure 7b](https://travisdowns.github.io/assets/intel-zero-opt/fig7b.svg)](https://travisdowns.github.io/misc/tables//intel-zero-opt/fig7b.html)
...
![Figure 7a Annotated](https://travisdowns.github.io/assets/intel-zero-opt/fig7a.svg)
...
![Figure 7a Annotated](https://travisdowns.github.io/assets/intel-zero-opt/fig7b.svg)
...
Using `-march=icelake-client` uses 256-bit instructions[1](https://travisdowns.github.io/blog/2020/05/18/icelake-zero-opt.html/#fn:still512):
...
GB/sSizeTrialfill0fill19864092.392.0191.991.9291.991.9392.492.2492.092.3592.192.1692.092.0792.392.1892.292.0992.092.110183.393.911197.3196.912197.3196.613196.6197.314197.3196.615196.6197.316196.6196.6

The performance follows a specific pattern with respect to the trials for both `fill0` and `fill1`: it starts out slow (about 90 GB/s) for the first 9-10 samples then suddenly jumps up the higher performance level (close to 200 GB/s). It turns out this is just [voltage and frequency management](https://travisdowns.github.io/blog/2020/01/17/avxfreq1.html) biting us again. In this case there is no frequency change: the [raw data](https://github.com/travisdowns/zero-fill-bench/blob/post2/results/icl512/overall-warm.csv#L546) has a frequency column that shows the trials always run at 3.5 GHz. There is only a voltage change, and while the voltage is changing, the CPU runs with reduced dispatch throughput[2](https://travisdowns.github.io/blog/2020/05/18/icelake-zero-opt.html/#fn:iclbetter).
...
[\[link to this chart\]](https://travisdowns.github.io/blog/2020/05/18/icelake-zero-opt.html/#fig8) [\[data table\]](https://travisdowns.github.io/misc/tables//intel-zero-opt/fig8.html) [\[raw data\]](https://github.com/travisdowns/zero-fill-bench/tree/master/results/icl/l2-focus.csv)

[![Figure 8](https://travisdowns.github.io/assets/intel-zero-opt/fig8.svg)](https://travisdowns.github.io/misc/tables//intel-zero-opt/fig8.html)

We finally have the elusive (near) 100% elimination of redundant zero stores! The `fill0` case peaks at 96% silent (eliminated[3](https://travisdowns.github.io/blog/2020/05/18/icelake-zero-opt.html/#fn:stricly)) evictions. Typical L3 bandwidth is ~59 GB/s with elimination and ~42 GB/s without, for a better than 40% speedup! So this is a potentially a big deal on Ice Lake.
...
[\[link to this chart\]](https://travisdowns.github.io/blog/2020/05/18/icelake-zero-opt.html/#fig9) [\[data table\]](https://travisdowns.github.io/misc/tables//intel-zero-opt/fig9.html) [\[raw data\]](https://github.com/travisdowns/zero-fill-bench/tree/master/results/icl/l3-focus.csv)

[![Figure 9](https://travisdowns.github.io/assets/intel-zero-opt/fig9.svg)](https://travisdowns.github.io/misc/tables//intel-zero-opt/fig9.html)
...
As with the L3 case, we see that the store elimination appears 96% effective: the number of uncore to memory writebacks flatlines at 4% for the `fill0` case. Compare this to [**Figure 3**](https://travisdowns.github.io/assets/intel-zero-opt/fig3.svg), which is the same benchmark running on Skylake-S, and note that only half the writes to RAM are eliminated.
...
The asymptotic speedup for the all zero case for the RAM region is less than the L3 region, at about 23% but that’s still not exactly something to sneeze at. The speedup for the alternating case is 10%, somewhat less than half the benefit of the all zero case[4](https://travisdowns.github.io/blog/2020/05/18/icelake-zero-opt.html/#fn:altwrites). In the L3 region, we also note that the benefit of elimination for `alt01` is only about 7%, much smaller than the ~20% benefit you’d expect if you cut the 40% benefit the all-zeros case sees. We saw a similar effect in Skylake-S.

Finally it’s worth noting this little uptick in uncore writes in the `fill0` case:

![Little Uptick](https://travisdowns.github.io/assets/intel-zero-opt/little-uptick.png)

This happens right around the transition from L3 to RAM, and this, the writes flatline down to 0.04 per line, but this uptick is fairly consistently reproducible. So there’s some interesting effect there, probably, perhaps related to the adaptive nature of the L3 caching[5](https://travisdowns.github.io/blog/2020/05/18/icelake-zero-opt.html/#fn:l3adapt).
...
Remember that we ended up with 256-bit and 512-bit versions due to unexpected behavior in the `-march` flag. Rather they _relying_ on this weirdness[6](https://travisdowns.github.io/blog/2020/05/18/icelake-zero-opt.html/#fn:gccfix), let’s just write slighly lazy[7](https://travisdowns.github.io/blog/2020/05/18/icelake-zero-opt.html/#fn:lazy) [methods](https://github.com/travisdowns/zero-fill-bench/blob/master/algos.cpp#L151) that explicitly use 256-bit and 512-bit stores but are otherwise identical. `fill256_0` uses 256-bit stores and writes zeros, and I’ll let you pattern match the rest of the names.

Here’s how they perform on my ICL hardware:

**Figure 10**

[\[link to this chart\]](https://travisdowns.github.io/blog/2020/05/18/icelake-zero-opt.html/#fig10) [\[data table\]](https://travisdowns.github.io/misc/tables//intel-zero-opt/fig10.html) [\[raw data\]](https://github.com/travisdowns/zero-fill-bench/tree/master/results/icl/256-512.csv)

[![Figure 10](https://travisdowns.github.io/assets/intel-zero-opt/fig10.svg)](https://travisdowns.github.io/misc/tables//intel-zero-opt/fig10.html)
...
[\[link to this chart\]](https://travisdowns.github.io/blog/2020/05/18/icelake-zero-opt.html/#fig11) [\[data table\]](https://travisdowns.github.io/misc/tables//intel-zero-opt/fig11.html) [\[raw data\]](https://github.com/travisdowns/zero-fill-bench/tree/master/results/icl/256-512-l2-l3.csv)

[![Figure 11](https://travisdowns.github.io/assets/intel-zero-opt/fig11.svg)](https://travisdowns.github.io/misc/tables//intel-zero-opt/fig11.html)
...
[\[link to this chart\]](https://travisdowns.github.io/blog/2020/05/18/icelake-zero-opt.html/#fig12) [\[data table\]](https://travisdowns.github.io/misc/tables//intel-zero-opt/fig12.html) [\[raw data\]](https://github.com/travisdowns/zero-fill-bench/tree/master/results/icl/256-512-l2-l3.csv)

[![Figure 12](https://travisdowns.github.io/assets/intel-zero-opt/fig12.svg)](https://travisdowns.github.io/misc/tables//intel-zero-opt/fig12.html)
...
I checked also the behavior with prefetching off, but the pattern is very similar, except with both approaches having reduced performance in L3 (you can [see for yourself](https://travisdowns.github.io/assets/intel-zero-opt/fig12-nopf.svg)). It is interesting to note that for zero-over-zero stores, the 256-bit store performance _in L3_ is almost the same as the 512-bit store performance _in L2!_ It buys you almost a whole level in the cache hierarchy, performance-wise (in this benchmark).

Normally I’d take a shot at guessing what’s going on here, but this time I’m not going to do it. I just don’t know[9](https://travisdowns.github.io/blog/2020/05/18/icelake-zero-opt.html/#fn:lied). The whole thing is very puzzling, because everything after the L1 operates on a cache-line basis: we expect the fine-grained pattern of stores made by the core, _within a line_ to basically be invisible to the rest of the caching system which sees only full lines. Yet there is some large effect in the L3 and even in RAM[10](https://travisdowns.github.io/blog/2020/05/18/icelake-zero-opt.html/#fn:RAM) related to whether the core is writing a cache line in two 256-bit chunks or a single 512-bit chunk.
...
For fine-grained advice, see the [list in the previous post](https://travisdowns.github.io/blog/2020/05/13/intel-zero-opt.html#tuning-advice).
...
01. It’s actually still using the EVEX-encoded AVX-512 instruction `vmovdqu32`, which is somewhat more efficient here because AVX-512 has more compact encoding of offsets that are a multiple of the vector size (as they usually are). [↩](https://travisdowns.github.io/blog/2020/05/18/icelake-zero-opt.html/#fnref:still512)

02. In this case, the throughput is only halved, versus the 1/4 throughput when we looked at dispatch throttling on SKX, so based on this very preliminary result it seems like the dispatch throttling might be less severe in Ice Lake (this needs a deeper look: we never used stores to test on SKX). [↩](https://travisdowns.github.io/blog/2020/05/18/icelake-zero-opt.html/#fnref:iclbetter)

03. Strictly speaking, a silent writeback is a _sufficient_, but not a _necessary_ condition for elimination, so it is a lower bound on the number of eliminated stores. For all I know, 100% of stores are eliminated, but out of those 4% are written back not-silently (but not in a modified state). [↩](https://travisdowns.github.io/blog/2020/05/18/icelake-zero-opt.html/#fnref:stricly)

04. One reason could be that writing only alternating lines is somewhat more expensive than writing half the data but contiguously. Of course this is obviously true closer to the core, since you touch half the number of the pages in the contiguous case, need half the number of page walks, prefetching is more effective since you cross half as many 4K boundaries (prefetch stops at 4K boundaries) and so on. Even at the memory interface, alternating line writes might be less efficient because you get less benefit from opening each DRAM page, can’t do longer than 64-byte bursts, etc. In a pathological case, alternating lines could be _half_ the bandwidth if the controller maps alternating lines to alternating channels, since you’ll only be accessing a single channel. We could try to isolate this effect by trying more coarse grained interleaving. [↩](https://travisdowns.github.io/blog/2020/05/18/icelake-zero-opt.html/#fnref:altwrites)

05. The L3 is capable of determining if the current access pattern would be better served by something like an MRU eviction strategy, for example when a stream of data is being accessed without reuse, it would be better to kick that data out of the cache quickly, rather than evicting other data that may be useful. [↩](https://travisdowns.github.io/blog/2020/05/18/icelake-zero-opt.html/#fnref:l3adapt)

06. After all, there’s a good chance it will be fixed in a later version of gcc. [↩](https://travisdowns.github.io/blog/2020/05/18/icelake-zero-opt.html/#fnref:gccfix)

07. These are lazy in the sense that I don’t do any scalar head or tail handling: the final iteration just does a full width SIMD store even if there aren’t 64 bytes left: we overwrite the buffer by up to 63 bytes. We account for this when we allocate the buffer by ensuring the allocation is oversized by at least that amount. This doesn’t matter for larger buffers, but it means this version will get a boost for very small buffers versus approaches that do the fill exactly. In any case, we are interested in large buffers here. [↩](https://travisdowns.github.io/blog/2020/05/18/icelake-zero-opt.html/#fnref:lazy)

08. Most likely, the L1 has a single 64 byte wide write port, like SKX, and the commit logic at the head of the store buffer can look ahead one store to see if it is in the same line in order to dequeue two stores in a single cycle. Without this feature, you could _execute_ two stores per cycle, but only commit one, so the long-run store throughput would be limited to one per cycle. [↩](https://travisdowns.github.io/blog/2020/05/18/icelake-zero-opt.html/#fnref:l1port)

09. Well I lied. I at least have some ideas. It may be that the CPU power budget is dynamically partitioned between the core and uncore, and with 512-bit stores triggering the AVX-512 power budget, there is less power for the uncore and it runs at a lower frequency (that could be checked). This seems unlikely given that it should not obviously affect the elimination chance. [↩](https://travisdowns.github.io/blog/2020/05/18/icelake-zero-opt.html/#fnref:lied)

10. We didn’t take a close look at the effect in RAM but it persists, albeit at a lower magnitude. 256-bit zero-over-zero writes are about 10% faster than 512-bit writes of the same type. [↩](https://travisdowns.github.io/blog/2020/05/18/icelake-zero-opt.html/#fnref:RAM)
