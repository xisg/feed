...
published: "2011-11-26T09:04:00Z"
feed: llvm
guid: https://blog.llvm.org/2011/11/llvm-30-type-system-rewrite.html
---
...
### Goals of the type system
 ...
 The [LLVM IR type system](http://llvm.org/docs/LangRef.html#typesystem) is a fairly straight-forward part of the IR. The type system consists of three major parts: primitive types (like 'double' and integer types), derived types (like structs, arrays and vectors), and a mechanism for handling forward declarations of types ('opaque').
...
### The old type system
 ...
 The [LLVM 2.9](http://llvm.org/releases/2.9/docs/LangRef.html#typesystem) type system has all of the straight-forward pieces and represents forward type declarations with an instance of [OpaqueType](http://llvm.org/releases/2.9/docs/LangRef.html#t_opaque). When this type is later resolved (e.g. at link time), a process know as "type refinement" _updates all pointers_ to the old OpaqueType to point to the new definition, mutating the type graph on the fly, and then deletes the original OpaqueType. For example, if a module contained:
...
```
 ...
 then T2 is a PointerType to an OpaqueType. If we resolve %T1 to {} (an empty struct), then %T2 _mutates_ to be a PointerType to the empty StructType. For more information on this, please see the [type resolution section](http://llvm.org/releases/2.9/docs/ProgrammersManual.html#TypeResolve ) of the LLVM 2.9 Programmer's Manual.
...
An even worse problem that this "type updating" problem caused is when you were manipulating and building IR through the LLVM APIs. Because types could move, it was very easy to get dangling pointers, causing a lot of confusion and a lot of broken clients of the LLVM API. This was compounded by the fact that it was _required_ to use type refinement to build [simple recursive types](http://llvm.org/releases/2.9/docs/ProgrammersManual.html#BuildRecType). We tried to make this simpler with the PATypeHolder and PATypeHandle classes, but they only worked if you used them exactly right and they were generally poorly understood.
...
```
 ...
 ... which is simple and to the point, much better than [the 2.9 way](http://llvm.org/releases/2.9/docs/ProgrammersManual.html#BuildRecType). There are a few non-obvious ramifications to this design though.
...
```
 ...
 The first thing the linker does is load the two modules into the same LLVMContext. Since the two types named "A" _must be different_ types, and since there can only be one type named %A, we actually get these two modules in memory:
