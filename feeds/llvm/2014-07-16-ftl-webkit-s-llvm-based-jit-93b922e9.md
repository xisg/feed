...
url: https://blog.llvm.org/2014/07/ftl-webkits-llvm-based-jit.html
published: "2014-07-16T23:00:00Z"
feed: llvm
guid: https://blog.llvm.org/2014/07/ftl-webkits-llvm-based-jit.html
---

# FTL: WebKit’s LLVM based JIT

Over the past year, the WebKit project made tremendous progress on the ability to optimize JavaScript applications. A major part of that effort was the introduction of the Fourth Tier LLVM (FTL) JIT. The Fourth Tier JIT targets long-running JavaScript content and performs a level of optimization beyond WebKit's interpreter, baseline JIT, and high-level optimizing JIT. See the FTL Optimization Strategy section below for more on WebKit's tiered optimizations. The engineering advancements within WebKit that made the FTL possible were described by Filip Pizlo in the Surfin' Safari Blog post, [Introducing the WebKit FTL JIT](http://www.webkit.org/blog/3362/introducing-the-webkit-ftl-jit/). On April 29, 2014, the WebKit team enabled FTL by default on trunk: [r167958](http://trac.webkit.org/changeset/167958).
...
- [FTL Performance](http://blog.llvm.org/2014/07/ftl-webkits-llvm-based-jit.html#ftl-performance)
- [FTL Optimization Strategy](http://blog.llvm.org/2014/07/ftl-webkits-llvm-based-jit.html#ftl-optimization-strategy)
- [LLVM Patch Points](http://blog.llvm.org/2014/07/ftl-webkits-llvm-based-jit.html#llvm-patch-points)
- [FTL-Style LLVM IR](http://blog.llvm.org/2014/07/ftl-webkits-llvm-based-jit.html#ftl-style-llvm-ir)
- [MCJIT and the LLVM C API](http://blog.llvm.org/2014/07/ftl-webkits-llvm-based-jit.html#mcjit-and-the-llvm-c-api)
- [Linking WebKit with LLVM](http://blog.llvm.org/2014/07/ftl-webkits-llvm-based-jit.html#linking-webkit-with-llvm)
- [FTL Efficiency](http://blog.llvm.org/2014/07/ftl-webkits-llvm-based-jit.html#ftl-efficiency)
- [Optimization Improvements](http://blog.llvm.org/2014/07/ftl-webkits-llvm-based-jit.html#optimization-improvements)
- [Extending Patch Points](http://blog.llvm.org/2014/07/ftl-webkits-llvm-based-jit.html#extending-patch-points)
- [Conclusion](http://blog.llvm.org/2014/07/ftl-webkits-llvm-based-jit.html#conclusion)

## [FTL Performance](http://blog.llvm.org/2014/07/ftl-webkits-llvm-based-jit.html)
...
[![](http://4.bp.blogspot.com/--gmTvH6H5ho/U8XprFD79_I/AAAAAAAAG1I/spxgXk6WL4M/s1600/asm_js_ftl_results.png)](http://4.bp.blogspot.com/--gmTvH6H5ho/U8XprFD79_I/AAAAAAAAG1I/spxgXk6WL4M/s1600/asm_js_ftl_results.png)
...
## [FTL Optimization Strategy](http://blog.llvm.org/2014/07/ftl-webkits-llvm-based-jit.html)
...
[![](http://4.bp.blogspot.com/-anUsfTvvZCs/U8XtyOp5l-I/AAAAAAAAG1Y/qjoH0nTncE0/s1600/ftl_pipeline.png)](http://4.bp.blogspot.com/-anUsfTvvZCs/U8XtyOp5l-I/AAAAAAAAG1Y/qjoH0nTncE0/s1600/ftl_pipeline.png)

**Figure 2.** The DFG and FTL JIT optimization pipelines (from [Introducing the WebKit FTL JIT](http://www.webkit.org/blog/3362/introducing-the-webkit-ftl-jit/)).
...
The DFG JIT front end generates LLVM IR in a form that is amenable to the same optimizations traditionally performed with C code. The most notable differences are summarized in [FTL-Style LLVM IR](http://blog.llvm.org/2014/07/ftl-webkits-llvm-based-jit.html#ftl-style-llvm-ir).

[![](http://4.bp.blogspot.com/-Hg0qG-SDMUM/U8XtyKA16tI/AAAAAAAAG1U/jEyHRJGVfqw/s1600/ftl-llvm-pipeline.png)](http://4.bp.blogspot.com/-Hg0qG-SDMUM/U8XtyKA16tI/AAAAAAAAG1U/jEyHRJGVfqw/s1600/ftl-llvm-pipeline.png)
...
## [LLVM Patch Points](http://blog.llvm.org/2014/07/ftl-webkits-llvm-based-jit.html)

Patch points are the key LLVM feature that allows dynamic type checking, inline caching, and runtime safety checks without penalizing performance. In October, 2013, we submitted a [proposal to amend LLVM IR with patch points](http://lists.cs.uiuc.edu/pipermail/llvmdev/2013-October/066573.html) to the LLVM developer list. Since then, we've successfully implemented patch points for multiple architectures and their performance impact has been validated for various use cases, including branch-to-fail safety checks, inline caches, and code invalidation points. The details of the current design are explained in the [LLVM specification of stack map and patch point intrinsics](http://llvm.org/docs/StackMaps.html).
...
## [FTL-Style LLVM IR](http://blog.llvm.org/2014/07/ftl-webkits-llvm-based-jit.html)
...
## [MCJIT and the LLVM C API](http://blog.llvm.org/2014/07/ftl-webkits-llvm-based-jit.html)

The FTL JIT successfully leverages LLVM's existing [MCJIT framework](http://llvm.org/docs/MCJITDesignAndImplementation.html) for runtime compilation. MCJIT was designed as a low-level toolkit that allows runtime compilers to be built by reusing as much of the static compiler's machinery as possible. This approach improves maintainability on the LLVM side. It integrates with the existing compiler toolchain and allows developers to test features of the runtime compiler without understanding a particular JIT client. The current API, however, does not provide a simple out-of-the-box abstraction for portable JITs. Overcoming the impedance mismatch between WebKit goals and the low-level MCJIT API required close collaboration between WebKit and LLVM engineers. As LLVM becomes more important as a JIT platform, it should provide a more complete C API to improve interoperability with JIT clients and decrease the fragility and maintenance burden within the client code base.
...
## [Linking WebKit with LLVM](http://blog.llvm.org/2014/07/ftl-webkits-llvm-based-jit.html)
...
## [FTL Efficiency](http://blog.llvm.org/2014/07/ftl-webkits-llvm-based-jit.html)
...
There are also considerable opportunities for improving code generation efficiency which will benefit JITs and static compilers alike. LLVM machine IR should be generated directly from LLVM IR without generating a Selection DAG, as proposed by Jakob Olesen in his [Proposal for a global instruction selector](http://lists.cs.uiuc.edu/pipermail/llvmdev/2013-August/064727.html). The benefit of this improvement would be considerable and widespread. More specific to high level languages, codegen passes should be tuned to handle branchy code more efficiently. For example, the register allocator can be taught to skip expensive analysis at points in the code where branches are not expected to be executed.
...
## [Optimization Improvements](http://blog.llvm.org/2014/07/ftl-webkits-llvm-based-jit.html)
...
## [Extending Patch Points](http://blog.llvm.org/2014/07/ftl-webkits-llvm-based-jit.html)
...
## [Conclusion](http://blog.llvm.org/2014/07/ftl-webkits-llvm-based-jit.html)
