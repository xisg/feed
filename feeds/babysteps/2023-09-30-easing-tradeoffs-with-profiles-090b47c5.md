...
url: https://smallcultfollowing.com/babysteps/blog/2023/09/30/profiles/?utm_source=atom_feed
published: "2023-09-30T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2023/09/30/profiles/
---
...
The idea of this `clone` is to call attention to the fact that custom code is executing here. This is not just a `memcpy` [1](#fn:1). I’ve been grateful for this some of the time. For example, when optimizing a concurrent data structure, I really like knowing exactly when one of my reference counts is going to change. But a lot of the time, these calls to clone are just noise, and I wish I could just write `let map2 = map` and be done with it.
