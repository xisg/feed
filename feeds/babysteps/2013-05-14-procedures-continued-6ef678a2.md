...
url: https://smallcultfollowing.com/babysteps/blog/2013/05/14/procedures/?utm_source=atom_feed
published: "2013-05-14T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2013/05/14/procedures/
---

# Procedures, continued

So, I didn’t actually _mean_ to post that previous post, I had
...
_UPDATE 2013.05.14 10:30am:_ Edited to tweak various errors and to
...
not tied to any particular stack frame. Instead, they _copy_ the
...
an the `&fn` type can be either a by-reference closure _or_ a copying
...
all—instead, when you create a closure today (i.e., with a `|x, y| ...`) expression, the compiler infers based on the expected types
...
by-reference closures. I propose to use the term _closure_ to refer
only to by-reference, stack-allocated closures. We could then use
another term, perhaps _procedure_, to refer to the copying
...
/blog/2013/04/30/dynamically-sized-types/) post from before with one crucial
difference: closure types have been split from procedures, and closure
types have moved into the category of _sized types_, meaning that you
...
_UPDATE:_ bstrie on IRC asked about fn items, which never have any
...
Closures would still be created with the form `|x, y| expr`. Procedures would be created using the keyword `proc`: `proc(x, y) expr`. If desired, we could integrate procedures into `do` using
...
bounds are only supposed to be constraining the _types_ of the
...
_is_ a complex idea. Combinator types would likely look like `@proc`
