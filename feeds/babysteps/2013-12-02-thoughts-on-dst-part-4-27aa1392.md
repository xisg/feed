...
url: https://smallcultfollowing.com/babysteps/blog/2013/12/02/thoughts-on-dst-4/?utm_source=atom_feed
published: "2013-12-02T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2013/12/02/thoughts-on-dst-4/
---
...
[treat vectors like any other container](https://smallcultfollowing.com/babysteps/
/blog/2013/11/14/treating-vectors-like-any-other-container/), which would (to some
extent) avoid the need for DST.

**Dynamically sized types (DST).** In [Part 1 of the series](https://smallcultfollowing.com/babysteps/
/blog/2013/11/26/thoughts-on-dst-1/), I
sketched out how “Dynamically Sized Types” might work. In that scheme,
`[T]` is interpreted as an [existential type](http://en.wikipedia.org/wiki/Type_system#Existential_types) like
`exists N. [T, ..N]`, and `Trait` is interpreted as `exists T:Trait. T`. The type system ensures that DSTs always appear behind
...
**Statically sized types (SST).** In [Part 2 of the series](https://smallcultfollowing.com/babysteps/
/blog/2013/11/27/thoughts-on-dst-2/), I
...
- **Disadvantage.** Incompatible with monomorphization except via
  ...
  [Part 3 of the series](https://smallcultfollowing.com/babysteps/
  /blog/2013/11/27/thoughts-on-dst-3/). I’ll elaborate a bit more here.
...
- [Treat vectors like any other container](https://smallcultfollowing.com/babysteps/
  /blog/2013/11/14/treating-vectors-like-any-other-container/) combined with SST for
  object types.

### Making DST work with RC requires some contortions

In [part 1](https://smallcultfollowing.com/babysteps/
/blog/2013/11/26/thoughts-on-dst-1/), I gave the example of how we could adapt an `RC`
...
Unfortunately, as Partick [pointed out on reddit](http://www.reddit.com/r/rust/comments/1rkfqq/thoughts_on_dst_part_1/cdo9iv9), this
...
[Limitation: DSTs much appear behind a pointer](https://smallcultfollowing.com/babysteps/
/blog/2013/11/26/thoughts-on-dst-1/#limitation) in
Part 1.
...
OK, so we can see that DST _does_ permit `RC<[int]>`, but only
...
DST imposes. But not entirely. In [part 3](https://smallcultfollowing.com/babysteps/
/blog/2013/11/27/thoughts-on-dst-3/) I described the
...
**First, it is rare that a type can be _entirely_ erased, even in**
...
worse, we don’t _always_ want to free the `T` value of an `RC<T>`,
...
**monomorphized variants anyhow.** Even if we could overcome _all_ the
...
OK, so I hope I’ve convinced you that SST and vector types _just do_
_not mix_.
...
potential for _object types_. There are a couple of crucial
differences here:
...
before, it’s not _necessary_ for the type to be erased, merely
helpful).
...
[treat them like any other container](https://smallcultfollowing.com/babysteps/
/blog/2013/11/14/treating-vectors-like-any-other-container/), and use the SST approach
