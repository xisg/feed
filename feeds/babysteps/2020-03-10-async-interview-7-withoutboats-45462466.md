...
url: https://smallcultfollowing.com/babysteps/blog/2020/03/10/async-interview-7-withoutboats/?utm_source=atom_feed
published: "2020-03-10T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2020/03/10/async-interview-7-withoutboats/
---

# Async Interview #7: Withoutboats

Hello everyone! I’m happy to be posting a transcript of my [async\
interview](http://smallcultfollowing.com/babysteps/blog/2019/11/22/announcing-the-async-interviews/) with withoutboats. This particularly interview took place
...
You can watch the [video](https://youtu.be/a-kZhPMqXRs) on YouTube. I’ve also embedded a copy here
...
[“self-referential structs”](https://boats.gitlab.io/blog/post/2018-01-25-async-i-self-referential-structs/). This blog post was the first in a
series of posts that ultimately documented the design that became the
[`Pin`](https://doc.rust-lang.org/std/pin/struct.Pin.html) type, which describes a pointer to a value that can never be
...
posted a [recorded stream on YouTube](https://www.youtube.com/watch?v=shtfSMTwKRw) that explores its design in
detail.
...
the async-await MVP](https://blog.rust-lang.org/2019/11/07/Async-await-stable.html) – but we’re not there yet. There remain a
...
[`Drop`](https://doc.rust-lang.org/std/ops/trait.Drop.html) trait to work better for async code. The [`Drop`](https://doc.rust-lang.org/std/ops/trait.Drop.html) trait, for
...
[Eliza’s use case](http://smallcultfollowing.com/babysteps/blog/2020/02/11/async-interview-6-eliza-weisman/#raii-and-async-fn-doesnt-always-play-well) with boats. Specifically, she wanted some way to
...
code would _never_ want to do – if you want to get the value from a
...
start, we can simply declare that the future generated by `async fn main` will be executed using `block_on`, which means it will block the
...
runtime crate](https://github.com/rustasync/runtime#attributes), many executors already offer a procedural macro
that lets you write an async main. So, for example, [tokio](https://tokio.rs/) and
[async-std](https://async.rs/) offer attributes called [`#[tokio::main]`](https://book.async.rs/tutorial/accept_loop.html) and
[`#[async_std::main]`](https://docs.rs/async-std/1.5.0/async_std/#examples) respectively, which means that if you have an
...
a _very_ bare-bones executor suitable only for simple examples. To
...
(Side note #2: This _also_ isn’t something that boats and I talked
...
executor. I’m thinking of things like [GStreamer](https://gstreamer.freedesktop.org/) or perhaps iOS with
...
to carry the [`Context`](https://doc.rust-lang.org/std/task/struct.Context.html) about the current async task down through the
...
[jonas-schievink](https://github.com/rust-lang/rust/pull/69033) opened PR [#69033](https://github.com/rust-lang/rust/pull/69033) which does exactly this, though
it’s not yet landed.)
...
more general mechanism of suspendable functions[1](#fn:1), the full power
...
pass in a new [`Context`](https://doc.rust-lang.org/std/task/struct.Context.html) when we wake up from a yield, and we can use
that mechanism to pass in the [`Context`](https://doc.rust-lang.org/std/task/struct.Context.html) argument from the future
API. But this is hidden from the end-user, since that [`Context`](https://doc.rust-lang.org/std/task/struct.Context.html) is
...
implement the [`Stream`](https://docs.rs/futures/0.3.1/futures/stream/trait.Stream.html) trait, which is basically the async version
...
_users_ of async, streams come up very, very often. At the moment,
_consuming_ streams is generally fairly easy, but _creating_ them is
...
**generator** [2](#fn:2), which is some sort of function that can yield up
...
In short, we would want to permit writing a `gen fn` (and `async gen fn`), which would be some function that is able to `yield` values and
...
yields (in the case of streams, we _would_ pass in data, but only the
[`Context`](https://doc.rust-lang.org/std/task/struct.Context.html) values that are not directly exposed to users). Similarly,
...
writing iterators and streams _as easy as we can make it_, which is a
good thing[3](#fn:3). (In case you can’t tell, I was sympathetic to
boats’ argument.)
...
[sfackler proposed in their interview](http://smallcultfollowing.com/babysteps/blog/2020/01/20/async-interview-5-steven-fackler/), where we address the
...
Some time back, boats wrote a post proposing [global executors](https://boats.gitlab.io/blog/post/global-executors/). This
...
- channels
  ...
  - interstingly, channel types _do_ show up in public APIs from time to time, as a way to receive data, so having them in std could be particularly useful
...
I’ve raised this question with others, is _where_ to put async-aware
...
1. In the compiler, we call these “suspendable functions” generators, but I’m avoiding that terminology for a reason. [↩︎](#fnref:1)

2. This is why I was avoiding using the term “generator” earlier – I want to say “suspendable functions” when referring to the implementation mechanism, and “generator” when referring to the user-exposed feature. [↩︎](#fnref:2)
