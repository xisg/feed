---
title: Rivertrail
url: https://smallcultfollowing.com/babysteps/blog/2012/10/10/rivertrail/?utm_source=atom_feed
published: "2012-10-10T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2012/10/10/rivertrail/
---

# Rivertrail

I have started work on implementing [Rivertrail](https://github.com/RiverTrail/RiverTrail/), Intel’s proposal
...
is going to be focused on Intel’s [specification](http://wiki.ecmascript.org/doku.php?id=strawman:data_parallelism), but I hope we
...
[Rivertrail specification](http://wiki.ecmascript.org/doku.php?id=strawman:data_parallelism) for the full list. These methods
...
- First, the function that is taken as argument is required to be
  a _pure_ function (defined shortly).
...
mutate _anything_: it can mutate local variables or objects that it
...
the limitations of the _current_ RiverTrail implementation, but I hope
to lift those particular limitations soon).

This function is _not_ pure, because it modifies `x` which
...
_conceptually_ safe to execute in parallel, but it does not mean that
...
On the other hand, if it _cannot_ analyze this store, in the worst
...
from the [Rivertrail prototype](https://github.com/RiverTrail/RiverTrail/) developed by Intel; the prototype
...
- _Sequentially:_ This is the fallback mode; it is basically equivalent
  ...
  console if you like).
- _Multicore:_ this is the mode we are currently working on. In Multicore
  ...
  over time.
- _Vectorized:_ Vectorized mode is similar to multicore, except that each
...
- _GPU:_ GPU is actually just a variant on vectorized execution in
...
not spent _any_ time optimizing the implementation. That said, here
...
ThreadsSeq (ms)Par (ms)Ratio (Seq/Par)2297625151.183295217821.655296414172.099288011492.5017289111092.60
...
[parallel JavaScript](blog/categories/pjs/), which I codenamed “PJs”. For now those
...
They are extremely easy to use _and_ guarantee that executions are
