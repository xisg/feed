...
url: https://smallcultfollowing.com/babysteps/blog/2012/07/30/type-inference-in-spidermonkey/?utm_source=atom_feed
published: "2012-07-30T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2012/07/30/type-inference-in-spidermonkey/
---

# Type inference in Spidermonkey

I’ve been slowly learning how [type inference](https://wiki.mozilla.org/TypeInference) works in
SpiderMonkey. As I understand it, SpiderMonkey’s type inference
scheme is the brain child of one Brian “Hack-it”, coder
extraordinaire. You may have seen a [recent PLDI publication](http://dl.acm.org/citation.cfm?id=2254094)
...
_entirely_ accurate and it may or may not be helpful.
...
TI has a similar concept called _type objects_. Each JavaScript
...
Each Type Object contains a mapping from properties to _type sets_. A
...
_what types it saw at this particular property load_. This is an
...
have actually been observed. What this means is that merely _storing_
...
“perhaps the algorithm waits until the assignment _occurs_ to do the
...
that the type sets for _loads of the property e_ are updated. When
...
load, the engine adds a _type barrier_. This is basically a note on
...
This does not (yet) modify `L`, however, so the JIT will _still_
...
will check for a string. Once a string is actually _observed_, the
...
code. I don’t _fully_ understand the constraint graph in detail yet.
...
The key idea of the TI system is to be _optimistic_. As much as
