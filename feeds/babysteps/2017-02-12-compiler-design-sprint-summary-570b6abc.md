...
url: https://smallcultfollowing.com/babysteps/blog/2017/02/12/compiler-design-sprint-summary/?utm_source=atom_feed
published: "2017-02-12T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2017/02/12/compiler-design-sprint-summary/
---
...
(e.g., [Cretonne](https://github.com/stoklund/cretonne), [WASM](http://webassembly.org/), and – in its own way – [miri](https://github.com/tsion/miri).). Now that
...
The goal with Cretonne, as [discussed on internals](https://internals.rust-lang.org/t/possible-alternative-compiler-backend-cretonne/4275), is
...
After we had finished admiring [stoklund](https://github.com/stoklund)’s admirable job of writing
...
debuginfo, and in particular some of [stoklund](https://github.com/stoklund)’s very cool ideas to
...
strategy. Almost any optimization we might do requires _some_ notion
...
with a VERY conservative notion that any local which has _ever_ been
...
Finally, we discussed [aatch’s inlining PR](https://github.com/rust-lang/rust/pull/39648), and iterated around
...
- We discussed how to ensure that we can still optimize safe code
  ...
  - Likely the kinds of assertions I was describing in
    [my most recent post on the topic](http://smallcultfollowing.com/babysteps/blog/2017/02/01/unsafe-code-and-shared-references/) aren’t quite right,
...
- We looked some at how LLVM handles dependence analysis and so forth,
  ...
  - The LLVM rules we looked at all seem to fall under the rubrik of
    ...
    resulting pointer). Apparently gcc will do _some_ amount of
...
We discussed integrating the [miri](https://github.com/tsion/miri) interpreter. The initial plan is
...
where [Cretonne’s](https://github.com/stoklund/cretonne) level of abstraction, which is lower than
...
We discussed the desire to support allocas ( [RFC 1808](https://github.com/rust-lang/rfcs/pull/1808)) coupled with
...
extends to support [nested method calls](https://internals.rust-lang.org/t/accepting-nested-method-calls-with-an-mut-self-receiver/4588) where the outer call has an
...
[Comments? Check out the internals thread.](http://smallcultfollowing.com/babysteps/blog/2017/02/12/compiler-design-sprint-summary/)
