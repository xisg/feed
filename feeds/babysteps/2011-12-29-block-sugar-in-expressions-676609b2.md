...
url: https://smallcultfollowing.com/babysteps/blog/2011/12/29/block-sugar-in-expressions/?utm_source=atom_feed
published: "2011-12-29T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2011/12/29/block-sugar-in-expressions/
---
...
Today we attempt to determine _in the parser_ whether an expression
...
others that I have asked find the other alternative ( `foo {|| ...} - 10` as an expression) visually hard to parse. If you want this, use
...
like `func(v) {...}`; while loops _always_ have unit type but
...
`if/else` is classified the same as an `if/else` like `if cond { 10 } else { 20 }` by the parser. These “value-bearing” if/else expressions
...
insufficient context: the _block itself_ appears in an expression
...
would have to put a semicolon. This leads us to our _next_ solution.
