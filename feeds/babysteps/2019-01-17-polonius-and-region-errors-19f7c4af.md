...
url: https://smallcultfollowing.com/babysteps/blog/2019/01/17/polonius-and-region-errors/?utm_source=atom_feed
published: "2019-01-17T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2019/01/17/polonius-and-region-errors/
---

# Polonius and region errors

Now that NLL has been shipped, I’ve been doing some work revisiting
[the Polonius project](https://github.com/rust-lang-nursery/polonius/). Polonius is the project that implements
[the “alias-based formulation” described in my older\
blogpost](https://smallcultfollowing.com/babysteps/
/blog/2018/04/27/an-alias-based-formulation-of-the-borrow-checker/). Polonius has come a long way since that post; it’s now
...
If you’ve never read the [the original Polonius post](https://smallcultfollowing.com/babysteps/
/blog/2018/04/27/an-alias-based-formulation-of-the-borrow-checker/), you should probably
do so now. But if you have, let me briefly review some of the key details
that are relevant to this post:

- Instead of interpreting the `'a` notation as the _lifetime_ of a
  reference (i.e., a set of points), we interpret `'a` as a _set of_
  _**loans**_. We refer to `'a` as a “region”[1](#fn:1) in order to
...
- `base_subset('b, 'X, mid(S0))` – as [described in the NLL\
  RFC](https://rust-lang.github.io/rfcs/2094-nll.html#reborrow-constraints), “re-borrowing” the referent of a reference (i.e.,
  `*_2`) creates a subset relation between the region of the region
  (here, `'b`) and the region of the borrow (here, `'X`). Written in
  the notation of the \[NLL RFC\], this would be the relation `'X: 'b @ mid(S0)`.
- `base_subset('X, 'a, mid(S0))` – the borrow expression in S0
  produces a result of type `&'X u32`. This is then assigned to `_0`,
  which has the type `&'a [u32]`. The [subtyping rules](https://rust-lang.github.io/rfcs/2094-nll.html#subtyping)
  require that `'X: 'a`.
...
for _any_ set of regions `'a` and `'b`, and hence we cannot know if
...
region like `'X` there only has to be _some_ valid value. This
difference is often called being _universally quantified_ (true for
all regions) versus _existentially quantified_ (true for _some_
region).
...
this[2](#fn:2), because we need to account for the fact that
...
Even placeholder regions are not _totally_ unknown though. The
...
regions. For example, if `foo` included a where clause like `where 'b: 'a`, then it would be perfectly legal.
...
polonius post](https://internals.rust-lang.org/t/blog-post-an-alias-based-formulation-of-the-borrow-checker/7411)
...
1. The term “region” is not an especially good fit, but it’s common in academia. [↩︎](#fnref:1)

2. Currently called `universal_region`, though I plan to rename it. [↩︎](#fnref:2)
