...
url: https://smallcultfollowing.com/babysteps/blog/2013/06/11/data-parallelism-in-rust/?utm_source=atom_feed
published: "2013-06-11T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2013/06/11/data-parallelism-in-rust/
---
...
[“Three layer cake for shared-memory programming”](http://dl.acm.org/citation.cfm?id=1953616), which
...
to statically guarantee data-race freedom _even with full support for_
_shared memory_ with only minimal, generally applicable modifications
...
forth). In keeping with the [“three layer cake”](http://dl.acm.org/citation.cfm?id=1953616) idea, these
...
rules for closures (more accurately, it _will_ fall out of those rules
...
- `&mut T` pointers are the _only way_ to modify the memory that they
  point at (thus guaranteeing that if job A has an `&mut` pointer,
  no other job can be writing that memory);
- `&T` pointers are _immutable_ (thus guaranteeing that if job A has a
  `&T` pointer, no job can be writing to that memory).

However, there are several guarantees that the borrow checker does
_not_ provide which are unnecessary in a sequential context but become
important when discussing data races:

- Although `&mut T` pointers are the only way to _mutate_ the memory
  they point at, they are not the only way to _read_ the memory they
  point at. Both `&const T` and `@mut T` can produce read-only
  aliases; thus is one job holds an `&mut T` and another an `&const T`, the two jobs could race.
...
the closure were close over an `&mut` pointer, that _same pointer_
would be available to many threads, and thus races could arise.

##### Enter closure bounds

We prevent both of these scenarios with _closure bounds_. A closure
...
the _desugared references_ found in a closure environment. Recall
...
the _variable_ `foo` is just `uint`, which does meet `Freeze`.
...
will have borrowed pointers of _some kind_ in its environment, and
they are not sendable.

_Hat tip_: Promising Young Intern [bblum](http://winningraceconditions.blogspot.com/) first expressed this
...
You may recall that the `parallel::execute()` fn was declared as `fn execute(jobs: &[fn:Isolate()])`. As the type indicates, it accepts an
...
between the _parallel jobs_ described here and the existing tasks are
that _jobs_ would have access to borrowed pointers and thus to data
...
in the [“three layer cake” of parallel programming](http://dl.acm.org/citation.cfm?id=1953616), enabling
