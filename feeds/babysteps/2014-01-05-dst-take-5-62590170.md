---
title: DST, Take 5
url: https://smallcultfollowing.com/babysteps/blog/2014/01/05/dst-take-5/?utm_source=atom_feed
published: "2014-01-05T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2014/01/05/dst-take-5/
---
...
can be [found here](https://github.com/nikomatsakis/rust-redex). Currently it only includes reduction
...
_existential variants_ of existing types. That is, there are
...
that implements `Trait`, and hence could be written `exists T:Trait. T`. Note that I do not propose adding existential syntax into
...
is. Hence I refer to these types as _dynamically sized_ – because the
size of their instances is not known at compilation time. More often,
I am sloppy and just call them _unsized_, because everybody knows that
...
Pointers to instances of dynamically sized types are _fat pointers_ –
...
here was basically that, since a type declaration never _invokes_
_methods_, it doesn’t need bounds, and we could mildly simplify things
by leaving them out.
...
is not actually a pointer to an unsized type. It is a struct that _contains_
...
conversion from _thin pointers_ to _fat pointers_. This is relatively
...
itself must be _sized_. This is less obvious and perhaps a bit
...
rule would therefore permit `Fat(RcData<[int, ..3]> as RcData<[int]>) = 3`, for example, but not `Fat(RcData<int> as RcData<float>)`.
...
_Note:_ As implied by my wording above, I expect there will be a small
...
`x` was _declared_ with unsized type `U` and hence has maximal
alignment).
