...
url: https://smallcultfollowing.com/babysteps/blog/2016/11/04/associated-type-constructors-part-3-what-higher-kinded-types-might-look-like/?utm_source=atom_feed
published: "2016-11-04T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2016/11/04/associated-type-constructors-part-3-what-higher-kinded-types-might-look-like/
---
...
1. [The first post](https://smallcultfollowing.com/babysteps/
   /blog/2016/11/02/associated-type-constructors-part-1-basic-concepts-and-introduction/) focused on introducing the basic idea of
   ATC, as well as introducing some background material.
2. [The second post](https://smallcultfollowing.com/babysteps/
   /blog/2016/11/03/associated-type-constructors-part-2-family-traits/) showed how we can use ATC to model HKT,
...
`CollectionFamily` trait was actually _necessary_. The answer is that
...
For some more thoughts on that, see [my comment on internals](https://internals.rust-lang.org/t/blog-post-series-alternative-type-constructors-and-hkt/4300/28?u=nikomatsakis).
...
[Scala’s syntax](http://blogs.atlassian.com/2013/09/scala-types-of-a-higher-kind/) here), we have declared that `I` represents a
_type constructor_, meaning something like `List`. To be a bit more
...
introduced by [RFC 1598](https://github.com/rust-lang/rfcs/pull/1598), and discussed in
[the previous post](https://smallcultfollowing.com/babysteps/
/blog/2016/11/03/associated-type-constructors-part-2-family-traits/). Basically we are saying that `I<T>` is
...
to keep secret. Consider this function `process()` [1](#fn:1), which takes a
...
`MyType` outside of the fn body[2](#fn:2). So being able to say something like
...
defined over _collection type constructors_ (like `List<_>`). So we
...
_type_, but rather to the type _constructor_.
...
in Rust tend to apply _some_ bounds on that. For example, `Vec<T>` and
`List<T>` both (implicitly) require that `T: Sized`. Or, if you have
`HashMap<K,V>`, you might consider it to be a collection of pairs `(K, V)`, except that it only works if `K: Hash + Eq + Sized` and `V: Sized`.
...
[“Type Classes: Exploring the Design Space” by Jones et al.](http://research.microsoft.com/en-us/um/people/simonpj/Papers/type-class-design-space/),
...
- Section 2.1, “Overloading with coupled parameters” basically talks
  about the idea that impls might not always apply to _all_ types. So
  ...
  - In these situations, it is often (but not always) the case that
    ...
    type. For example, we might have changed `trait Collection<Item> { ... }` to `trait Collection { type Item; ... }`. This would have
    ...
    - I talked some about this tradeoff in the “Things I learned”
      section from [my post on Rayon](http://smallcultfollowing.com/babysteps/blog/2016/02/25/parallel-iterators-part-2-producers/); the rule of thumb I
...
HKT. In particular, I want to talk more about what a _kind_ is and why
we could call a type constructor like `List<_>` _higher_-kinded. The
idea is that just like types tell us what sort of value we have (e.g.,
`i32` vs `f32`), kinds tell us what sort of _generic parameter_ we have.
...
And so higher-kinded type parameters _are_ kind of like functions,
except that instead of calling them at runtime ( `foo(22)`), you
_apply_ them to types ( `Foo<i32>`). In general, when we can talk about
...
`I<'_>`, then you could apply it like `I<'foo>`, and – assuming `I = ListIter<'_, i32>` – you would get `ListIter<'foo, i32>`.

Speaking more generally, we can say that the _kind_ `K` of a type
...
not parameterized by another type, but rather by a type _constructor_,
...
you could supply _any_ kind of function which, given one type, yields
...
can be _any_ “type lambda”, then `I` could be a lambda that returns
...
traits from the [previous post](https://smallcultfollowing.com/babysteps/
/blog/2016/11/03/associated-type-constructors-part-2-family-traits/)! Let’s go back in time to that
...
Basically here the `F` is playing _exactly_ this “type lambda” role.
`F::Collection<T>` is the same as `I<T>`. Moreover, using impls and
traits, we can write arbitrary, [turing-complete](https://www.reddit.com/r/rust/comments/2o6yp8/brainfck_in_rusts_type_system_aka_type_system_is/) functions on types!

(Note: it sounds like being turing-complete is hard; it’s not. It’s
actually hard to _avoid_ once you start adding in any reasonably
...
_function_ that maps between them, at least not uniquely.

As an analogy, it’d be like if I told you “ok, so `f(1) = 2` and `f(2) = 3`, what is the function `f`?”. Naturally there is no unique
...
_suffix_ of parameters left blank.
...
(This scheme is called _currying_ in Haskell and it’s actually really
...
`&` to a particular lifetime ( `'a`), yielding a HKT like `type -> type`. This is good for those cases where you wish to treat `&'a T`
...
_suffix_ of type parameters unapplied, and that is just (in practice)
...
because that’s a _prefix_ of the types we need, not a _suffix_.

One strategy that might work ok in practice is to say, in Rust, you
can name a HKT by putting an `_` on some _prefix_ of the parameters
_for any given kind_. So e.g. we can do the following:
...
- `?T<?U> = Rc<i32>`, solvable:
  ...
  - but not that this only works because _all_ type parameters of `Rc` were made into `_`
...
- `?T<?U, ?V> = Result<i32, u32>`, solvable if we assume that ordering must be respected:
  ...
  - again, this only works because _all_ type parameters of `Result` were made into `_`

So, essentially, choosing a prefix (or suffix) is actually _more_
...
partially apply a type alias. This is precisely because `Unresult<X, Y>` is a _distinct type_ from `Result<Y, X>`, which is not the case
with a type alias.)
...
There is also a _subset_ of the higher-order unification called
...
- Higher-kinded types let you use a _type constructor_ as a parameter;
...
- Higher-ranked trait bounds (which Haskell doesn’t offer, but which
  are part of the [ATC RFC](https://github.com/rust-lang/rfcs/pull/1598)) permit functions to declare
...
- What Haskell _does_ offer as an alternative is traits whose `Self` type is higher-kinded.
  ...
  - Note that _collection families_ naturally permit one to apply where conditions and side clauses.
- Higher-kinded types in Haskell are limited to a “curried type declaration”:
  ...
  - One thing that might work is that one can omit a _prefix_ of the type parameters
    of any given kind.
...
1. I was not able to find an even-mildly-convincing variant on `floatify` for this. =) [↩︎](#fnref:1)

2. This problem is not specific to types declared in fn bodies;
one can easily construct similar examples where it would be
necessary to make private structs public. [↩︎](#fnref:2)
