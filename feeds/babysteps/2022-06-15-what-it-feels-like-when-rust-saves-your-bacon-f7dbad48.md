...
url: https://smallcultfollowing.com/babysteps/blog/2022/06/15/what-it-feels-like-when-rust-saves-your-bacon/?utm_source=atom_feed
published: "2022-06-15T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2022/06/15/what-it-feels-like-when-rust-saves-your-bacon/
---

# What it feels like when Rust saves your bacon

You’ve probably heard that the Rust type checker can be a great “co-pilot”, helping you to avoid subtle bugs that would have been a royal pain in the !@#!$! to debug. This is truly awesome! But what you may not realize is how it feels _in the moment_ when this happens. The answer typically is: **really, really frustrating!** Usually, you are trying to get some code to compile and you find you just can’t do it.
...
All the code for this blog post is available in a [github repository](https://github.com/nikomatsakis/2022-06-15-blogpost/).

## Setting the scene: lowering the AST

In the compiler, we first represent Rust programs using an [Abstract Syntax Tree (AST)](https://doc.rust-lang.org/nightly/nightly-rustc/rustc_ast/ast/index.html). I’ve prepared a [standalone example](https://github.com/nikomatsakis/2022-06-15-blogpost/blob/f280f91e9be03d37f273acf13502ef7dc1015db8/examples/a.rs) that shows roughly how the code looks today (of course the real thing is a lot more complex). The AST in particular is found in the [ast module](https://github.com/nikomatsakis/2022-06-15-blogpost/blob/f280f91e9be03d37f273acf13502ef7dc1015db8/examples/a.rs#L4) containing various data structures that map closely to Rust syntax. So for example we have a `Ty` type that represents Rust types:
...
The `impl Trait` notation references a [`TraitRef`](https://github.com/nikomatsakis/2022-06-15-blogpost/blob/f280f91e9be03d37f273acf13502ef7dc1015db8/examples/a.rs#L12-L15), which stores the `Trait` part of things:
...
After parsing, this AST gets translated to something called [High-level Intermediate Representation (HIR)](https://doc.rust-lang.org/nightly/nightly-rustc/rustc_middle/hir/index.html) through a process called _lowering_. The snippet doesn’t include the HIR, but it includes a number of methods like [`lower_ty`](https://github.com/nikomatsakis/2022-06-15-blogpost/blob/8232a6ee30e92faa2117dd23ee28d5c145509d92/examples/a.rs#L98-L116) that take as input an AST type and produce the HIR type:
...
Each method is defined on this `Context` type that carries some common state, and the methods tend to call one another. For example, [`lower_signature`](https://github.com/nikomatsakis/2022-06-15-blogpost/blob/f280f91e9be03d37f273acf13502ef7dc1015db8/examples/a.rs#L57-L65) invokes [`lower_ty`](https://github.com/nikomatsakis/2022-06-15-blogpost/blob/8232a6ee30e92faa2117dd23ee28d5c145509d92/examples/a.rs#L98-L116) on all of the input (argument) types and on the output (return) type:
...
Of course, now we start getting errors in the functions that _call_ `lower_ty`. For example, `lower_signature` says:
...
Jumping to that line, we see this function [`lower_trait_ref`](https://github.com/nikomatsakis/2022-06-15-blogpost/blob/f280f91e9be03d37f273acf13502ef7dc1015db8/examples/b.rs#L85-L97):
...
So what’s this about? Well, the _purpose_ of this code is a bit clever. As we saw before, Rust has two syntaxes for trait-refs, you can use parentheses like `FnOnce(u32)`, in which case you only have types, or you can use angle brackets like `Foo<'a, u32>`, in which case you could have either lifetimes _or_ types. So this code is normalizing to the angle-bracket notation, which is more general, and then using the same lowering helper function.
...
Well, there are a variety of ways you _might_ fix it. One of them is to use an _arena_, like the [`typed-arena`](https://crates.io/crates/typed-arena) crate. An arena is a memory pool. Instead of storing the temporary `Vec<Parameter>` vector on the stack, we’ll put it in an arena, and that way it will live for the entire time that we are lowering things. [Example C](https://github.com/nikomatsakis/2022-06-15-blogpost/blob/f280f91e9be03d37f273acf13502ef7dc1015db8/examples/c.rs) in the repo takes this approach. It starts by adding the `arena` field to the [`Context`](https://github.com/nikomatsakis/2022-06-15-blogpost/blob/f280f91e9be03d37f273acf13502ef7dc1015db8/examples/c.rs#L54-L60):
...
## Appendix: why not have `Context` _own_ the `TypedArena`?
...
This is because `Context<'ast>` stores a `&'ast TypedArena<_>`, and so the caller must create the arena. If we modified `Context` to _own_ the arena, then the API could be better. So why didn’t I do that? To see why, check out [example D](https://github.com/nikomatsakis/2022-06-15-blogpost/blob/f280f91e9be03d37f273acf13502ef7dc1015db8/examples/c.rs) (which doesn’t build). In that example, the `Context` looks like…
...
This is saying: the `'ast` parameter might refer to data owned by self, or maybe by sig. Seems sensible, but if you try to build [Example D](https://github.com/nikomatsakis/2022-06-15-blogpost/blob/f280f91e9be03d37f273acf13502ef7dc1015db8/examples/c.rs), though, you get lots of errors. Here is one of the most interesting to me:
