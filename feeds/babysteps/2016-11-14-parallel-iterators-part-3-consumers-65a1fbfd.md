...
url: https://smallcultfollowing.com/babysteps/blog/2016/11/14/parallel-iterators-part-3-consumers/?utm_source=atom_feed
published: "2016-11-14T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2016/11/14/parallel-iterators-part-3-consumers/
---
...
1. The first post, [“Foundations”](https://smallcultfollowing.com/babysteps/
   /blog/2016/02/19/parallel-iterators-part-1-foundations/), explains how sequential
   iterators work. It is also a nice introduction to some of the key
   techniques for zero-cost abstraction.
2. The second post, [“Producers”](https://smallcultfollowing.com/babysteps/
   /blog/2016/02/25/parallel-iterators-part-2-producers/), then shows how we can adapt
...
In the [second post](https://smallcultfollowing.com/babysteps/
/blog/2016/02/25/parallel-iterators-part-2-producers/), I introduced the basics of how parallel
...
that [I covered in my previous blog post](https://smallcultfollowing.com/babysteps/
/blog/2016/02/25/parallel-iterators-part-2-producers/#implementing-sum-with-producers);
`sum_producer()` basically executes the `sum()` operation, but
extracting data from a producer. Later on in the post, we’re going to
see how consumers abstract out the _sum_ part of this code, leaving us
...
1. You start out with one producer/consumer pair; using `split_at()`,
   ...
   called a _reducer_, we’ll get to its role in a bit.
2. At some point, to process sequentially, you convert the producer
   ...
   a _folder_ using `into_folder()`. You then draw items from the
...
3. As we walk back up the stack, at each point where we had split the
   ...
   using the _reducer_ (also returned by `split_at()`).

Let’s take a closer look at the folder and reducer. Folders are
defined by [the `Folder` trait](https://github.com/nikomatsakis/rayon/blob/a0047facd2df584c771775bd8812c02f915e577c//src/par_iter/internal.rs#L60-L72), a simplified version of which
...
_reducer_ that knows how to combine those results back
again. [The `Reducer` trait](https://github.com/nikomatsakis/rayon/blob/a0047facd2df584c771775bd8812c02f915e577c//src/par_iter/internal.rs#L74-L78) is shown below. It just consists
...
[this function is called `bridge_producer_consumer`](https://github.com/nikomatsakis/rayon/blob/a0047facd2df584c771775bd8812c02f915e577c//src/par_iter/internal.rs#L170-L197). Here is a
...
[written in terms of a more efficient primitive](https://github.com/nikomatsakis/rayon/blob/a0047facd2df584c771775bd8812c02f915e577c/src/par_iter/from_par_iter.rs#L39-L43),
...
good for [double buffering](https://en.wikipedia.org/wiki/Double_buffering) scenarios. To use `collect_into()`
...
(perhaps with `0` values). The [real version](https://github.com/nikomatsakis/rayon/blob/a0047facd2df584c771775bd8812c02f915e577c/src/par_iter/collect/consumer.rs) works
...
_unique_ reference, which means that we don’t have to worry about
...
work. Whereas the [previous post](https://smallcultfollowing.com/babysteps/
/blog/2016/02/25/parallel-iterators-part-2-producers/) introduced parallel
