...
url: https://smallcultfollowing.com/babysteps/blog/2017/04/23/unification-in-chalk-part-2/?utm_source=atom_feed
published: "2017-04-23T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2017/04/23/unification-in-chalk-part-2/
---

# Unification in Chalk, part 2

In my previous post, I talked over the basics of how
[unification works](https://smallcultfollowing.com/babysteps/
/blog/2017/03/25/unification-in-chalk-part-1/) and showed how that “mathematical version” winds
up being expressed in chalk. I want to go a bit further now and extend
that base system to cover [associated types](https://doc.rust-lang.org/nightly/book/second-edition/ch19-03-advanced-traits.html#associated-types). These turn out to be a
...
is “projecting out”[1](#fn:1) the type `Item`.

Let’s look at an impl to make this more concrete. Consider
[the type `std::vec::IntoIter<T>`](https://doc.rust-lang.org/std/vec/struct.IntoIter.html), which is one of the iterators
...
`Vec<?T> = Vec<i32>`, we knew that this could **only** be true if `?T == i32`.
...
elements: but we have no way of knowing _which_ iterator it is, there
...
`<IntoIter<i32> as Iterator>::Item`, when _normalized_, should be
...
same code. If [you recall from my first Chalk post](https://smallcultfollowing.com/babysteps/
/blog/2017/01/26/lowering-rust-traits-to-logic/), we can
...
this case, if we look at the impl of `Iterator` for [the `IntoIter` type](https://doc.rust-lang.org/std/vec/struct.IntoIter.html):
...
(here, I’ll use [the notation I was using in my first post](https://smallcultfollowing.com/babysteps/
/blog/2017/01/26/lowering-rust-traits-to-logic/#associated-types-and-type-equality)):
...
So, to solve the normalization constraint `<IntoIter<i32> as Iterator>::Item ==> ?X`, we translate that into the goal
...
[in my first blog post](https://smallcultfollowing.com/babysteps/
/blog/2017/01/26/lowering-rust-traits-to-logic/), but I’ll present it in a bit more detail
...
In the [previous post](https://smallcultfollowing.com/babysteps/
/blog/2017/03/25/unification-in-chalk-part-1/), I gave [various pointers](http://smallcultfollowing.com/babysteps/blog/2017/03/25/unification-in-chalk-part-1/#how-this-is-implemented) into the
...
its own subgoals. So e.g. when we transform `IntoIter<i32> = IntoIter<?T>`, we actually just
[invoke the code to equate their arguments immediately](https://github.com/nikomatsakis/chalk/blob/6a7bb25402987421d93d02bda3f5d79bf878812c/src/solve/infer/unify.rs#L107-L109).

The one exception to this is normalization goals. In that case, we
push the goals into
[a separate list that is returned to the caller](https://github.com/nikomatsakis/chalk/blob/6a7bb25402987421d93d02bda3f5d79bf878812c/src/solve/infer/unify.rs#L41). The
...
sometimes we want to work with generics we _can’t_ normalize the
...
The second is that we can prove it just by having all the _inputs_ be equal:
...
quantification etc](http://smallcultfollowing.com/babysteps/blog/2017/01/26/lowering-rust-traits-to-logic/#type-checking-generic-functions-beyond-horn-clauses)):
...
involved. [Rust’s trait matching is turing complete](https://sdleffler.github.io/RustTypeSystemTuringComplete/), after all.
However, we _do_ wish to ensure that our own unification algorithms
...
type nontheless. When that _applicative_ type is processed later, it
...
`<Iterator::Item><T>`). The distinction between an unnormalized `<T as Iterator>::Item` and one that we failed to normalize further is made
...
based on congruence closure, and even implemented (in [ena](https://crates.io/crates/ena))
[an extended version of the algorithm described here](http://www.alice.virginia.edu/~weimer/2011-6610/reading/nelson-oppen-congruence.pdf). However,
...
(e.g, [Selsam and de Moura](https://arxiv.org/pdf/1701.04391.pdf)), but haven’t yet had time to read
it.

### Comments?

I’ll be monitoring [the internals thread](https://internals.rust-lang.org/t/blog-series-lowering-rust-traits-to-logic/4673) for comments and discussion. =)

### Footnotes

* * *

1. Projection is a very common bit of jargon in PL circles, though it typically refers to accessing a field, not a type. As far as I can tell, no mainstream programmer uses it. Ah well, I’m not aware of a good replacement. [↩︎](#fnref:1)
