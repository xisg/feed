...
url: https://smallcultfollowing.com/babysteps/blog/2019/11/22/announcing-the-async-interviews/?utm_source=atom_feed
published: "2019-11-22T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2019/11/22/announcing-the-async-interviews/
---
...
also what the _specific concerns_ are around stabilizing particular
traits or macros. What sorts of libraries do they enable? And so
forth. (You can view my rough [interview script](https://gist.github.com/nikomatsakis/ae2ede32c4c7d49cbda088a1539724d9), but I plan to tailor
...
the [#wg-async-foundations stream](https://rust-lang.zulipchat.com/#narrow/stream/187312-wg-async-foundations) on [the rust-lang Zulip](https://rust-lang.zulipchat.com) and say
hi!

### So what _are_ the things we might do now that async-await is stable?

If you take a look at my rough [interview script](https://gist.github.com/nikomatsakis/ae2ede32c4c7d49cbda088a1539724d9), you’ll see a long
...
library. It was removed in [RFC #230](https://gist.github.com/nikomatsakis/ef21d903717ef20b8bbf4ae5c1c03ba0), and a big part of the
motivation was that we knew we were unlikely to find a _single runtime_
_design_ that was useful for all tasks. And, even if we could, we
certainly knew we hadn’t found it _yet_. Therefore, we opted to pare
...
- [fuschia’s runtime](https://fuchsia.googlesource.com/), used for the Fuschia work at Google;
- [tokio](https://tokio.rs/), a venerable, efficient runtime with a rich feature set;
- [async-std](https://async.rs/), a newer contender which aims to couple libstd-like APIs
  with highly efficient primitives;
- [bastion](https://bastion.rs/), exploring a resilient, Erlang-like model[2](#fn:2);
- [embrio-rs](https://github.com/Nemo157/embrio-rs), exploring the embedded space.
...
[quinn](https://github.com/djc/quinn), combine it with “middleware” crates like [async-compression](https://github.com/Nemo157/async-compression),
...
_next_ steps we can take to help improve things further?
...
[`Stream`](https://docs.rs/futures/0.3.1/futures/stream/trait.Stream.html), [`AsyncRead`](https://docs.rs/futures/0.3.1/futures/io/trait.AsyncRead.html), and [`AsyncWrite`](https://docs.rs/futures/0.3.1/futures/io/trait.AsyncWrite.html). Maybe some of these
...
- Regardless of how we signal stability, I also want to talk about the
  ...
  - What are the specific concerns that remain about the design of
    this item? The [`AsyncRead`](https://docs.rs/futures/0.3.1/futures/io/trait.AsyncRead.html) and [`AsyncWrite`](https://docs.rs/futures/0.3.1/futures/io/trait.AsyncWrite.html) traits, for
    example, presently align quite closely with their synchronous
    counterparts [`Read`](https://doc.rust-lang.org/std/io/trait.Read.html) and [`Write`](https://doc.rust-lang.org/std/io/trait.Write.html). However, this interface
    does require that the buffer used to store data must be
    zeroed. The [tokio](https://tokio.rs/) crate is [considering altering its own local\
    definition of `AsyncRead`](https://github.com/tokio-rs/tokio/pull/1744) for this reason, is that
    something we should consider as well? If so, how?
- On a broader note, what are the sorts of things crates need to truly
  operate that are _not_ covered by the existing traits? For example,
  the [global executors](https://boats.gitlab.io/blog/post/global-executors/) that boats recently proposed would give
...
- **Async destructors.** As boats described [in this blog post](https://boats.gitlab.io/blog/post/poll-drop/),
...
- **Async fn in traits.** We support `async fn` in free functions and
  ...
  post](http://smallcultfollowing.com/babysteps/blog/2019/10/26/async-fn-in-traits-are-hard/), there are a lot of challenges to support async fn in
  traits properly (but consider using [the `async-trait` crate](https://crates.io/crates/async-trait)).
- **Async closures.** Currently, we support async _blocks_ ( `async move { .. }`), which evaluate to a future, and async _functions_
  ( `async fn foo()`), which are a function that returns a future. But,
  at least on stable, we have no way to make a _closure_ that returns
  a future. Presumably this would be something like `async || { ... }`. (In fact, on nightly, we do have support for async
  closures, but there are some issues in the design that we need to
  work out.)
- **Combinator methods like [`map`](https://docs.rs/futures/0.3.1/futures/future/trait.FutureExt.html#method.map), or macros like [`join!`](https://docs.rs/futures/0.3.1/futures/macro.join.html) and**
  **[`select!`](https://docs.rs/futures/0.3.1/futures/macro.select.html).** The futures crate offers a number of useful combinators
...
1. Indeed, shortly after I published this post, I [was directed](https://twitter.com/redtwitdown/status/1198001288648069120) to the [drone-os](https://www.drone-os.com/) project. [↩︎](#fnref:1)

2. Woohoo! I just want to say that I’ve been hoping to see something like OTP for Rust for…quite some time. [↩︎](#fnref:2)
