...
url: https://smallcultfollowing.com/babysteps/blog/2013/10/10/fn-types-in-rust/?utm_source=atom_feed
published: "2013-10-10T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2013/10/10/fn-types-in-rust/
---
...
[we could not soundly permit closures to recurse](https://smallcultfollowing.com/babysteps/
/blog/2013/04/30/the-case-of-the-recurring-closure/), which implies
that closures must be tracked in a linear fashion. This caused
interactions with the prior plans we had for
[dynamically sized types](https://smallcultfollowing.com/babysteps/
/blog/2013/04/30/dynamically-sized-types/), and led to [several](https://smallcultfollowing.com/babysteps/
/blog/2013/05/13/mutable-fn-alternatives/) [alternative](https://smallcultfollowing.com/babysteps/
/blog/2013/05/14/procedures/) [proposals](https://smallcultfollowing.com/babysteps/
/blog/2013/05/30/removing-procs/). The most radical involved keeping only one closure
...
into two groups, which I will call _closures_ and _procedures_. These
...
The type `|T, U| -> R` is called a _closure_. A closure is a function
...
A _procedure_ `proc(T, U) -> R` is intended to be the means for
...
- `'a ... 'z` are _named lifetime parameters_ that may appear in the
  argument or return type. While it is not so unusual to use named
  lifetime parameters in function declarations, it is _very_ rare to
  see them in function _types_. The current codebase contains no
...
- The closure bounds `K` describe the closed-over variables in the
  ...
  [`Isolate` and `Share`](https://smallcultfollowing.com/babysteps/
  /blog/2013/06/11/data-parallelism-in-rust/) and others in the future). It would be
...
[in this previous blog post](https://smallcultfollowing.com/babysteps/
/blog/2013/01/17/destructors-and-finalizers-in-rust/), but we have plans to generalize
...
to exist, they capture any variables that they access _by value_,
...
`A` and `B` are the inferred types for the parameters ( `proc(a: A, b: B) -> () { ... }` would be the fully explicit form).
...
liftime parameters: the type `|&T| -> U` is equivalent to `<'a> |&'a T| -> U`. The trait equivalent would be `<'a> Fn<&'a T, U>` except
