...
url: https://smallcultfollowing.com/babysteps/blog/2018/06/15/mir-based-borrow-check-nll-status-update/?utm_source=atom_feed
published: "2018-06-15T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2018/06/15/mir-based-borrow-check-nll-status-update/
---
...
check” that I outlined in [my previous post](https://smallcultfollowing.com/babysteps/
/blog/2018/04/27/an-alias-based-formulation-of-the-borrow-checker/), which we have since
dubbed [Polonius](https://github.com/rust-lang-nursery/polonius/). We have implemented that analysis and solved the
...
group hangs out [on the `#wg-nll` stream in Zulip](https://rust-lang.zulipchat.com/#narrow/stream/122657-wg-nll). We have
...
amongst [the list of GitHub issues](https://github.com/rust-lang/rust/issues?utf8=%E2%9C%93&q=is%3Aopen+label%3AWG-compiler-nll+-label%3ANLL-deferred) – probably the [diagnostics\
issues](https://github.com/rust-lang/rust/labels/NLL-diagnostics) are the best place to start.

### Transition period

As I mentioned earlier, the MIR-based borrow checker [fixes a lot of\
bugs](https://github.com/rust-lang/rust/labels/NLL-fixed-by-NLL) – this is largely a side effect of making the check operate
over the [MIR](https://blog.rust-lang.org/2016/04/19/MIR.html). This is great! However, as a result, we can’t just
...
_used_ to compile with the old borrow checker but doesn’t with the new
checker (because we fixed a bug in the borrow check). The way we do
this is to run _both_ the old and the new borrow checker. If the new
checker would report an error, we first check if the old check would
_also_ report an error. If so, we can issue the error as normal. If
...
big strides here. (For example, I’ve got a [PR up right now](https://github.com/rust-lang/rust/pull/51460)
that [adds some simple memoization for a 20% win](http://perf.rust-lang.org/compare.html?start=61d88318aa66669fba061e9af529365172d63cd0&end=757cd050fc1ef84d7235d6f4d9228189eed878cc&stat=instructions%3Au), and I’m
...
_at all_, which I think means we should be able to make it run quite a
...
checker. As an example[1](#fn:1), consider this error that the old borrow
checker gives:
...
value needs to live until here” it doesn’t tell you _why_ the borrowed
...
In my [previous blog post](https://smallcultfollowing.com/babysteps/
/blog/2018/04/27/an-alias-based-formulation-of-the-borrow-checker/), I described a new version of the
borrow check, which we have since dubbed [Polonius](https://github.com/rust-lang-nursery/polonius/). That analysis
...
returned – but only _sometimes_. In the MIR borrowck on nightly, this
...
RFC](https://rust-lang.github.io/rfcs/2094-nll.html), this was called [Problem Case #3](https://rust-lang.github.io/rfcs/2094-nll.html#problem-case-3-conditional-control-flow-across-functions). However, we had to
...
Polonius](https://smallcultfollowing.com/babysteps/
/blog/2018/04/27/an-alias-based-formulation-of-the-borrow-checker/) I wrote:
...
1. Hat tip to steveklabnik for providing this example! [↩︎](#fnref:1)
