...
url: https://smallcultfollowing.com/babysteps/blog/2014/04/24/parallel-pipelines-for-js/?utm_source=atom_feed
published: "2014-04-24T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2014/04/24/parallel-pipelines-for-js/
---
...
_parallel pipelines_ (that’s just a working title; I expect the name
...
prototyped the API on a [branch of SpiderMonkey](https://github.com/nikomatsakis/gecko-dev/tree/Pipeline-b), though the
...
a _parallel pipeline_ that, when executed, will iterate over the
...
We call the full iteration space a _shape_. Shapes are a list, where
...
object arrays takes an optional _depth_ argument specifying how many
...
2. `pipeline.mapTo(type, [func])` – invokes `func` on each element,
...
2. `pipeline.reduce(func, [initial])` – executes the pipeline and reduces
...
`map` always pass a _single value_ to their callback. I imagine we
