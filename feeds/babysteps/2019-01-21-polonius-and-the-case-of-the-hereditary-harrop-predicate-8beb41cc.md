...
url: https://smallcultfollowing.com/babysteps/blog/2019/01/21/hereditary-harrop-region-constraints/?utm_source=atom_feed
published: "2019-01-21T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2019/01/21/hereditary-harrop-region-constraints/
---

# Polonius and the case of the hereditary harrop predicate

In my [previous post](https://smallcultfollowing.com/babysteps/
/blog/2019/01/17/polonius-and-region-errors/) about Polonius and subregion obligations, I
...
_problem_ is in the first place and sketches out the general solution
...
subtype relation is saying that `f` can be given the type `fn(&'b u32)` – that is, a fn that can be called with a `'b` reference. That
...
_contravariant_ in their argument type).
...
with _any_ lifetime `'b`. In fact, this subtyping relation should
...
_subtype_ side are instantiated with “there exists”, not “for
...
effective _reduce_ this “rich” constraint to the subset relation `'a: 'c`.

To do this reduction, we draw a distinction between the _bound_ and
the _free_ regions. _Bound_ regions are those that are bound within a
`forall` and `exists` quantifiers (e.g., `'b`), and _free_ regions
those that are not. When we are reducing, we only care about two things:

- **Do we have something _unsatisfiable_ about the constraint?** This
...
familiar – if you recall [my very first Chalk post](https://smallcultfollowing.com/babysteps/
/blog/2017/01/26/lowering-rust-traits-to-logic/#type-checking-generic-functions-beyond-horn-clauses), I talked
about the [need to go beyond Prolog’s core “Horn clauses” and to\
support “Hereditary Harrop” (HH) predicates](https://smallcultfollowing.com/babysteps/
/blog/2017/01/26/lowering-rust-traits-to-logic/#type-checking-generic-functions-beyond-horn-clauses). The basic idea was
...
really give any sort of _algorithm_. I’ve done some experiments here,
...
One really _nice_ aspect of this general direction, though, is that it
...
blog post series](https://internals.rust-lang.org/t/blog-post-an-alias-based-formulation-of-the-borrow-checker/7411).
