...
url: https://smallcultfollowing.com/babysteps/blog/2012/04/23/vectors-strings-and-slices/?utm_source=atom_feed
published: "2012-04-23T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2012/04/23/vectors-strings-and-slices/
---
...
by-value string that is _always_ 10 characters long. This is not like
a fixed-length buffer that can hold strings _up to_ 10 characters
long. Rather, as I understand it, it can _only_ store strings of
...
some kind of special treatment around assignments so that `let x = [1, 2, 3]` ends up with a slice, I guess. Have to think a bit about
that.
...
have the default be that types _do_ have a known type and you have to
say when the type variable might _not_. This is also ok although
generally type bounds _enable_ operators, not _disable_ them.
...
be a prefix anymore. I’d still rather it come near the _front_ of the
type, and not tacked on the end. So far, my preferred notation for
_this_ is something like the following:
...
need an expression form. To be honest, I don’t care about this _that_
...
mutability. Something like `[mut T]` cannot be written as `vec<mut T>`, at least not today. Sigh. Well, I’ll post this blog post
