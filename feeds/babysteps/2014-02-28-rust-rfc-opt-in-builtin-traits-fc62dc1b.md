...
url: https://smallcultfollowing.com/babysteps/blog/2014/02/28/rust-rfc-opt-in-builtin-traits/?utm_source=atom_feed
published: "2014-02-28T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2014/02/28/rust-rfc-opt-in-builtin-traits/
---
...
structs and enums would _move by default_ unless their type is
...
user makes an _explicit choice_ between these three options.
...
obvious why a `T` and `~T` value, which are _semantically equivalent_,
...
cases, types must _opt out_ of the builtin traits.

In general, the _opt out_ approach seems to be hard to reason about:
many people (including myself) find it easier to think about what
properties a type _has_ than what properties it _does not_ have,
...
unsafe methods are generally _wrong by default_. As an example,
...
However, the _methods_ attached to `Cell` use unsafe magic to mutate
...
To accommodate this, we currently use _marker types_ – special types
...
to _opt in_ to the `Share` kind, and types like `Cell<T>` would simply
do nothing.
...
will _have_ to be opt out anyway, so perhaps it is best to be
