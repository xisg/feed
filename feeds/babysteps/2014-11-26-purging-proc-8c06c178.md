---
title: Purging proc
url: https://smallcultfollowing.com/babysteps/blog/2014/11/26/purging-proc/?utm_source=atom_feed
published: "2014-11-26T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2014/11/26/purging-proc/
---
...
[pull request that removes `proc` from the language](https://github.com/rust-lang/rust/pull/19338). I started
...
closure design (sometimes called _unboxed closures_). This part of the
...
though, you are _required_ to use the parentheses form, because we
...
will get a distinct copy of the fn being called _for every_
_callsite_. This is a great boon to inlining (at least sometimes), but
...
surrouding _context_ – basically, the closure was an argument to a
...
_Caveat:_ It is still possible we’ll change the `&:`/ `&mut:`/ `:`
...
environment is _borrowed_ (that is, the struct stores a reference to
...
`proc() { ... }` to create a “procedure” should now use `move|| { ... }`, to create a “moving closure”. The idea of a _moving closure_
...
`Thunk` and `Invoke` are essentially _implementation details_ that end
