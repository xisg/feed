...
url: https://smallcultfollowing.com/babysteps/blog/2018/10/31/mir-based-borrowck-is-almost-here/?utm_source=atom_feed
published: "2018-10-31T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2018/10/31/mir-based-borrowck-is-almost-here/
---

# MIR-based borrowck is almost here

Now that [the final Rust 2018 Release Candidate has\
shipped](https://blog.rust-lang.org/2018/10/30/help-test-rust-2018.html), I thought it would be a good idea to do another
update on the state of the MIR-based borrow check (aka NLL). The [last\
update](https://smallcultfollowing.com/babysteps/
/blog/2018/06/15/mir-based-borrow-check-nll-status-update/) was in June, when we were still hard at work on getting
things to work.
...
[the salsa crate](https://github.com/salsa-rs/salsa) [1](#fn:1), which uses Rust 2018, and I’ve really
...
and #2](https://smallcultfollowing.com/babysteps/
/blog/2016/04/27/non-lexical-lifetimes-introduction/) I highlighted way back when[2](#fn:2), but they interact in
...
chain. Using NLL, I can write this code like so ( [playground](https://play.rust-lang.org/?version=nightly&mode=debug&edition=2018&gist=b8dfafd14113f2933c1b5127c861df44)):
...
action to please the borrow checker[3](#fn:3). But the borrowing
...
error messages’. Unfortunately, you won’t see _all_ of those
...
In the **medium term**, the plan is to push more on the [Polonius](https://github.com/rust-lang-nursery/polonius/)
formulation of NLL that [I described here](https://smallcultfollowing.com/babysteps/
/blog/2018/04/27/an-alias-based-formulation-of-the-borrow-checker/). In addition
to offering a crisp formalization of our analysis, Polonius promises
to fix the [Problem Case #3](https://smallcultfollowing.com/babysteps/
/blog/2016/04/27/non-lexical-lifetimes-introduction/#problem-case-3-conditional-control-flow-across-functions) that I identified in the
...
1. Did you see how smoothly I worked in that plug for [salsa](https://github.com/salsa-rs/salsa)? I’ll write a post about it soon, I promise. [↩︎](#fnref:1)

2. Note that the current NLL implementation does not solve Problem Case #3. See \[the “What Next?” section\]\[wn\] for more.
\[wn\]: #what-next [↩︎](#fnref:2)

3. Interestingly, I remember an example almost exactly like this being shown to me by a Servo intern – I forget which one – many years ago. At the time, it didn’t seem like a big deal to do the workarounds, but I realize now I was wrong about that. Ah well. [↩︎](#fnref:3)
