...
url: https://smallcultfollowing.com/babysteps/blog/2012/07/17/borrowed-pointer-tutorial/?utm_source=atom_feed
published: "2012-07-17T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2012/07/17/borrowed-pointer-tutorial/
---
...
We call borrowed pointers _borrowed_ because, unlike shared and unique
...
current function. In this context, _owning_ memory basically means
...
to _own_ the canvas, just use it, so `draw_context` contains a borrowed
...
In the compiler, each borrowed pointer is associated with a _lifetime_
(you may also have heard the term _region_). A lifetime is a block or
...
To handle this case, functions can be _parameterized_ by lifetimes.
...
can _change the types_ of their contents or even cause their contents
...
ways, depending on the _owner_ of the data being borrowed.
...
_data ownership_. In Rust, owning data essentially means that you are
...
- **Value types** and **unique boxes** both imply exclusive ownership.
  ...
  out of scope, its contents will be _dropped_ (I’ll define drop more
...
_before_ the assignment marked with `(*)`:
...
the stack frame. This does not _necessarily_ mean that the data is
...
actions. _Pure_ actions are actions which do not modify any data
...
legal to create an _immutable pointer_ to _mutable data_ so long as
...
However, if there is an attempt to borrow _unstable_ memory then the
...
refers to `x`. This is called _rooting_ `x`, because it ensures that
...
for the lifetime of the borrow is _pure_. Pure code is code which does
