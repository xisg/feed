...
url: https://smallcultfollowing.com/babysteps/blog/2012/10/01/moves-based-on-type/?utm_source=atom_feed
published: "2012-10-01T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2012/10/01/moves-based-on-type/
---
...
The idea is to repurpose the idea of an _implicitly copyable_ (IC)
type. Expressions whose type is not IC would be _moved by default_.
...
`a.b.c` were a non-IC type, but you wanted to _force_ a copy, you
...
you write a closure that references a non-IC type, it always _moves_
...
if copy/move were defined _syntactically_ rather than being based on
...
clearer what _any particular expression_ does. But it turns out that,
to a first approximation, you _never_ want to copy non-IC types. In
...
In the proposed system, in contrast, you _know_ that nothing will be
...
that we _can_ use: any data owned by the current stack frame and can
be moved. This is the same set of data that a pure fn can modify and
so forth.

### Historical precedent?

I _think_ that most every language that includes affine or linear
...
We already have the notion of an _implicitly copyable type_: this is a
...
with _identity_. Therefore, copying those is generally an error.
