...
url: https://smallcultfollowing.com/babysteps/blog/2013/11/20/parameter-coercion-in-rust/?utm_source=atom_feed
published: "2013-11-20T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2013/11/20/parameter-coercion-in-rust/
---

# Parameter coercion in Rust

Alex Chrichton recently sent a
[message to the rust-dev mailing list](https://mail.mozilla.org/pipermail/rust-dev/2013-November/006849.html) discussing the fate of
...
terminology and precisely what the rules _are_. I refer to “autoref”
...
- For parameter passing, local variable initializers with a declared
  type, and struct field initializers, we apply _coercion_. This is a
  more limited set of transformations. We could and probably should
  apply coercion in a _somewhat_ wider set of contexts, notably return
  expressions.


Currently we are specifically referring to what the _coercion_ rules
...
In addition, I believe that we _should_ have the rule that we will
...
_reborrowing_, though it’s really a special case of autoborrow. The
idea here is that when we see a parameter of type `&'a T` or `&'a mut T` we always “reborrow” it, effectively converting to `&'b T` or `&'b mut T`. While both are borrowed pointers, the reborrowed version has
...
This is needed because `&mut` pointers are _affine_, meaning that
...
rules. The rules were designed around the idea of _pointers_ – so
...
mutability was not inherited, so you would have had to declare `let x = ~mut [1, 2, 3]`, in which case the fact that `sort` mutates `x`
...
_actually_ possible to reason about a function independently of its
...
The issue discusses two possible changes. We could either _limit_
coercion by removing autoborrowing (but keeping reborrowing), or we
could _expand_ coercion by including autoref.
...
Independently, we could _tweak_ coercion by removing the ability to
...
other way to achieve that. (Unless we added a [slice operator](http://smallcultfollowing.com/babysteps/blog/2013/11/14/treating-vectors-like-any-other-container/), as
discussed in another blog post.) _But_ I am very sympathetic to the
