...
url: https://smallcultfollowing.com/babysteps/blog/2016/02/25/parallel-iterators-part-2-producers/?utm_source=atom_feed
published: "2016-02-25T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2016/02/25/parallel-iterators-part-2-producers/
---
...
all you want to do is _use_ parallel iterators, you don’t really have
...
This post is part 2 of a series. In the [initial post](http://smallcultfollowing.com/babysteps/blog/2016/02/19/parallel-iterators-part-1-foundations/) I covered
...
[previous post](http://smallcultfollowing.com/babysteps/blog/2016/02/19/parallel-iterators-part-1-foundations/), I sort of did it bottom-up: I started
...
convert the iterator into a producer. A _producer_ is kind of like a
...
[`bridge_producer_consumer`](https://github.com/nikomatsakis/rayon/blob/bed0da76215aef1a0d852339fd79cedba9ec4c40/src/par_iter/internal.rs#L100-L124); it uses the same basic divide-and-conquer
...
freed. We can model this easily by having those resources be _owned_
by the parallel iterator but _borrowed_ by the producers, since the
...
iterator type `SliceIter` that we saw in the [initial post](http://smallcultfollowing.com/babysteps/blog/2016/02/19/parallel-iterators-part-1-foundations/) (in
...
- the field `map_op` contains a reference `&MAP_OP`:
  ...
  - The sequential map iterator owned the closure `MAP_OP`, but the
    ...
    those two copies can’t _both_ own the `map_op`, they need to share
    it.
...
2. The closure `map_op` is _owned_ by the parallel iterator.
...
where-clauses so that instead of requiring that `ITER: ParallelIterator`, we require that `ITER: BoundedParallelIterator` and
so forth:
...
best way to do that is with a _callback_. The idea is that instead of
...
make the `callback` method _generic_ over the kind of producer that
...
the [actual code](https://github.com/nikomatsakis/rayon/blob/312fc8ccd7a28289138d2b0d3ce16dfec6269b04/src/par_iter/map.rs#L59-L90) from Rayon):
