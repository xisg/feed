...
url: https://smallcultfollowing.com/babysteps/blog/2013/11/14/treating-vectors-like-any-other-container/?utm_source=atom_feed
published: "2013-11-14T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2013/11/14/treating-vectors-like-any-other-container/
---

# Treating vectors like any other container

Some [astute](https://mail.mozilla.org/pipermail/rust-dev/2013-November/006381.html) [comments](https://mail.mozilla.org/pipermail/rust-dev/2013-November/006376.html) on a recent thread to rust-dev got me
...
experiment, not exactly a _proposal_.
...
sidesteps the need for [DST](https://smallcultfollowing.com/babysteps/
/blog/2013/04/30/dynamically-sized-types/) when it comes to vectors. Instead,
...
In the [DST](https://smallcultfollowing.com/babysteps/
/blog/2013/04/30/dynamically-sized-types/) proposal, slicing and borrowing are the same
...
(Note that this _also_ implies, I believe, that if we permit
...
`R`. At the time, we would search for an implementation of `Dice<U> for R` (where `U` is a fresh variable). If we find one, we can then
...
can _really_ generate on the stack or at compile-time, so they are
...
Clearly a problem. Note that we will [want to fix this](https://github.com/mozilla/rust/issues/6515) regardless of
...
reason to separate the two is so that hashtables can use `map[key] = index` as the notation for inserting a key even if the key is not
...
illegal anyhow as part of [issue 5016](https://github.com/mozilla/rust/issues/5016). The reason is that they
...
_cleaner_ or _simpler_. This approach makes vectors much more like all
...
expand to the same code as above, and perhaps `map!(k1 -> v1, k2 -> v2)` could cover the map case. Macros would be much simpler on the
