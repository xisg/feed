...
url: https://smallcultfollowing.com/babysteps/blog/2023/03/03/trait-transformers-send-bounds-part-3/?utm_source=atom_feed
published: "2023-03-03T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2023/03/03/trait-transformers-send-bounds-part-3/
---

# Trait transformers (send bounds, part 3)

I previously introduced [the “send bound” problem](https://smallcultfollowing.com/babysteps/
/blog/2023/02/01/async-trait-send-bounds-part-1-intro/), which refers to the need to add a `Send` bound to the future returned by an async function. This post continues my tour over the various solutions that are available. This post covers “Trait Transformers”. This proposal arose from a joint conversation with myself, Eric Holk, Yoshua Wuyts, Oli Scherer, and Tyler Mandry. It’s a variant of Eric Holk’s [inferred async send bounds](https://blog.theincredibleholk.org/blog/2023/02/13/inferred-async-send-bounds/) proposal as well as the work that Yosh/Oli have been doing in the [keyword generics](https://blog.rust-lang.org/inside-rust/2023/02/23/keyword-generics-progress-report-feb-2023.html) group. Those posts are worth reading as well, lots of good ideas there.[1](#fn:1)

## Core idea: the trait transformer

A _transformer_ is a way for a single trait definition to define multiple variants of that trait. For example, where `T: Iterator` means that `T` implements the `Iterator` trait we know and love, `T: async Iterator` means that `T` implements the _async version_ of `Iterator`. Similarly, `T: Send Iterator` means that `T` implements the _sendable version_ of `Iterator` (we’ll define both the “sendable version” and “async version” more precisely, don’t worry).
...
You can think of a trait transformer as being like a fancy kind of macro. When you write a maybe-async trait like `Iterator` above, you are effectively defining a _template_ from which the compiler can derive a family of traits. You could think of the `#[maybe(async)]` annotation as a macro that derives two related traits, so that…
...
…when you have a where-clause like `T: async Iterator`, then, the compiler would be transforming that to `T: AsyncIterator`. In fact, Oli and Yosh implemented a procedural macro crate that does more-or-less _exactly_ this.
...
- the value returned by any RPITIT method must be `Send` [2](#fn:2)
...
using the [return type notation](https://smallcultfollowing.com/babysteps/blog/2023/02/13/return-type-notation-send-bounds-part-2/).
...
### How is this different from eholk’s [Inferred Async Send Bounds](https://blog.theincredibleholk.org/blog/2023/02/13/inferred-async-send-bounds/)?
...
### How is this different from the [keyword generics](https://blog.rust-lang.org/inside-rust/2023/02/23/keyword-generics-progress-report-feb-2023.html) post?

Yosh and Oli posted a [keyword generics update](https://blog.rust-lang.org/inside-rust/2023/02/23/keyword-generics-progress-report-feb-2023.html) that included notation for “maybe async” traits (they wrote `?async`) along with some other things. The ideas in this post are very similar to those, the main difference is treating `Send` as an independent transformer, similar to the previous question.
...
Personally, I rather prefer the idea that inside a `#[maybe(async)]` block, you define the trait as it were _always_ async…
...
### Do we still want [return type notation](https://smallcultfollowing.com/babysteps/blog/2023/02/13/return-type-notation-send-bounds-part-2/)?

Yes, [RTN](https://smallcultfollowing.com/babysteps/blog/2023/02/13/return-type-notation-send-bounds-part-2/) is useful for giving more precise specification of which methods should return send-futures (you may not want to require that _all_ async methods are send, for example). It’s also needed internally by the compiler anyway as the “desugaring target” for the `Send` transformer.

### Can we allow `\#[maybe]` on types/functions?

Maybe![3](#fn:3) That’s basically full-on keyword generics. This proposal is meant as a stepping stone. It doesn’t permit code or types to be generic whether they are async/send/whatever, but it does permit us to define multiple versions of trait. To the language, it’s effectively a kind of _macro_, so that (i.e.) a single trait definition `#[maybe(async)] trait Iterator` effectively defines two traits, `Iterator` and `AsyncIterator`, and the `T: async Iterator` notation is being used to select the second one. (This is only an example, I don’t mean that users would literally be able to reference a `AsyncIterator` trait.)
...
Yosh at some point suggested that we could think of “send” or “not send” as another application of [keyword generics](https://blog.rust-lang.org/inside-rust/2023/02/23/keyword-generics-progress-report-feb-2023.html), and that got me very excited. It’s a known problem that people have to define two versions of their structs (see e.g. the [im](https://crates.io/crates/im) and [im-rc](https://crates.io/crates/im-rc) crates). Maybe we could permit something like
...
and then permit variables of type `Shared<u32>` or `Send Shared<u32>`. The [keywosrd generics](https://blog.rust-lang.org/inside-rust/2023/02/23/keyword-generics-progress-report-feb-2023.html) proposals already are exploring the idea of structs whose types vary depending on whether they are async or not, so this fits in.

## Conclusion

This post covered “trait transformers” as a possible solution the [“send bounds”](https://smallcultfollowing.com/babysteps/
/blog/2023/02/01/async-trait-send-bounds-part-1-intro/) problem. Trait transformers are not exactly an _alternative_ to the [return type notation](https://smallcultfollowing.com/babysteps/blog/2023/02/13/return-type-notation-send-bounds-part-2/) proposed earlier; they are more like a complement, in that they make the “easy easy”, but effectively provide a convenient desugaring to uses of [return type notation](https://smallcultfollowing.com/babysteps/blog/2023/02/13/return-type-notation-send-bounds-part-2/).

The full set of solutions thus far are…

- [Return type notation (RTN)](https://smallcultfollowing.com/babysteps/blog/2023/02/13/return-type-notation-send-bounds-part-2/)
  - _Example:_ `T: Fetch<fetch(): Send>`
  - _Pros:_ flexible and expressive
  - _Cons:_ verbose
- eholk’s [inferred async send bounds](https://blog.theincredibleholk.org/blog/2023/02/13/inferred-async-send-bounds/)
  - _Example:_ `T: async(Send) Fetch`
  - _Pros:_ concise
  - _Cons:_ specific to async notation, doesn’t support `-> impl Future` functions; requires RTN for completeness
- trait transformers (this post)
  - _Example:_ `T: async Send Fetch`
  - _Pros:_ concise
  - _Cons:_ requires RTN for completeness

* * *

1. I originally planned to have part 3 of this series simply summarize those posts, in fact, but I consider Trait Transformers an evolution of those ideas, and close enough that I’m not sure separate posts are needed. [↩︎](#fnref:1)

2. It’s unclear if `Send Foo` should always convert [RPITIT](https://rust-lang.github.io/impl-trait-initiative/RFCs/rpit-in-traits.html) return values to be `Send`, but it _is_ clear that we want some way to permit one to write `-> impl Future` in a trait and have that be `Send` iff async methods are `Send`. [↩︎](#fnref:2)

3. See what I did there? [↩︎](#fnref:3)
