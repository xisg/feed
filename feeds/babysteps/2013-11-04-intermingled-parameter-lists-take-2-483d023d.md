...
url: https://smallcultfollowing.com/babysteps/blog/2013/11/04/intermingled-parameter-lists/?utm_source=atom_feed
published: "2013-11-04T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2013/11/04/intermingled-parameter-lists/
---

# Intermingled parameter lists, take 2

I got a lot of feedback on my post about
[intermingled parameter lists](https://smallcultfollowing.com/babysteps/
/blog/2013/10/29/intermingled-parameter-lists/) – most of it negative – and I’ve
...
_Aside #1:_ Some [good](https://smallcultfollowing.com/babysteps/
/blog/2013/10/29/intermingled-parameter-lists/#comment-1103218392) [arguments](https://smallcultfollowing.com/babysteps/
/blog/2013/10/29/intermingled-parameter-lists/#comment-1103344213) were advanced in favor of
making _all_ lifetime parameters late bound and relying instead on
[higher-kinded types (HKT)](http://en.wikipedia.org/wiki/Kind_%28type_theory%29) or some kind of explicit universal
...
_Aside #2:_ I’d like to find a better name for early and late
...
distinction between _type schemes_ and _types_ in ML or Haskell, so
...
Let’s begin with the `get_index` early from my [earlier post](https://smallcultfollowing.com/babysteps/
/blog/2013/10/29/intermingled-parameter-lists/):
...
Thanks to `_`, it is also possible to specify values for _just some_
...
_An aside:_ the subtyping rules for universally quantified functions
...
[wonderfully readable, but rather long, paper](http://research.microsoft.com/en-us/um/people/simonpj/papers/higher-rank/) on this topic if
...
the type bound for `I`, which means that it is _early bound_. In case
...
_Review:_ The reason for this is that, because we don’t support types
...
commentors argued that [HKT](https://smallcultfollowing.com/babysteps/
/blog/2013/10/29/intermingled-parameter-lists/#comment-1103218392) or [skolemization](https://smallcultfollowing.com/babysteps/
/blog/2013/10/29/intermingled-parameter-lists/#comment-1103344213) provides an
...
`Iterator` protocol _must_ be implemented in a way that is completely
...
respect to all lifetimes. Rather, it is defined with respect to _one_
_specific lifetime_. For this reason, `Alphabet` would not
...
the design space for [multi-parameter type classes](http://research.microsoft.com/en-us/um/people/simonpj/papers/type-class-design-space/). In short,
...
- Parameter lists consists of some number of lifetimes and some
  number of types (i.e., the two are _not_ intermingled, for now anyway).
- Lifetime parameters on fns or methods that appear within a type
  bound are considered _early bound_. Those that do not are considered
  _late bound_.
- Whenever a type- or lifetime-parameterized path is referenced, users
  have the option of either:
  - supplying values for _all_ parameters, both lifetime and type; or,
  - supplying values for _only_ the type parameters, in which case
