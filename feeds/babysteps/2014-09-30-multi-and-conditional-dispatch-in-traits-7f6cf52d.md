...
url: https://smallcultfollowing.com/babysteps/blog/2014/09/30/multi-and-conditional-dispatch-in-traits/?utm_source=atom_feed
published: "2014-09-30T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2014/09/30/multi-and-conditional-dispatch-in-traits/
---

# Multi- and conditional dispatch in traits

I’ve been working on a branch that implements both _multidispatch_
(selecting the impl for a trait based on more than one input type) and
_conditional dispatch_ (selecting the impl for a trait based on where
clauses). I wound up taking a direction that is slightly different
from what is described in the [trait reform RFC](https://github.com/rust-lang/rfcs/blob/master/active/0024-traits.md), and I
...
_coherence_. The idea is (at least as implemented in the `master`
...
_other_ type parameters are different. So this second impl is ok,
...
_blanket impls_. So how do blanket impls interact with the coherence
...
In the trait reform RFC, I mentioned a desire to support _crate_
_concatenability_, which basically means that you could take two crates
...
_except_ when it comes to inference. That’s where things get
interesting.
...
converted to _either_ `uint` or `MyInt`. This means that the snippet
...
[associated types RFC](https://github.com/rust-lang/rfcs/blob/master/active/0059-associated-items.md), there is even a syntactic way to
express this.

However, when you start trying to implement _conditional dispatch_
...
we’re strict about crate concatenability, the answer is _no_.
...
vector, but we don’t know what it’s a vector _of_ – to infer the
...
- We could introduce some new syntax for distinguishing _conditional_
  _dispatch_ vs other where clauses (basically the input/output
...
_different_ traits, for example. Consider the following example:
...
process, but it seems very _usable_. Basically it is guided only by
