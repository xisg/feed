...
url: https://smallcultfollowing.com/babysteps/blog/2016/11/03/associated-type-constructors-part-2-family-traits/?utm_source=atom_feed
published: "2016-11-03T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2016/11/03/associated-type-constructors-part-2-family-traits/
---
...
any collection of integers into _any_ collection of floats, but those
...
“collection of `f32` values”, but we don’t know what _kind_ of
...
be different). So the compiler doesn’t want to just _guess_.
...
that we would _want_ is maybe something like this (ignoring the where
clauses for now):
...
_Note:_ While writing this post I realized that Haskell also has a
...
_family_ of collection types. Then we can can use `F::Collection<i32>` to
...
problem! Knowing the _value_ of an associated type ( `?F::Collection<i32>`)
...
deduce that `?C = List<i32>`. Next, solving the constraint `?C: Collection<i32, Family=?F>` will allow us to deduce that `?F = ListFamily`. And hence we are all set.

### Side-note: extending higher-ranked trait bounds

There’s one part of [RFC 1598](https://github.com/rust-lang/rfcs/pull/1598) that I haven’t covered so far. I just
...
Rust’s _higher-ranked trait bounds_ to support generalization over
...
_any_ lifetime. For example, `for<'a> T: Foo<'a>` means “for any
lifetime `'a`, `T` implements `Foo<'a>`”; you could also write `T: for<'a> Foo<'a>`, which is equivalent. This `'a` lifetime can also
appear as part of the type, so one might write `for<'a> &'a T: Foo<'a>` (in this case, you can’t move the `for<'a>` around, since it
...
(This is a simplification. Really, `T` must meet _some_ requirements
...
sorts of “family” traits, and how it would _really_ feel to use them
...
constructors ( _or_ HKT) really gives us that! The problem is that
borrow expressions operate on _paths_, and we have no way to reify
