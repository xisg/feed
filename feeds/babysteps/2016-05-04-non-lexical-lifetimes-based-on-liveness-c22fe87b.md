...
url: https://smallcultfollowing.com/babysteps/blog/2016/05/04/non-lexical-lifetimes-based-on-liveness/?utm_source=atom_feed
published: "2016-05-04T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2016/05/04/non-lexical-lifetimes-based-on-liveness/
---

# Non-lexical lifetimes based on liveness

In my [previous post](http://smallcultfollowing.com/babysteps/blog/2016/04/27/non-lexical-lifetimes-introduction/) I outlined several cases that we would like
...
1. Define a **lifetime** as a **set of points in the control-flow**
   **graph**, where a **point** here refers to some particular statement
   in the control-flow graph (i.e., not a [basic block](https://en.wikipedia.org/wiki/Control_flow_graph), but some
...
them – let’s look at the initial example from [my previous post](http://smallcultfollowing.com/babysteps/blog/2016/04/27/non-lexical-lifetimes-introduction/).
...
What we would like is to observe that `slice` is _dead_ – which is
[compiler-speak](https://en.wikipedia.org/wiki/Live_variable_analysis) for “it won’t ever be used again” – after the call to
...
Here again the reference `slice` is _still_ only be required to live
...
the reference `slice` is in fact still [live](https://en.wikipedia.org/wiki/Live_variable_analysis) after the call to
...
the approach that [RFC 396](https://github.com/rust-lang/rfcs/pull/396) took, for example. I initially explored
...
[RFC 396](https://github.com/rust-lang/rfcs/pull/396) had special rules around backedges. Edited to clarify.)
...
as an [inference rule](https://en.wikipedia.org/wiki/Rule_of_inference)):
...
Here I have substituted _live-range_ for _scope_. By [live-range](https://en.wikipedia.org/wiki/Live_variable_analysis)
...
in the first example, the variable `slice` is _dead_ after the call to
capitalize: it will never be used again. But in the second variation,
the one with a loop, `slice` is _live_, because it may be used in the
...
example, while this bit of code _will_ type-check:
...
field access. _However,_ the [dynamic drop](https://github.com/rust-lang/rfcs/blob/master/text/0320-nonzeroing-dynamic-drop.md) analysis is
already computing a set of _fragments_, which are basically minimal
...
compatible with the fragment logic used for [dynamic drop](https://github.com/rust-lang/rfcs/blob/master/text/0320-nonzeroing-dynamic-drop.md), so it
...
[RFC 1238](https://github.com/rust-lang/rfcs/blob/master/text/1238-nonparametric-dropck.md) but are colloquially called
[“dropck”](https://github.com/rust-lang/rfcs/blob/master/text/1238-nonparametric-dropck.md). They basically state that when we create some
variable `x` whose type `T` has a destructor, then `T` must outlive
the _parent_ scope of `x`. That is, the references in `x` don’t have
to just be valid for the scope of `x`, they have to be valid for
_longer_ than the scope of `x`.
...
case. The reason is that, in [MIR](http://blog.rust-lang.org/2016/04/19/MIR.html), all drops are made explicit in
the [control-flow graph](https://en.wikipedia.org/wiki/Control_flow_graph), and hence if a variable `x` has a
...
[my introduction](http://smallcultfollowing.com/babysteps/blog/2016/04/27/non-lexical-lifetimes-introduction/). I also covered some of the implementation
...
[this internals thread](http://internals.rust-lang.org/t/non-lexical-lifetimes-based-on-liveness/3428) instead.)
