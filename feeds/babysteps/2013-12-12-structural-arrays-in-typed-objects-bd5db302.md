...
url: https://smallcultfollowing.com/babysteps/blog/2013/12/12/structural-arrays-in-typed-objects/?utm_source=atom_feed
published: "2013-12-12T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2013/12/12/structural-arrays-in-typed-objects/
---

# Structural arrays in Typed Objects

[Dave Herman](https://twitter.com/littlecalculist) and I were tossing around ideas the other day for a
...
we invoke the method `dim`, which produces a _dimensioned type_:
...
Here `S` is what I call a _single_ type. It can either be a scalar
...
we defined a `U` to represent the set of type objects, it would be `S | [S]`. But when you instantiate an array `[S]` you give it a concrete
...
that struct types are _nominal_, meaning that the type is tied to a
...
did an [extensive redesign](https://smallcultfollowing.com/babysteps/
/blog/2013/09/03/type-specifications-in-parallel-js/) to accommodate nominal array
...
[transforms on images taken from the camera](https://github.com/IntelLabs/ParallelJavaScript). Those APIs define
...
on. For example, [the filter for detecting faces](https://github.com/IntelLabs/ParallelJavaScript/blob/a06e1a9daeadad345b0321313edee556e5f6a774/tutorial/src/complete/filters.js#L210) looks like
this:
...
window. This means that we are defining _new array types for every_
_frame_.
