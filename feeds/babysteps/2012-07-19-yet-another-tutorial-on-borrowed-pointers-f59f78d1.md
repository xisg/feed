...
url: https://smallcultfollowing.com/babysteps/blog/2012/07/19/yet-another-tutorial-on-borrowed-pointers/?utm_source=atom_feed
published: "2012-07-19T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2012/07/19/yet-another-tutorial-on-borrowed-pointers/
---
...
Borrowed pointers are called _borrowed_ because they are only valid
...
unrooted and therefore be subject to garbage collection _(an aside: in_
_our current implementation, the garbage collector is implemented using_
_reference counting and cycle detection)_.
...
The previous example demonstrated _rooting_, the process by which the
...
For unique boxes, therefore, the compiler will only allow a borrow _if_
_the compiler can guarantee that the unique box will not be reassigned_
_or moved for the lifetime of the pointer_. This does not necessarily
...
_as soon as their owning reference is changed or goes out of scope_.
...
of memory immediately _before_ the re-assignment of `x`:
...
applied to any memory which is _(uniquely) owned by the stack frame_.
...
fact `x.f`. Moreover, `x.f` is both _mutable_ and _aliasable_.
...
alias of `x`. Therefore, to be safe, the compiler only accepts _pure_
...
one option is to use the _swap_ operator to bring that unique box
onto your stack:
...
(Like any up-to-date engineer, we use the [tau circle constant](http://www.math.utah.edu/~palais/pi.html)
...
pointer of type `&size` into the _interior of the enum_.
...
to store that shape value would still be valid, _it would have a_
_different type!_ This is shown in the following diagram, depicting
...
pointer has type `&shape`, which means “borrowed pointer to _immutable_
_memory_ containing a shape”. If however the type of that pointer were
...
> [part of #2855](https://github.com/mozilla/rust/issues/2855). Instead, right now, _all_ pattern bindings
...
was given. You’ll note that _both_ the parameter and the return value
...
time one which does _not_ compile:
...
during the lifetime of the borrow are _pure_. Pure actions are those
