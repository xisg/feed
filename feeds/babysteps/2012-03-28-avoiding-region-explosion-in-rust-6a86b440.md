...
url: https://smallcultfollowing.com/babysteps/blog/2012/03/28/avoiding-region-explosion-in-rust/?utm_source=atom_feed
published: "2012-03-28T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2012/03/28/avoiding-region-explosion-in-rust/
---
...
_crate context_, or `ccx`, which stores crate-wide data such as
linkage information about top-level functions, constants, and so
forth; the _function context_, or `fcx`, which stores per-function
data such as references to the LLVM variables representing its
parameters and locals; and finally the _block context_, or `bcx`,
...
[diamond-shaped if-then-else pattern](http://en.wikipedia.org/wiki/File:If-then-else-control-flow-graph.svg). The initial block is the
...
for which there exists an `impl` offering an `alloc(sz: uint, align: uint) -> *()` method, which allocates `sz` bytes of memory at the
given alignment and returns a pointer. An expression like `new (pool) value` will cause `pool.alloc()` to be invoked and will then store the
...
_seems_ that this is enough in practice for most real use cases. Time
will tell, I suppose.
