...
url: https://smallcultfollowing.com/babysteps/blog/2018/04/16/rust-pattern-rooting-an-rc-handle/?utm_source=atom_feed
published: "2018-04-16T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2018/04/16/rust-pattern-rooting-an-rc-handle/
---
...
I invoke `process_datum`, I am giving it both `&mut self` _and_ a
...
it won’t cause your callers to stop compiling[1](#fn:1). It also ensures
...
total time scales linearly with the number of functions[2](#fn:2)).
...
within, which would invalidate `datum`. Similarly, writing `self.data = ...` could cause the vector to be freed as well (as we saw earlier).
...
`self.data` into a local variable and borrowed _that_:
...
reported. Note that the compiler _still_ assumes the worst about
...
is to use a [persistent collection type](https://smallcultfollowing.com/babysteps/
/blog/2018/02/01/in-rust-ordinary-vectors-are-values/) – such as those provided by
the [im](https://crates.io/crates/im) crate.)

If the data _is_ mutable, there are various other patterns that you
...
time, which would basically let you declare that _nobody_ will
...
not”[3](#fn:3). I suspect we would not go in this direction.
...
1. Or crash, as would happen without the compiler’s checks. [↩︎](#fnref:1)

2. Total time for the safety check, that is. Optimizations and other things are sometimes inter-procedural. [↩︎](#fnref:2)

3. Interestingly, C++ does this when you have `const` fields. [↩︎](#fnref:3)
