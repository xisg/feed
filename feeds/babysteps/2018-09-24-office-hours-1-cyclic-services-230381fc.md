...
url: https://smallcultfollowing.com/babysteps/blog/2018/09/24/office-hours-1-cyclic-services/?utm_source=atom_feed
published: "2018-09-24T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2018/09/24/office-hours-1-cyclic-services/
---

# Office Hours #1: Cyclic services

This is a report on the second [“office hours”](https://github.com/nikomatsakis/office-hours), in which we
...
You _could_ setup something with [`Arc`](https://doc.rust-lang.org/std/sync/struct.Arc.html) (atomic reference counting)
in Rust that has a similar flavor. For example, the directory might
have an [`Arc`](https://doc.rust-lang.org/std/sync/struct.Arc.html) to each service and the services might have weak refs
back to the directory. But [`Arc`](https://doc.rust-lang.org/std/sync/struct.Arc.html) really works best when the data is
immutable, and we want services to have state. We could solve _that_
with [atomics](https://doc.rust-lang.org/std/sync/atomic/struct.AtomicU32.html) and/or [locks](https://doc.rust-lang.org/std/sync/struct.RwLock.html), but at this point we might want to step
...
What is this [`Sender`](https://doc.rust-lang.org/std/sync/mpsc/struct.Sender.html) type? It is part of the channels that ship in
Rust’s standard library. The idea of a channel is that when you create
it, you get back two “entangled” values: a [`Sender`](https://doc.rust-lang.org/std/sync/mpsc/struct.Sender.html) and a [`Receiver`](https://doc.rust-lang.org/std/sync/mpsc/struct.Receiver.html). You
...
to communicate with it. The actor takes the [`Receiver`](https://doc.rust-lang.org/std/sync/mpsc/struct.Receiver.html) and the
[`Sender`](https://doc.rust-lang.org/std/sync/mpsc/struct.Sender.html) goes into the directory for other servies to use.
...
complete example on the Rust playground](https://play.rust-lang.org/?gist=9fc3d90b50e8af1470a0d488fb3993b9&version=stable&mode=debug&edition=2015).
...
Again, [here is a link to the complete example on the Rust playground](https://play.rust-lang.org/?gist=9fc3d90b50e8af1470a0d488fb3993b9&version=stable&mode=debug&edition=2015).
