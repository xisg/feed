...
url: https://smallcultfollowing.com/babysteps/blog/2018/08/13/never-patterns-exhaustive-matching-and-uninhabited-types-oh-my/?utm_source=atom_feed
published: "2018-08-13T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2018/08/13/never-patterns-exhaustive-matching-and-uninhabited-types-oh-my/
---
...
of its implications, particularly around _exhaustiveness checking_ –
...
Note that the `!` pattern _can_ be used with the `!` type, but it can
...
- If you are matching against an `enum`, you might cover _some_ of the enum variants
...
- When matching an enum, then the “auto-never” would add all missing variants
  ...
  - e.g., if you write `match x { None => .. .}` where `x: Option<T>`, then we will attempt to insert `Some(P)` where the
...
is represented in memory the same as `String` – _and_ the compiler
...
is when you start thinking carefully about _unsafe_ code – and in
...
Ralf’s lovely recent blog post on [Stacked Borrows](https://www.ralfj.de/blog/2018/08/07/stacked-borrows.html), and in
...
so what data does _that_ access?
...
- **Uninitialized memory**, which explains why – when we auto-never a tuple type –
  we require _all_ fields of the tuple to have uninhabited type, instead
  of just one.
...
require that _all_ fields by uninhabited?

The answer is that, using unsafe code, it is possible to _partially_
...
possibly uninitialized (similar to the one introduced in [RFC 1892](https://github.com/rust-lang/rfcs/pull/1892)):
...
Then we initialize _just_ the `x.value.0` part of the tuple:
...
forth, [we hit code doing **exactly** patterns like this](https://internals.rust-lang.org/t/recent-change-to-make-exhaustiveness-and-uninhabited-types-play-nicer-together/4602).
...
understand that the `Err` variant ought to be treated as _impossible_
in such a type.
