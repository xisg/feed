...
url: https://smallcultfollowing.com/babysteps/blog/2016/04/27/non-lexical-lifetimes-introduction/?utm_source=atom_feed
published: "2016-04-27T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2016/04/27/non-lexical-lifetimes-introduction/
---
...
start out with a post that lays out roughly how today’s _lexical_
_lifetimes_ work and gives several examples of problem cases that we
would like to solve.
...
the _lifetime_ of this borrow will be confined to just that call. The
_scope_ of data, in contrast, is much larger, and corresponds to a
...
- A scope generally corresponds to some block (or, more specifically,
  a _suffix_ of a block that stretches from the `let` until the end of
...
This _particular_ example is relatively easy to workaround. One can
...
the caller, but the borrow checker can see that this borrow _will not_
_have even started_ outside of the `if`. So it does not consider the
