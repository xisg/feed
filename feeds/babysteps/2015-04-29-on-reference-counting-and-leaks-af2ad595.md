...
url: https://smallcultfollowing.com/babysteps/blog/2015/04/29/on-reference-counting-and-leaks/?utm_source=atom_feed
published: "2015-04-29T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2015/04/29/on-reference-counting-and-leaks/
---
...
marked unstable, and there is an [RFC](https://github.com/rust-lang/rfcs/pull/1084) proposing a safe
alternative.
...
[recent](https://github.com/rust-lang/rfcs/pull/1085) [proposals](https://github.com/rust-lang/rfcs/pull/1094) have suggested that we should place
...
(see, for example, [this section of the Rust reference manual](http://doc.rust-lang.org/reference.html#behavior-not-considered-unsafe))
and move on.
...
we realized we could [do much better](http://smallcultfollowing.com/babysteps/blog/2013/06/11/data-parallelism-in-rust/) – **the**
**[same type system rules](http://smallcultfollowing.com/babysteps/blog/2013/06/11/on-the-connection-between-memory-management-and-data-race-freedom/) that ensured memory safe in sequential**
**code could be used to permit sharing in parallel code as well**,
particularly once we adopted [RFC 458](https://github.com/rust-lang/rfcs/blob/master/text/0458-send-improvements.md) (a brilliant insight by
...
So where does the fault lie? From the point of view of _history_, it
...
already [an RFC showing how this can be done](https://github.com/rust-lang/rfcs/pull/1084) (I’ll summarize it
...
limits on the RC API to prevent leaks, or at least prevent _some_
...
examine the Rust standard library, it turns out that _all_ of the
...
2. Leaking a [mutex guard](http://doc.rust-lang.org/std/sync/struct.MutexGuard.html) means that the mutex will never be released.
   This is likely to cause deadlock, but not memory unsafety.
3. Leaking a [`RefCell` guard](http://doc.rust-lang.org/std/cell/struct.Ref.html) means that the `RefCell` will remain
...
4. Even fancy iterator APIs like `drain`, which was initially thought
   ...
   [such a way that they cause leaks to occur if they are leaked](http://cglab.ca/~abeinges/blah/everyone-poops/),
...
1. _Ownership:_ Things like `Box` and `Vec` simply own the values they are
...
2. _Pre-poisoning:_ Other guards, like `MutexGuard`, put the value
...
and these other APIs; this difference was [well articulated](https://github.com/rust-lang/rfcs/pull/1084#issuecomment-96875651) by
Kevin Ballard:
...
though is that, frequently, it is not necessary to _guarantee_ that
...
out in [RFC 1084](https://github.com/rust-lang/rfcs/pull/1084). The basic idea is to restructure the API so
...
a parallel [quicksort](http://en.wikipedia.org/wiki/Quicksort):
...
this topic: [RFC 1085](https://github.com/rust-lang/rfcs/pull/1085) and [RFC 1094](https://github.com/rust-lang/rfcs/pull/1094).
...
The first of the two RFCs is [RFC 1085](https://github.com/rust-lang/rfcs/pull/1085). This RFC introduces a
...
mostly invisible. Indeed, in the prototype that [Jonathan Reem](https://github.com/reem/)
...
There is one other point. [RFC 1085](https://github.com/rust-lang/rfcs/pull/1085) also includes an unsafe
...
there, we vastly weaken the safety guarantees of Rust _in_
_practice_. I’d rather that we increase the power of safe APIs at the
...
[RFC 1094](https://github.com/rust-lang/rfcs/pull/1094) takes a different tack. Rather than dividing types
...
The goal of [RFC 1094](https://github.com/rust-lang/rfcs/pull/1094) is to enable a particular “mental model”
...
[RFC 1094](https://github.com/rust-lang/rfcs/pull/1094) requires that to construct a `Rc<T>` (or `Arc<T>`),
...
1. The vast majority of RAII-based APIs are either safe or can be made
   ...
   - In cases like this, where there is often no _need_ to retain the
