...
url: https://smallcultfollowing.com/babysteps/blog/2019/10/26/async-fn-in-traits-are-hard/?utm_source=atom_feed
published: "2019-10-26T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2019/10/26/async-fn-in-traits-are-hard/
---

# why async fn in traits are hard

After reading [boat’s excellent post on asynchronous destructors](https://boats.gitlab.io/blog/post/poll-drop/),
...
meantime, you can use the [`async-trait`](https://crates.io/crates/async-trait) crate (but I get ahead of
myself…).
...
excellent [`async-trait`](https://crates.io/crates/async-trait) crate. This allows you to write
...
documentation explains](https://github.com/dtolnay/async-trait#explanation), this declaration is getting transformed to
...
need for the [`async-trait`](https://crates.io/crates/async-trait) crate to begin with!

## Async fn normally returns an impl Future

We saw that the [`async-trait`](https://crates.io/crates/async-trait) crate converts an `async fn` to something
...
_some_ kind of future, but the precise type will be determined by the
details of the impl (and perhaps inferred by the compiler). So, if
know _logically_ how `impl Trait` in traits should behave, what stops
...
and then we would in turn desugar _that_ into something that uses
an associated type:
...
Hmm, did you notice that I wrote `type GetUser<'s>`, and not `type GetUser`? Yes, that’s right, this is not just an associated type,
it’s actually a [**generic** associated type](https://github.com/rust-lang/rfcs/blob/master/text/1598-generic_associated_types.md). The reason for
...
[chalk](https://github.com/rust-lang/chalk), though that’s not entirely clear. In any case, this work is
...
we’re going to want to write a function that looks _something_ like
this[1](#fn:1):
...
- First, we wrote the name `GetUser`, but that is something we
  introduced as part of “manually” desugaring `async fn get_user`. What name would the user _actually_ use?
...
- Third, our example `Database` trait has only one async fn, but
  ...
  _all_ of them `Send` or `None` – so you can expand a lot more
...
how should we _name_ this associated type? In my example, I introduced
...
In my example, I used the strawman syntax `for<'s> D::GetUser<'s>: Send`. In real life, unfortunately, the bounds you need may well get
...
If we require functions to specify the _exact_ futures that are
...
Or perhaps we’d even want to avoid _any_ syntax, and somehow
...
in return position[2](#fn:2), or indeed any associaed
...
Now imagine that had our `trait Database`, containing an `async fn get_user`. We might like to write functions that operate over `dyn Database`
...
2. And anyway, each `dyn Database` is going to have a _distinct_ `GetUser` type. If we have to specify `GetUser`, then, that kind of
...
`get_user` is that it gets back _some_ future which it can poll. It
...
ergonomic choice – but there are a few concerns with _that_.
...
design of the [`async-trait`](https://crates.io/crates/async-trait) crate comes from:

- To avoid Complications #1 and #2, [`async-trait`](https://crates.io/crates/async-trait) desugars `async fn`
  to return a `dyn Future` instead of an `impl Future`.
- To avoid Complication #3, [`async-trait`](https://crates.io/crates/async-trait) chooses for you to use
  a `Pin<Box<dyn Future + Send>>` (you can [opt-out](https://github.com/dtolnay/async-trait#non-threadsafe-futures) from the `Send` part).
...
create small combinators, and there the boxing costs _might_ start to
...
- We do want the ability to have `dyn Trait` versions of traits that contain associated
  ...
  - Therefore, in the case of `dyn Trait`, we would want to have the
    `async fn` within returning _some_ form of `dyn Future`. But we would have to effectively
...
for the time being, the solution is clear: use the [`async-trait`](https://crates.io/crates/async-trait)
crate.
...
1. Astute readers might note that I’m eliding a further challenge, which is that you need a scoping mechanism here to handle the lifetimes. Let’s assume we have something like [Rayon’s scope](https://docs.rs/rayon/1.2.0/rayon/fn.scope.html) or [crossbeam’s scope](https://docs.rs/crossbeam/0.7.2/crossbeam/thread/struct.Scope.html) available. [↩︎](#fnref:1)

2. Still, consider a trait `IteratorX` that is like `Iterator`, where the adapters return `impl Trait`. In such a case, you probably want a way to say not only “I take a `T: IteratorX + Send`” but also that the `IteratorX` values returned by calls to `map` and the like are `Send`. Presently you would have to list out the specific associated types you want, which also winds up revealing implementation details. [↩︎](#fnref:2)
