...
url: https://smallcultfollowing.com/babysteps/blog/2012/10/24/restrict-pointers/?utm_source=atom_feed
published: "2012-10-24T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2012/10/24/restrict-pointers/
---
...
I described the problem in [this paper review I wrote](https://smallcultfollowing.com/babysteps/
/blog/2012/09/26/type-system-for-borrowing-permissions/), but I will
...
systems like [Alms](http://www.eecs.harvard.edu/~tov/pubs/alms/).
...
`restrict` (the keyword is [borrowed from C99](http://en.wikipedia.org/wiki/Restrict)). An `&restrict` pointer
is still a borrowed pointer, so it does not convey ownership, but it
is _unique_, meaning that it is the only (live) pointer that points at
...
is _immutable_, you might as well just do an `&T` pointer. There is
...
did not mutate them (as discussed in this post on [purity](https://smallcultfollowing.com/babysteps/
/blog/2012/10/12/extending-the-definition-of-purity-in-rust/);
...
those of you who _are_ familiar with fractional permissions, I think a
...
_restrict_ pointer, the permission drops to 0.
