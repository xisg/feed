...
url: https://smallcultfollowing.com/babysteps/blog/2016/11/02/associated-type-constructors-part-1-basic-concepts-and-introduction/?utm_source=atom_feed
published: "2016-11-02T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2016/11/02/associated-type-constructors-part-1-basic-concepts-and-introduction/
---
...
The first topic I’m going to focus on is [RFC 1598](https://github.com/rust-lang/rfcs/pull/1598), which is a
...
Before I can get to [RFC 1598](https://github.com/rust-lang/rfcs/pull/1598), let me lay out a bit of
...
[this old blog post of mine](https://smallcultfollowing.com/babysteps/
/blog/2016/02/19/parallel-iterators-part-1-foundations/), which walks through several
...
anymore). Anyway, then we can implement the iterator _trait_ like so:
...
Here you see that the impl specifies the type `Item` to be `&'iter T`. This is sort of interesting, because, in a sense, it’s not really
...
us operate generically over iterators like `ListIter<'iter, T>`. That’s very useful, but you might be wondering if it’s possible
...
find that it _almost_ works, but not quite. Let’s give it a try!
...
This shows that in fact modeling _collections_ is actually harder than
modeling _iterators_. Recall that, with iterators, we said that once
...
_collections_, knowing the collection type ( `List<T>`) does **not**
tell us everything we need to know to get the type of an iterator
( `ListIter<'iter, T>`).

[RFC 1598](https://github.com/rust-lang/rfcs/pull/1598) proposes to solve this problem by making it possible to
have not only _associated types_ but associated type **constructors**.
