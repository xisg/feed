...
url: https://smallcultfollowing.com/babysteps/blog/2019/04/22/aic-collaborative-summary-documents/?utm_source=atom_feed
published: "2019-04-22T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2019/04/22/aic-collaborative-summary-documents/
---

# AiC: Collaborative summary documents

In my [previous post](http://smallcultfollowing.com/babysteps/blog/2019/04/19/aic-adventures-in-consensus/), I talked about the idea of _mapping the_
_solution space_:
...
process, but also do something deeper: change the _focus_ of the
...
document](https://paper.dropbox.com/doc/Future-proof-the-Futures-API-Summary--AbplHExNn34jm1~y2i02FYARAg-JODniiQQQcNhHD7iNZ8iM). [This document](https://paper.dropbox.com/doc/Future-proof-the-Futures-API-Summary--AbplHExNn34jm1~y2i02FYARAg-JODniiQQQcNhHD7iNZ8iM) contained my best effort at
...
After that, I [made an announcement](https://github.com/rust-lang/rust/pull/59119#issuecomment-473655294) on the original thread asking
people to participate in the document. Specifically, [as the document\
states](https://paper.dropbox.com/doc/Future-proof-the-Futures-API-Summary--AbpsrNFMirDHgOtZimF11AEUAg-JODniiQQQcNhHD7iNZ8iM#:uid=739200718850749032543986&h2=This-is-an-experiment), the idea was for people to do something like this:

- Read the document, looking for things they didn’t agree with or felt were unfairly represented.
- Leave a comment explaining their concern; or, better, supplying alternate wording that they _did_ agree with
...
things don’t always fit neatly into a _tree_; sometimes I had to
...
let us hide details into a recursive structure. But I worry it’s _too_
_much_ freedom.
...
another way, you also need to explain the _weighting_ that each side
puts on the facts.
...
I tried to address this by [crafting a “summary comment”](https://github.com/rust-lang/rust/pull/59119#issuecomment-474444350) on the main
...
started to _understand_ why one might choose one way or the other when
...
the wrong call” – e.g., in this case, what happens if we decide _not_
to future proof, but then we regret it; in contrast, what happens if
we decide to _add_ future proofing, but we never use it.
...
**document as _advocates_ for their point-of-view, rather than trying to**
...
a way that you _do_ agree with – but which still preserves (what you
...
relatively quickly, and I suspect some people just didn’t even _see_
...
- We should experiment with the best way to _describe_ the
...
work _feels_, by encouraging all of us to deeply understand the design
...
and I will try to respond, though I often get overwhelmed[1](#fn:1), so
...
1. So many things, so little time. [↩︎](#fnref:1)
