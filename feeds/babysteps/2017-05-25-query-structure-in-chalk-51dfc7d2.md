...
url: https://smallcultfollowing.com/babysteps/blog/2017/05/25/query-structure-in-chalk/?utm_source=atom_feed
published: "2017-05-25T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2017/05/25/query-structure-in-chalk/
---

# Query structure in chalk

For my next post discussing [chalk](https://github.com/nikomatsakis/chalk/), I want to take kind of a
...
[this post for an introduction to the big idea](https://smallcultfollowing.com/babysteps/
/blog/2017/01/26/lowering-rust-traits-to-logic/).)
...
_Future compatibility note:_ It’s worth pointing out that I expect
some the particulars of a “query response” to change, particularly as
aturon continues [the work on negative reasoning](http://aturon.github.io/blog/2017/04/24/negative-chalk/). I’m
...
actual _query_. In the section of Prolog examples, I used the Prolog
...
information to find a _unique_ answer. The idea is that we will
...
possibly apply (but see [aturon’s post](http://aturon.github.io/blog/2017/04/24/negative-chalk/), which covers how we
...
In rustc, such a query would look more like `Vec<i32>: AsRef<Vec<?22>>`, where we have simply used an existing inference
...
( [chalk code is in `query.rs`](https://github.com/nikomatsakis/chalk/blob/1f63c8ad20d27f3ef394f230a56430c89482d8d4/src/solve/infer/query.rs#L10-L39)). This is done by replacing
...
with different inference variables (e.g., `Vec<i32>: AsRef<Vec<?44>>`), when we make the canonical form of that query, we’d
get the same result.
...
chalk engine gives you back the lifetime constraints that _would have_
_to be satisfied_ for the query to be provable.
...
we don’t actually _know_ the original lifetimes or their relationships
...
[aturon outlined in their blog post](http://aturon.github.io/blog/2017/04/24/negative-chalk/), when you get back a
