...
url: https://smallcultfollowing.com/babysteps/blog/2014/01/09/rvalue-lifetimes-in-rust/?utm_source=atom_feed
published: "2014-01-09T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2014/01/09/rvalue-lifetimes-in-rust/
---

# Rvalue lifetimes in Rust

I’ve been working on [Issue #3511](https://github.com/mozilla/rust/issues/3511), which is an effort to
...
time ago, I did [some thinking on this issue](https://smallcultfollowing.com/babysteps/
/blog/2012/09/15/rvalue-lifetimes/) and then let it lie
...
[branch now](https://github.com/nikomatsakis/rust/tree/issue-3511-rvalue-lifetimes) that rewrites the cleanup system so that it can
...
values; the tricky part is deciding what the _lifetime_ of these
...
and create a pointer to it. It is _roughly_ equivalent to something
like this:
...
cause inapprorpriate moves. For example, if I wrote `assert_eq!(x.a, y.b)`, and the type of `x.a` was affine, then `assert_eq!` would move
