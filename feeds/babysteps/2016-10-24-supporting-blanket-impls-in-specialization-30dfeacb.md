...
url: https://smallcultfollowing.com/babysteps/blog/2016/10/24/supporting-blanket-impls-in-specialization/?utm_source=atom_feed
published: "2016-10-24T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2016/10/24/supporting-blanket-impls-in-specialization/
---

# Supporting blanket impls in specialization

In my [previous post](https://smallcultfollowing.com/babysteps/
/blog/2016/09/29/distinguishing-reuse-from-override/), I talked about how we can separate out
...
- So, for example, if impl I is `impl<T: Clone> Clone for Option<T>`, and impl J is `impl<U: Copy> Clone for U`, then I will
...
not necessarily an _alternative_ to the intersection impls I discussed
...
In [previous blog post](https://smallcultfollowing.com/babysteps/
/blog/2016/09/29/distinguishing-reuse-from-override/) I gave an example of a `Dump` trait that
...
However, this poses a problem: impl A only applies when `Widget<T>: Debug`, which _may_ be true but is not always true. In particular,
...
for any type T. It is only when we consider the _where clauses_ that
...
- Naturally one might expect that `T: AsRef<T>` would always hold.
  ...
  `&T` reference (duh) – which is sometimes called being _reflexive_.
...
[“Algorithmic formulation” section of the specialization RFC](https://github.com/rust-lang/rfcs/blob/master/text/1210-impl-specialization.md#algorithmic-formulation),
...
J, we can say define an ordering _Subset(I, J)_ that indicates I
matches a subset of the types of J (the RFC calls it `I <= J`). The
current rules then say that I _specializes_ J if _Subset(I, J)_
holds but _Subset(J, I)_ does not.

To decide if _Subset(I, J)_ holds, we apply two tests (both of which
must pass):
...
- **Type(I, J):** For any way of instantiating `I.vars`,
  ...
  - The actual technique here is to [skolemize `I.vars`](https://github.com/rust-lang/rfcs/blob/master/text/1210-impl-specialization.md#skolemization-asking-forallthere-exists-questions) and
    then [attempt unification](https://github.com/rust-lang/rfcs/blob/master/text/1210-impl-specialization.md#unification-solving-equations-on-types). If unification succeeds, then
    `Type(I, J)` holds.
- **WhereClause(I, J):** For the instantiation of `I.vars` used in
  _Type(I, J)_, if you assume `I.wc` holds, you can prove `J.wc`.
...
- _Specializes(I, J)_:
  ...
  - If _Type(I, J)_ holds:

    - If _Type(J, I)_ does not hold:

      - true
    - Otherwise, if _WhereClause(I, J)_ holds:

      - If _WhereClause(J, I)_ does not hold:
...
You could also write this as _Specializes(I, J)_ is:
...
Unlike before, we also need a separate test to check whether _reuse_
is legal. Reuse is legal if _Subset(I, J)_ holds.

You can view the _Specializes(I, J)_ test as being based on a partial
order, where the `<=` predicate is the lexicographic combination of
two other partial orders, _Type(I, J)_ and _WhereClause(I, J)_. This
...
type `T`, so _Type(A, B)_ and _Type(B, A)_ will both hold) and (b)
...
previous section). This is because if I add an impl like `impl<T: Debug> Dump for T` where none existed before, some other crate may
already have an impl like `impl Dump for MyType`, and then if `MyType: Debug`, we would have an overlap conflict, and hence that downstream
crate will not compile (see [RFC 1023](https://github.com/rust-lang/rfcs/blob/master/text/1023-rebalancing-coherence.md) for more information on
these rules).
...
because of [the orphan rules](http://smallcultfollowing.com/babysteps/blog/2015/01/14/little-orphan-impls/), have more specific types, and
...
`Debug` and `Display`. But with this new impl, _all_ types that
...
does not lie in the _impls_, but rather in the _traits_. In
particular, the `Copy` trait is a _subtrait_ of `Clone` – that is,
anything which is copyable must also be cloneable. But `Display` and
`Debug` have no relationship; in fact, the blanket impl
interconverting between them is effectively _imposing_ an
...
[similar example raised by withoutboats and reddit user oconnor663](https://github.com/rust-lang/rfcs/pull/1658#issuecomment-249453099)
...
_syntactic_ criteria for when something is more specialized – in that
...
_subset_ rule. The rule is fairly simple to describe: **impls with**
...
currently subject to the rules in [RFC 1023](https://github.com/rust-lang/rfcs/blob/master/text/1023-rebalancing-coherence.md). This implies that
...
this, by tweaking the _WhereClause(I, J)_ test to be more
