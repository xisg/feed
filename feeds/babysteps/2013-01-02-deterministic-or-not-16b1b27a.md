...
url: https://smallcultfollowing.com/babysteps/blog/2013/01/02/deterministic-or-not/?utm_source=atom_feed
published: "2013-01-02T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2013/01/02/deterministic-or-not/
---
...
1. _Deterministic results where possible:_ The function behaves “as
   if” it executed sequentially, executing the kernel from 0 to n,
   just like `Array.map`.
2. _Error:_ An exception is thrown.
3. _Non-determinstic results:_ The function behaves “as if” it
   executed sequentially, but the items were mapped in an unspecified
   order.

The [branch](https://github.com/syg/iontrail) currently implements option 3: I believe it is
...
If you examine [the specification for `ParallelArray.reduce()`](http://wiki.ecmascript.org/doku.php?id=strawman:data_parallelism#reduce),
...
users will want the _best parallel performance they can get_, and they
...
always the possibility of _bailout_. A bailout can occur because we
...
If, however, we choose _not_ to guarantee deterministic results, these
...
[our current code does](https://github.com/syg/iontrail). Each worker tracks its current
...
[argued in a previous post](https://smallcultfollowing.com/babysteps/
/blog/2012/10/24/purity-in-parallel-javascript/)) it does so by imposing a
