...
url: https://smallcultfollowing.com/babysteps/blog/2014/02/25/rust-rfc-stronger-guarantees-for-mutable-borrows/?utm_source=atom_feed
published: "2014-02-25T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2014/02/25/rust-rfc-stronger-guarantees-for-mutable-borrows/
---

# Rust RFC: Stronger guarantees for mutable borrows

Today, if you do a mutable borrow of a local variable, you lose the
ability to _write_ to that variable except through the new reference
you just created:
...
However, you retain the ability to _read_ the original variable:
...
[data parallelism in Rust](http://smallcultfollowing.com/babysteps/blog/2013/06/11/data-parallelism-in-rust/) based on closure bounds. The rough idea
...
_always_ mutable. If we permitted two threads to touch the same
...
`fork_join` would be _bounded_ by the builtin trait `Share`. As I
wrote in [issue 11781](https://github.com/mozilla/rust/issues/11781#issuecomment-35559695), the trait `Share` indicates data that
...
Things which are _not_ sharable include:
...
There is a wrinkle though. With the _current_ borrow checker rules,
forkable data is only safe to access from a parallel thread if the
_main thread_ is suspended. Put another way, forkable closures can
...
_Note:_ This is a refinement of the [previous proposal](http://smallcultfollowing.com/babysteps/blog/2013/06/11/data-parallelism-in-rust/), which was
...
that’s coming) plans for [unboxed closures](https://github.com/mozilla/rust/issues/8622), which are required
...
_An aside:_ it should be noted that this API also opens the door
...
similar to what research languages like [Mezzo](http://protz.github.io/mezzo/) do. (By the way,
