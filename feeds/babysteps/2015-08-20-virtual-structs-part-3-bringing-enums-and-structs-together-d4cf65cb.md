...
url: https://smallcultfollowing.com/babysteps/blog/2015/08/20/virtual-structs-part-3-bringing-enums-and-structs-together/?utm_source=atom_feed
published: "2015-08-20T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2015/08/20/virtual-structs-part-3-bringing-enums-and-structs-together/
---

# Virtual Structs Part 3: Bringing Enums and Structs Together

So, in [previous](http://smallcultfollowing.com/babysteps/blog/2015/05/05/where-rusts-enum-shines/) [posts](http://smallcultfollowing.com/babysteps/blog/2015/05/29/classes-strike-back/), I discussed the pros and cons of two different
...
another post (or perhaps an RFC). _Spoiler alert:_ they can be viewed
as a special case of [specialization](https://github.com/rust-lang/rfcs/pull/1210).
...
_Note:_ I want to be clear that the designs in this blog post are not
...
This is often called an _algebraic data type_, but “algebra” brings
back memories of balancing equations in middle school (not altogether
unpleasant ones, admittedly), so I’m going to use the term _type_
_hierarchy_ instead. Anyway, to see what I mean, let’s look at my
...
Let’s revisit an example from [the previous post](http://smallcultfollowing.com/babysteps/blog/2015/05/29/classes-strike-back/). In the compiler,
...
_One interesting question is whether we can concisely state_
_conditions in which one would prefer to have “precise variant sizes”_
_(class-like) vs “largest variant” (enum). I think the “precise_
_sizes” approach is better when the following apply:_

- _A recursive type (like a tree), which tends to force boxing_
  _anyhow. Examples: the AST or types in the compiler, DOM in servo, a_
  _GUI._
- _Instances never change what variant they are._
- _Potentially wide variance in the sizes of the variants._

Therefore, it is possible to declare the root enum in a type hierarchy
as either sized (the default) or _unsized_; this choice is inherited
...
- `dynamic_cast` is somewhat inefficient, since it must handle the
  ...
  [reimplementing RTTI (the C++ name for matching) from scratch](http://llvm.org/docs/CodingStandards.html#do-not-use-rtti-or-exceptions).
...
form of phantom data, and that would be _binding_ on the types
...
_Finally,_ confining extension to a trait is relevant to the
...
[HTML DOM](https://developer.mozilla.org/en-US/docs/Web/API/Document_Object_Model/Introduction). It might look something like this (browser people:
...
- Traits start with the assumption that you want to work with any
  type, and let you narrow that. Basically, you get code that is _as_
  _general as possible_.
- In contrast, enums assume you want to work with a fixed set of
  types. This means you can write code that is _as specific as_
  _possible_. Enums also work best when the types you are choosing
...
traits, as [eddyb and kimundi proposed in RFC #250](https://github.com/rust-lang/rfcs/pull/250).) Also,
...
variant”. Note that a `TypeData::struct` value is _not_ a `TypeData`
...
case of [“DST coercions”](https://github.com/rust-lang/rfcs/blob/master/text/0982-dst-coercion.md). Basically, coercing to `&Node` is
...
normally actually subtyping between _references_. That is, in Java we
...
[object slicing](http://stackoverflow.com/questions/274626/what-is-object-slicing). The problem here is that in C++ the superclass
type is really just the superclass fields; so if you do `superclass = subclass`, then you are just going to drop the extra fields from the
...
fact that `&mut None<i32>` -\> `&mut Option<i32>` is _not_ safe is an
example of why sized enums can in fact be more challenging here. (If it’s
not clear why that should be unsafe, the [Nomicon’s section on variance](https://doc.rust-lang.org/nightly/nomicon/subtyping.html#variance)
