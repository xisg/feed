...
url: https://smallcultfollowing.com/babysteps/blog/2016/05/23/unsafe-abstractions/?utm_source=atom_feed
published: "2016-05-23T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2016/05/23/unsafe-abstractions/
---
...
[Rust memory model](https://github.com/rust-lang/rfcs/issues/1447), which is basically a set of rules that help
...
excellent blog post on [Helix](http://blog.skylight.io/introducing-helix/) for more details, as well as
...
Of course, just because it’s _possible_ to use unsafe code to make
...
such as how [Crossbeam](https://github.com/aturon/crossbeam) and [deque](https://github.com/kinghajj/deque) use unsafe code to implement
non-blocking data structures, or [Jobsteal](https://github.com/rphmeier/jobsteal) and [Rayon](http://smallcultfollowing.com/babysteps/blog/2015/12/18/rayon-data-parallelism-in-rust/) use unsafe
...
if you [try to build it](https://is.gd/2UpNUr) you will find it gets a compilation
...
1. In general, the compiler does not try to reason precisely about
   ...
   ( `foo[_]`, effectively). This means that it cannot tell that `&mut self[0..mid]` is disjoint from `&mut self[mid..]`. The reason for
...
2. In fact, the `[]` operator is not builtin to the language when
   ...
   [implemented in the standard library](https://github.com/rust-lang/rust/blob/b9a201c6dff196fc759fb1f1d3d292691fc5d99a/src/libcore/slice.rs#L572-L589). Therefore, even if
   the compiler knew that `0..mid` and `mid..` did not overlap, it
   wouldn’t necessarily know that `&mut self[0..mid]` and `&mut self[mid..]` return disjoint slices.
...
Rust type system expects. _Within_ the unsafe boundary, however, you
are free to bend the rules (of course, just _how_ free you are is the
...
slices must be _disjoint_ (that is, pointing at different regions of
memory).
...
what [the standard library actually does](https://github.com/rust-lang/rust/blob/b9a201c6dff196fc759fb1f1d3d292691fc5d99a/src/libcore/slice.rs#L338-L349). However, this
...
[implementation in the standard library](https://github.com/rust-lang/rust/blob/b9a201c6dff196fc759fb1f1d3d292691fc5d99a/src/libcore/slice.rs#L338-L349) (it uses some
slightly different helpers, but it’s the same idea).

### Extending the abstraction boundary

Of course, it might happen that we actually _wanted_ to assume `mid`
...
Although there is nothing in the language that _explicitly_ connects
...
look at the [actual definition](https://github.com/rust-lang/rust/blob/cf37af162721f897e6b3565ab368906621955d90/src/libcollections/vec.rs#L272-L275) of `Vec` from the standard
...
type](https://github.com/rust-lang/rust/blob/cf37af162721f897e6b3565ab368906621955d90/src/liballoc/raw_vec.rs) that encapsulates the idea of a pointer and a capacity:
...
the data _in_ that buffer, you can only do so by calling
[the `ptr` method](https://github.com/rust-lang/rust/blob/cf37af162721f897e6b3565ab368906621955d90/src/liballoc/raw_vec.rs#L143-L145), which returns a `*mut T`. This is a raw
...
about _exactly_ what is legal within unsafe code and not. Clearly, the
...
[aim to address](https://github.com/rust-lang/rfcs/issues/1447). In fact there has even been a
[first RFC](https://github.com/rust-lang/rfcs/pull/1578) introduced on the topic, though I think we can
expect a fair amount of iteration before we arrive at the final and
complete answer.

As I [wrote on the RFC thread](https://github.com/rust-lang/rfcs/pull/1578#issuecomment-217184537), my take is that we should be
