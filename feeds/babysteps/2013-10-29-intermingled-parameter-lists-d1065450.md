...
url: https://smallcultfollowing.com/babysteps/blog/2013/10/29/intermingled-parameter-lists/?utm_source=atom_feed
published: "2013-10-29T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2013/10/29/intermingled-parameter-lists/
---

# Intermingled parameter lists

I’ve been hard at work finishing up work on Rust’s
[“new” syntax for lifetimes](https://github.com/mozilla/rust/issues/4846) – I put “new” in quotes because
...
are needed to fix [issue #5121](https://github.com/mozilla/rust/issues/5121). The full
...
- Require that, when explicit parameters are supplied, values for _all_
...
at runtime. This is because Rust uses a _monomorphization_ scheme for
...
To put it another way, when you specify a closure type like `|uint| -> float`, we don’t permit that type to be _generic_ with respect to
...
[new syntax](http://smallcultfollowing.com/babysteps/blog/2013/10/10/fn-types-in-rust/) that was recently proposed). Even when you
define a generic function (like `fn foo<T>(t: T) -> uint`), the _type_
...
_early bound_, meaning that we substitute concrete values for them as
early as possible.
...
lifetime `'a` – but the lifetime `'a` can be different _each time the_
_closure is called_). I will call this a _late-bound lifetime_
_parameter_, because we don’t need to substitute a specific lifetime
...
actually _do_ the dereference. Therefore, `'l` can be late bound. We
...
(well, you can _write_ `Foo` in some cases, but the compiler will
...
_references the lifetime parameter_ `'arena`. This means that we
...
The way this function is written, `'l` would be considered _early_
_bound_ because it comes _before_ the type parameter `T`. That means
...
What the user could _not_ do, given this definition of `get_index`, is to
...
requested as [issue #9508](https://github.com/mozilla/rust/issues/9508).
