---
title: More on fns
url: https://smallcultfollowing.com/babysteps/blog/2013/06/03/more-on-fns/?utm_source=atom_feed
published: "2013-06-03T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2013/06/03/more-on-fns/
---
...
thinking that the proper name is probably `Thunk`. I quote [Wikipedia](http://en.wikipedia.org/wiki/Thunk_%28functional_programming%29)
...
other [contrary uses](http://en.wikipedia.org/wiki/Thunk_%28object-oriented_programming%29) for the term)
...
be equivalent to `func(|| ...)`. If the function expects a `[sigil] Thunk`, then `do func { ... }` would be equivalent to
...
standard library, these functions are not _that_ common, but they do
occur.
...
a [recent e-mail to rust-dev](https://mail.mozilla.org/pipermail/rust-dev/2013-May/004281.html), where I suggested that we
...
the `|| default` closure is _moving_ the argument default, and this is
...
[kballard encountered it](https://github.com/mozilla/rust/pull/6815#issuecomment-18781051) when refactoring the methods on
hashmap.
...
_copying_ vs _by reference_ closure. The way I described thunks, they
...
_more_ complicated to me than saying “ `thunk` s copy/move the values
...
closures should be something else, such as `closure(S) -> T`, `proc(S) -> T`, or even `|S| -> T`. This is pure bikeshedding, but it has some
