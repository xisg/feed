...
url: https://smallcultfollowing.com/babysteps/blog/2017/02/21/non-lexical-lifetimes-using-liveness-and-location/?utm_source=atom_feed
published: "2017-02-21T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2017/02/21/non-lexical-lifetimes-using-liveness-and-location/
---

# Non-lexical lifetimes using liveness and location

At the recent compiler design sprint,
[we spent some time discussing **non-lexical lifetimes**](https://public.etherpad-mozilla.org/p/rust-compiler-design-sprint-paris-2017-nll),
...
I have a [prototype implementation](https://github.com/nikomatsakis/nll) of this system. The example
...
example as “Example 4”. This is because my [previous post](http://smallcultfollowing.com/babysteps/blog/2016/04/27/non-lexical-lifetimes-introduction/)
...
will take place in the context of [MIR](https://blog.rust-lang.org/2016/04/19/MIR.html), in which programs are represented
...
subtyping constraints arise. For example, at point A/0, we have `p = &'1 foo`. Here, the type of `&'1 foo` is `&'1 i32`, and the type of
...
- Our earliest thoughts assumed continuous regions (e.g., [RFC 396](https://github.com/rust-lang/rfcs/pull/396)).
...
- An alternative solution might be to consider continuous regions but apply
  ...
  - This allows the example in this post to type, but it falls down on
    more advanced examples, such as [vec-push-ref](https://github.com/nikomatsakis/nll/blob/a6609ab17fd483f8d47ef919af3838bf214954e5/test/vec-push-ref.nll) (hat tip,
  ...
  - You can go farther, and give variables a distinct type at
    ...
    [stateful MIR for Rust](https://github.com/Ericson2314/a-stateful-mir-for-rust). But even then you must contend with
  ...
  - Exploring this led to the development of the “localized” subregion
    ...
    [in my original series](http://smallcultfollowing.com/babysteps/blog/2016/05/09/non-lexical-lifetimes-adding-the-outlives-relation/) but which we elaborated more fully at the
    rustc design sprint.
...
- I’d like to talk about how to incorporate lifetime parameters on fns
  ...
  [as illustrated by this example in my prototype](https://github.com/nikomatsakis/nll/blob/master/test/get-default.nll)).
- There are various options for modeling the
  [“deferred borrows” needed to accept `vec.push(vec.len())`](https://internals.rust-lang.org/t/accepting-nested-method-calls-with-an-mut-self-receiver/4588).
- We might consider a finer-grained notion of liveness that operates
  ...
  when doing move-checking. This would help to make `let (p, q) = (&foo, &bar)` and `let pair = (&foo, &bar)` entirely equivalent (in
...
Comments? [Let’s use this old internals thread.](https://internals.rust-lang.org/t/non-lexical-lifetimes-based-on-liveness/3428)
