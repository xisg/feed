...
url: https://smallcultfollowing.com/babysteps/blog/2017/01/26/lowering-rust-traits-to-logic/?utm_source=atom_feed
published: "2017-01-26T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2017/01/26/lowering-rust-traits-to-logic/
---
...
😉). More seriously, if you’ve never worked with Prolog, don’t
...
Rust type – is a _predicate_ that represents the idea that the type
`Foo` implements `Clone`. These rules are _program clauses_ that state
...
_equality_ means in this logic. You can see that I’ve been writing
...
Intuitively, this should hold, because `<IntoIter<usize> as Iterator>::Item` is defined to be `usize`, and we know that
...
`Clone(usize)` to prove the goal `Clone(<IntoIter<usize> as Iterator>::Item)`.
...
take the same rules for type equality that we defined as `TypeEqual(A, B)` but move it into the _trait-solving engine itself_ (or, depending
...
was that I was searching out what is the _simplest_ and most minimal
...
constraints (e.g., \`fn foo() where A = i32\`\`). The key to doing
...
[one of them](http://www.alice.virginia.edu/~weimer/2011-6610/reading/nelson-oppen-congruence.pdf) in the [ena](http://github.com/nikomatsakis/ena) crate that I’m using to do
...
let’s turn our focus a bit towards _type-checking_. Type-checking is
...
type-safe _for all types `T`_, not just for some specific type. We might express
this like so:
...
[“A Proof Procedure for the Logic of Hereditary Harrop Formulas”](http://dl.acm.org/citation.cfm?id=868380).
...
prototype solver in the [chalk](http://github.com/nikomatsakis/chalk) repository. The code consists of a
[parser](https://github.com/nikomatsakis/chalk/blob/master/chalk-rust-parse/src/parser.lalrpop) for a [subset of Rust syntax](https://github.com/nikomatsakis/chalk/blob/master/chalk-rust-parse/src/ast.rs). It then [lowers this syntax](https://github.com/nikomatsakis/chalk/blob/master/chalk-rust/src/lower/mod.rs)
into an [internal IR](https://github.com/nikomatsakis/chalk/blob/master/chalk-rust/src/ir/mod.rs) that maps fairly cleanly to the things I’ve been
showing you here. What I would like is for chalk to become the
**normative implementation** of the trait system: that is, chalk
basically would describe how the trait system is _supposed_ to
...
implementing them in the normative codebase _first_, so that we can
uncover any complications. As a proof of concept of that approach,
I’ve implemented withoutboat’s
[associated type constructor RFC](https://github.com/rust-lang/rfcs/pull/1598), which I will describe in a
future post (preview: it’s very easy to do and works out beautifully;
in fact, it doesn’t add _anything at all_ to the logic, once you
...
I started a [thread on internals](https://internals.rust-lang.org/t/blog-series-lowering-rust-traits-to-logic/4673) to discuss this post and other
