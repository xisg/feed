...
url: https://smallcultfollowing.com/babysteps/blog/2015/01/14/little-orphan-impls/?utm_source=atom_feed
published: "2015-01-14T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2015/01/14/little-orphan-impls/
---

# Little Orphan Impls

We’ve recently been doing a lot of work on Rust’s _orphan rules_,
which are an important part of our system for guaranteeing _trait_
_coherence_. The idea of trait coherence is that, given a trait and
...
any time. It has [its own complications](https://mail.mozilla.org/pipermail/rust-dev/2011-December/001036.html); if you’re curious for
more background on why we use coherence, you might find this
[rust-dev thread](https://mail.mozilla.org/pipermail/rust-dev/2011-December/thread.html#1036) from a while back to be interesting
reading.)

The role of the _orphan rules_ in particular is basically to prevent
you from implementing _external traits for external types_. So
...
`Show` and `Vec` are defined in the standard library. But you _can_
...
When we extended the trait system to [support](http://smallcultfollowing.com/babysteps/blog/2014/09/30/multi-and-conditional-dispatch-in-traits/) [multidispatch](https://github.com/rust-lang/rfcs/blob/master/text/0195-associated-items.md), I confess that I originally didn’t give the
...
[Rust discusstion forums](http://discuss.rust-lang.org/t/orphan-rules/1322).
...
Now the problems arise because those same rules _also_ permit an impl
...
This problem was first brought to our attention by [arielb1](https://github.com/arielb1), who
filed [Issue 19470](https://github.com/rust-lang/rust/issues/19470). To resolve it, he proposed a rule that I
will call the _ordered rule_. The ordered rule goes like this:
...
2. The name of some local struct or enum must appear on that line before the first
   type parameter.
   - _More formally:_ When visiting the types in pre-order, a local type must be visited
...
accepts. For example, consider the case of the [`Modifier` trait](https://github.com/reem/rust-modifier)
that is used in hyper and iron. As you can see in [this issue](https://github.com/rust-lang/rust/issues/20974),
...
likely (in the abstract), would _not_ be accepted:
...
I’ll call the _covered_ rule. The idea of the covered rule was to say
...
_covered_ by a local type. Covered means that it appears “inside” the
...
This rule
[turns out to give us the required orphan rule guarantees](https://github.com/rust-lang/rust/issues/19470#issuecomment-66846120). To
...
the so-called _auxiliary_ pattern, in which a trait takes a type parameter
...
for most types, they will work with _any_ hashing function. Sadly, this impl is rejected,
...
method itself, but then _that_ is limiting. It makes the `Hash` trait
not object safe, for one thing, and it also prohibits us from writing
types that _are_ specialized to particular hash functions.
...
there is _no_ answer that permits the covered rule to handle the
...
algorithm and thus is inherently _not_ going to be covered by the type
...
ordering is to classify type parameters as _self-like_ and _auxiliary_.
...
reject _both impls_. This is because the type parameter `I` is
