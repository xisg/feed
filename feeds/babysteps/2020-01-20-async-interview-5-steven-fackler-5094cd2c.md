...
url: https://smallcultfollowing.com/babysteps/blog/2020/01/20/async-interview-5-steven-fackler/?utm_source=atom_feed
published: "2020-01-20T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2020/01/20/async-interview-5-steven-fackler/
---

# Async Interview #5: Steven Fackler

Hello! For the latest [async interview](http://smallcultfollowing.com/babysteps/blog/2019/11/22/announcing-the-async-interviews/), I spoke with Steven Fackler
( [sfackler](https://github.com/sfackler/)). sfackler has been involved in Rust for a long time and
is a member of the Rust libs team. He is also the author of [a lot of\
crates](https://crates.io/users/sfackler), most notably [tokio-postgres](https://crates.io/crates/tokio-postgres).
...
You can watch the [video](https://youtu.be/nerrc3L9qrM) on YouTube. I’ve also embedded a copy here
...
So what are the concerns that are motivating [tokio-rs/tokio#17144](https://github.com/tokio-rs/tokio/pull/1744)?
...
version of [`Read`](https://doc.rust-lang.org/std/io/trait.Read.html) from `std`, and both of these concerns apply there
as well. In fact, part of why I wanted to talk to sfackler
specifically is that he is the author of an [excellent paper\
document](https://paper.dropbox.com/doc/MvytTgjIOTNpJAS6Mvw38) that covers the problem of using uninitialized memory
...
After all, the `Read` trait is not supposed to be _reading_ from
`buf`, it’s just supposed to be _writing_ into it – so it shouldn’t
...
isn’t _supposed_ to, the `Read` impl can trivially read from `buf` without
...
to start, then the first `n` bytes should be written now – but _are_
...
This impl has no unsafe code. It _claims_ that it has initialized the
...
sfackler’s paper document](https://paper.dropbox.com/doc/MvytTgjIOTNpJAS6Mvw38), which covers much of the same
material.
...
- Finally, creating a struct `BufMuf` in the stdlib for dealing with partially initialized
  buffers, and adding a `read` method for _that_.
...
imagine that when you first allocate some memory, it contains _some_
...
you try to read from it, the compiler will substitute _some_ value,
but it won’t necessarily be the _same_ value every time. This behavior
...
_until_ the virtual page is written to. At that point, the memory is
...
If you’d like a deeper treatment, I recommend [Ralf’s blog post](https://www.ralfj.de/blog/2019/07/14/uninit.html).
...
the `&mut [u8]` _reference_, we need to fix the memory it _refers to_.
...
doc](https://paper.dropbox.com/doc/MvytTgjIOTNpJAS6Mvw38) can wind up having to zero the buffer multiple times,
...
make `Read` not “dyn-safe”[1](#fn:1), and that’s a non-starter.
...
with a struct.[2](#fn:2) The API of this struct would be fairly similar
...
create it from a slice of “maybe initialized” bytes (e.g., `&mut [MaybeUninit<u8>]`. There can also be convenience methods to create a
...
noted that we could make _both_ `read` and `read_uninit` have a
default implementation, one implementing in terms of the other. In
this case, users would have to implement one or the other
(implementing _neither_ would lead to an infinite loop, and we would
...
[tokio-rs/tokio#17144](https://github.com/tokio-rs/tokio/pull/1744), and I did get back some interesting results,
\[which I summarized in this hackmd at the time\]\[tokio-hackmd\]. In
short, hyper’s benchmarks show a fairly sizable impact, with
uninitialized data getting speedups[3](#fn:3) of 1.3-1.5x. Other
...
thread](https://github.com/tokio-rs/tokio/pull/1744#issuecomment-553179399) and in our call). He was pointing out [#23820](https://github.com/rust-lang/rust/pull/23820), a PR which
rewrote [`read_to_end`](https://doc.rust-lang.org/std/io/trait.Read.html#method.read_to_end) in the stdlib. The older implementation was
...
buffer to write the data into. But a _vectorized_ interface takes a
...
to _detect_ when a reader supports vectorized reads. This would allow
...
to the vectorized read method returns _just enough_ data to fit in the
...
_do_, people who wished to implement the method may forget to do so –
...
1. Most folks say “object-safe” here, but I’m trying to shift our terminology to talk more about the dyn keyword. [↩︎](#fnref:1)

2. Carl Lerche proposed something similar on the tokio thread [here](https://github.com/tokio-rs/tokio/pull/1744#issuecomment-553575438). [↩︎](#fnref:2)
...
\[tokio-hackmd\]: [https://hackmd.io/ukeyehx7Ta-6KhaVRFi2mg#Measuring-the-impact](https://hackmd.io/ukeyehx7Ta-6KhaVRFi2mg#Measuring-the-impact)
\[#26950\]: [https://github.com/rust-lang/rust/pull/26950](https://github.com/rust-lang/rust/pull/26950) [↩︎](#fnref:3)
