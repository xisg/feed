...
url: https://smallcultfollowing.com/babysteps/blog/2011/12/12/challengines-implementing-unique-closures/?utm_source=atom_feed
published: "2011-12-12T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2011/12/12/challengines-implementing-unique-closures/
---

# Challengines implementing unique closures

**Update:** See the recent post addressing
[the solution to this problem](https://smallcultfollowing.com/babysteps/
/blog/2011/12/13/partially-ordered-unique-closures/).
...
relationship. There is also the fact that I think it should _always_
...
away. But if we down _that_ route, then we end up with a lot of
different kinds of functions; we can no longer unify bare functions
and sendable functions, since bare functions _must_ be usable as
...
[my previous post about procedures/coroutines](https://smallcultfollowing.com/babysteps/
/blog/2011/12/06/coroutines-for-rust/). I like this
...
environment. Thus the type for _all_ closures would be `fn(T)->U`.
This would be a [dynamically sized type](https://smallcultfollowing.com/babysteps/
/rust/no-implicit-copies#dynamically-sized-types) and hence subject to
...
- it relies on the [no implicit copies (NIC) proposal](https://smallcultfollowing.com/babysteps/
  /rust/no-implicit-copies#dynamically-sized-types) and
  [regions](https://github.com/graydon/rust/wiki/Proposal-for-regions), neither of which have been accepted, much less
  implemented;
...
down to _one type of function_ ( `fn(T)->U`) without any loss of
...
implement the [NIC proposal](https://smallcultfollowing.com/babysteps/
/rust/no-implicit-copies#dynamically-sized-types) and fully spec out and implement
[region support](https://github.com/graydon/rust/wiki/Proposal-for-regions). However, the third and fourth options are not
