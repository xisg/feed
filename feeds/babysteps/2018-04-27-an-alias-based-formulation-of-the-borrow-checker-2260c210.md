...
url: https://smallcultfollowing.com/babysteps/blog/2018/04/27/an-alias-based-formulation-of-the-borrow-checker/?utm_source=atom_feed
published: "2018-04-27T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2018/04/27/an-alias-based-formulation-of-the-borrow-checker/
---
...
suite and also handles a few cases – such as [#47680](https://github.com/rust-lang/rust/issues/47680#issuecomment-363131420) – that the
...
that time](https://github.com/rust-lang/rust/commit/50a3dd40ae8ae6494e55d5cfc29eafdb4172af52), gives a good picture of what Rust was like then.[1](#fn:1)[2](#fn:2)
...
- In the system described in the NLL RFC, `'a` – called a lifetime –
  ...
  control-flow graph.
- Under _this_ proposal, `'a` – which I will be calling a region[3](#fn:3) –
...
write, or move – would invalidate the loan. For a shared loan like `p = &x`, reading through `x` (or `p`) is allowed, but writing or
...
traditionally called this an _outlives relationship_, but I am going
to call it a _subset relationship_ instead, as befits the new meaning
of regions[4](#fn:4).
...
[Datalog](https://en.wikipedia.org/wiki/Datalog) rules. Datalog is – in some sense – a subset of Prolog
designed for efficient execution. It basically corresponds to rules
like this (using the syntax from the [Souffle](https://github.com/oracle/souffle/wiki) project):
...
here those relations are declared with `.decl` [5](#fn:5). Some relations
...
“logical not” and “negation as failure”.[6](#fn:6)

To implement these rules, I’ve been using Frank McSherry’s awesome
[differential-dataflow](https://crates.io/crates/differential-dataflow) crate. This has been a pretty great
...
iterate, so you must ensure both the older relations _and_ the newer
...
the control-flow graph edges:[7](#fn:7)
...
So far, we introduced the _subset_ relation that shows the
...
works here, it’s the same as it was defined in the [NLL RFC](https://rust-lang.github.io/rfcs/2094-nll.html).
...
then it also requires `L` at the successor `Q` – _so long as `L` is_
_not “killed” at `P`_. So what is this `!killed(L, P)` rule? The killed
...
reflected by _killing_ the loan L0, thus indicating that `y` would no
...
`L1`. Moreover, the variable `v` holds references of type `&'0 i32`, and we can see that `'4` is a subset of `'0`:
...
This is _almost_ the analysis that I implemented, except for one
...
The problem arises because we _re-assigned_ an existing variable `p`
rather than declaring a new one. This re-uses the same region `'0`.
We _could_ therefore solve this by modifying the program to use a
fresh variable:
...
This improvement is also crucial to accepting the example from [#47680](https://github.com/rust-lang/rust/issues/47680#issuecomment-363131420),
...
(and indeed it _should_ be), but with the current rules it was not,
...
I called [three-point form](https://rust-lang.github.io/rfcs/2094-nll.html#leveraging-intuition-framing-errors-in-terms-of-points):
...
top-down and bottom-up propagation (e.g., [magic sets](https://www.sciencedirect.com/science/article/pii/S0004370212000562)); there are
...
tuple (like `error(P)`). [One such technique](http://www.vldb.org/pvldb/vol9/p1137-chothia.pdf) was even
[implemented](https://github.com/frankmcsherry/explanation) and defined using [differential-dataflow](https://crates.io/crates/differential-dataflow), which
is great.
...
I’ve opened an [thread on the Rust internals board](https://internals.rust-lang.org/t/blog-post-an-alias-based-formulation-of-the-borrow-checker/7411) for discussion.
...
[differential-dataflow](https://crates.io/crates/differential-dataflow) crate really did enable me to iterate a lot
...
the [Viper static verifier](http://www.pm.inf.ethz.ch/research/viper.html), which required them to re-engineer
...
1. We were still using [the `alt` and `ret` keywords](https://github.com/rust-lang/rust/commit/50a3dd40ae8ae6494e55d5cfc29eafdb4172af52#diff-26be476d05bea7e3cd4e452d6104482dR1758), and not yet using `=>` for match arms! Neat. And still kind of inscrutable to me. [↩︎](#fnref:1)

2. And [macros were like `#foo[..]` instead of `foo!(..)`](https://github.com/rust-lang/rust/commit/50a3dd40ae8ae6494e55d5cfc29eafdb4172af52#diff-20cc6d854aa3f056ddd3c36b7c257765R332). I remember pcwalton used to complain about “squashed spiders” all over the code. [↩︎](#fnref:2)

3. Region is the standard term from academia, so I am adopting it by default, but it doesn’t necessarily carry the right “intuitions” here. We should maybe fish about for a better term. [↩︎](#fnref:3)

4. Interestingly, this means that the type `&'a u32` is _covariant_ with respect to `'a`, whereas before it was most naturally defined as _contravariant_ – that is, _subtypes_ correspond to _smaller sets_ (but _larger lifetimes_). Again, not a thing that really matters to Rust users, but a nice property for those delving into the type system. [↩︎](#fnref:4)

5. These `.foo` directives are specific to souffle, as far as I know. [↩︎](#fnref:5)

6. We employ negation in these rules, but only in a particularly trivial way – negated inputs. [↩︎](#fnref:6)

7. This subset propagation rule is the rule that we are going to refine later. [↩︎](#fnref:7)
