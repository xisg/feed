...
url: https://smallcultfollowing.com/babysteps/blog/2016/03/02/nice-errors-in-lalrpop/?utm_source=atom_feed
published: "2016-03-02T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2016/03/02/nice-errors-in-lalrpop/
---

# Nice errors in LALRPOP

For the last couple of weeks, my mornings have been occupied with a
pretty serious revamping of [LALRPOP’s](http://smallcultfollowing.com/babysteps/blog/2015/09/14/lalrpop/) error message output. I will
...
While working on [adapting the Rust grammar to LALRPOP](https://github.com/nikomatsakis/rustypop), I
found I was wasting a lot of time trying to decrypt the error
messages, and I wanted to do something about it. This work
is the result.

**An aside:** It’s definitely worth citing [Menhir](http://gallium.inria.fr/~fpottier/menhir/) as an inspiration,
...
0.10, though, you get this (the actual output even [uses ANSI colors](http://imgur.com/nHdMXt5),
if available):
...
`"import"` token. An `"import"` is the _start_ of an `ImportDecl`, but
...
but we _do_ see a `Path`, and a `Path` is defined like so:
...
the stack _would_ be the right thing to do. But for other inputs, it
...
listed after `"import"` should be a _path_ not an _identifier_. Now of
...
change I would _actually_ make to that grammar would probably be to
...
[lane table](http://cssauh.com/xc/pub/LaneTable_APPLC12.pdf) generation algorithm and other variations on LR(1). This
...
- an explanation of the [lane table](http://cssauh.com/xc/pub/LaneTable_APPLC12.pdf) algorithm, which is rather
  underdocumented (but I’m still figuring it out myself);
- [rustypop](https://github.com/nikomatsakis/rustypop), my Rust grammar in LALRPOP, is coming along, and I want
