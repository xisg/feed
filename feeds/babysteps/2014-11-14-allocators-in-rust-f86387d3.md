...
url: https://smallcultfollowing.com/babysteps/blog/2014/11/14/allocators-in-rust/?utm_source=atom_feed
published: "2014-11-14T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2014/11/14/allocators-in-rust/
---
...
1. **It is better to have one global allocator than two.** Allocators like
   [jemalloc](http://www.canonware.com/jemalloc/), [dlmalloc](http://g.oswego.edu/dl/html/malloc.html), and so forth are all designed to be the
...
the _default_ behavior, to some extent. The Rust standard library
...
_normal setup_ that all C programs use by default, so relative to a
...
A [recent PR](https://github.com/rust-lang/rust/pull/18678) aimed to remove the prefix. It was rejected
...
1. The host process wins, `malloc` means the same thing
   everywhere (this occurs on [linux by default](https://gist.github.com/alexcrichton/8638ac35a79834adcc57)).
2. `malloc` means different things in the host and the client
   (this occurs [on mac by default](https://gist.github.com/alexcrichton/8638ac35a79834adcc57), and [on linux](https://gist.github.com/alexcrichton/781eb2f958150a890dd3) with the
   `DEEPBIND` flag).
...
by the host). The [jemalloc documentation](http://www.canonware.com/download/jemalloc/jemalloc-latest/doc/jemalloc.html) isn’t 100% explicit
...
place. But if you _are_ sure, then it should be possible.)

**UPDATE:** I’ve been told that on Windows, freeing across DLL
boundaries is something you can never do. On Reddit,
[Mr\_Alert writes](http://www.reddit.com/r/rust/comments/2mcew2/allocators_in_rust_from_nmatsakiss_blog/cm35d8f): “In Windows, allocating memory in one DLL and
...
the allocator. So, it would probably be best to link rust\_alloc to
...
[this gist](https://gist.github.com/alexcrichton/41c6aad500e56f49abda), would be to have Rust call neither the standard
...
[strcat’s PR #18915](https://github.com/rust-lang/rust/pull/18915), which ensures that Rust executables can
...
this area. I’ve talked things over with [strcat](https://github.com/thestinger) a fair
...
is time to experiment and measure. One thing that _is_ clear to me is
