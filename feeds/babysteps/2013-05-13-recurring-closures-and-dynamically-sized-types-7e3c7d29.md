...
url: https://smallcultfollowing.com/babysteps/blog/2013/05/13/recurring-closures-and-dynamically-sized-types/?utm_source=atom_feed
published: "2013-05-13T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2013/05/13/recurring-closures-and-dynamically-sized-types/
---

# Recurring closures and dynamically sized types

I realized today that there is an unfortunate interaction between the
proposal for [dynamically sized types](https://smallcultfollowing.com/babysteps/
/blog/2013/04/30/dynamically-sized-types/) and closure types. In
particular, in the [case of the recurring closure](https://smallcultfollowing.com/babysteps/
/blog/2013/04/30/the-case-of-the-recurring-closure/), I described
...
not play well with the proposal for [dynamically sized types](https://smallcultfollowing.com/babysteps/
/blog/2013/04/30/dynamically-sized-types/),
because under this proposal all things written `&T` must behave the
same, no matter what `T` is. This is in fact _the whole point_ of the
...
1. Instead of writing `&fn()` you could write `&mut fn()`. This is
   ...
   non-starter. Writing `&fn()` for a closure is…tolerable, but `&mut fn()` is not. It’s verbose and it seems sort of nonsensical (although
...
2. We go back to the older notation and move sigils for closures
   _after_ the fn. This actually has some notational perks. For example,
...
5. “Const functions”: functions that do not close over mutable state.
   ...
   achieve lightweight fork-join parallelism a la [PJS](https://smallcultfollowing.com/babysteps/
   /blog/categories/pjs).
...
1. “Sendable const functions”: functions that can be sent between tasks
   _and_ do not close over mutable state. You could safely share such
   functions between tasks in an ARC (atomically referenced counted
   container) and execute them multiple times in parallel.
2. “Combinators”: combinator libraries create _and return_ closures that
...
[PJS](https://smallcultfollowing.com/babysteps/
/blog/categories/pjs) and described in [this HotPar paper I wrote](https://www.usenix.org/conference/hotpar12/parallel-closures-new-twist-old-idea).
...
is a [parser combinator](http://en.wikipedia.org/wiki/Parser_combinator), which is a simple way to create
...
_Anyway,_ a typical parser combinator library would begin with a primitive
like the following:
...
If we were to extend the example to include an actual _combinator_,
...
main goals of the [dynamically sized types (DST)](https://smallcultfollowing.com/babysteps/
/blog/2013/04/30/dynamically-sized-types/) proposal was to
