...
url: https://smallcultfollowing.com/babysteps/blog/2017/03/17/the-lane-table-algorithm/?utm_source=atom_feed
published: "2017-03-17T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2017/03/17/the-lane-table-algorithm/
---
...
- the full LR(1) algorithm gives pretty intuitive results but produces
  ...
  generate and process even the [full Rust grammar](https://github.com/nikomatsakis/rustypop) – but this
...
The [Lane Table algorithm](http://cssauh.com/xc/pub/LaneTable_APPLC12.pdf) published by Pager and Chen at APPLC
...
_when possible_ and gracefully scale up to the full LR(1) state
machine _as needed_.

I found the approach appealing, as it seemed fairly simple, and also
seemed to match what I would try to do
intuitively. [I’ve been experimenting with the Lane Table algorithm in LALRPOP and I now have a simple prototype that seems to work.](https://github.com/nikomatsakis/lalrpop/blob/27e154f07230ee35268bd014a6bc499d840c80b2/lalrpop/src/lr1/lane_table/README.md)
...
_can_, since it augments LR(0) with a token of lookahead; using that,
...
read up separately. Basically, we will enumerate a number of different _states_
...
moves”, since these new possibilities arise from consuming _no_ input,
...
an `X`, or a `Y`. (The _nonterminals_ `X` and `Y` can “come next” once
...
`Y`, we are at the beginning of _another_ `X` or `Y`:
...
it contains some items where the `(*)` comes at the very end, like `X = "e" (*)`. What this means is that we have seen an `"e"` in the
...
transform the state to add some _conditions_, and then it is clear
what to do:
...
We’ll call this augmented state a LR(0-1) state (it’s not _quite_
...
information contributed by each state. Note that for the _shift_ the
...
illustrate with conflict C1. We have the conflicting item `X = "e" (*)`, and we are basically looking to find its lookahead. We know
...
S0 and also S1. So we can push two states in our search: `(S0, X = (*) "e")` and `(S1, X 5B= (*) "e")`. Let’s consider each in turn.

The next state is then `(S0, X = (*) "e")`. Here the `(*)` lies at the
front of the item, so we search **the same state** S0 for items that
would have led to this state via an _epsilon move_. This basically
...
conflict C1. In some cases, we might find something like `Foo = ... (*) X` instead, where the `X` we are looking for appears at the
...
information. We need to know what is in the `...`, because `"a" "e" "c"` means we reduce `"e"` to `Y` and `"b" "e" "c"` means we reduce to
`X`. In terms of our _state machine_, this corresponds to _splitting_
...
found, _then_ we search for one with a mergable context. If _that_
