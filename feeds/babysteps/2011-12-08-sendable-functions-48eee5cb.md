...
url: https://smallcultfollowing.com/babysteps/blog/2011/12/08/sendable-functions/?utm_source=atom_feed
published: "2011-12-08T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2011/12/08/sendable-functions/
---

# Sendable functions

One of the last remaining tasks for Rust 0.1 is to find a way to
address issues # [1128](https://github.com/graydon/rust/issues/1128) and # [1038](https://github.com/graydon/rust/issues/1038). The key problem is
that, right now, we can only spawn a task with a _bare function_,
which is to say a function that carries no closure or environment.
Due to the way that Rust is implemented, this even excludes _generic_
_functions_. I have been wanting to lift this restriction but have
...
system, which divides data amongst the _exchange heap_, _task-local_
_heap_ (confusingly called _shared_), and the _stack_. The three
function types are:

- `fn[send](A*) -> R`, pronounced _sendable closure_
- `fn(A*) -> R` pronounced _task-local closure_
- `block(A*) -> R` pronouned _blocks_ (or _stack closure_)

The final function type is `fn[bare](A*)->R`, pronounced _bare_
_function_. A bare function is a top-level function that closes over
...
Here is Marijn’s example from bug [#1128](https://github.com/graydon/rust/issues/1128), written for this
proposal:
...
It is unclear to me whether there is _ever_ a need for bare function
