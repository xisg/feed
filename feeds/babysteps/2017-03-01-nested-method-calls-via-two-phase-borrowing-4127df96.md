...
url: https://smallcultfollowing.com/babysteps/blog/2017/03/01/nested-method-calls-via-two-phase-borrowing/?utm_source=atom_feed
published: "2017-03-01T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2017/03/01/nested-method-calls-via-two-phase-borrowing/
---

# Nested method calls via two-phase borrowing

In my previous post, I
[outlined a plan for non-lexical lifetimes](https://smallcultfollowing.com/babysteps/
/blog/2017/02/21/non-lexical-lifetimes-using-liveness-and-location/). I wanted to write a
...
[recent discussion on internals](https://internals.rust-lang.org/t/accepting-nested-method-calls-with-an-mut-self-receiver/4588), although what I describe
...
_This section is partially copied from the internals post; if you’ve read_
_that, feel free to skip or skim._
...
part of the [2017 Roadmap](https://github.com/rust-lang/rfcs/blob/master/text/1774-roadmap-2017.md).
...
(and indeed the [original post on the internals thread](https://internals.rust-lang.org/t/accepting-nested-method-calls-with-an-mut-self-receiver/4588)
...
So, in terms of our example, when we execute the MIR statement `tmp0 = &mut vec`, that creates a **reservation** on `vec`, but doesn’t start
...
We can _partially_ desugar the call to `push_str()` into MIR
...
[the `index_mut()` method](https://doc.rust-lang.org/std/ops/trait.IndexMut.html#tymethod.index_mut) has a signature like:
...
the comments, is that `i` is considered _reserved_ during the first
...
would be accepted (at least once we move to [NLL](https://smallcultfollowing.com/babysteps/
/blog/2017/02/21/non-lexical-lifetimes-using-liveness-and-location/)), since the borrow
...
- Lifetime check (infers the lifetimes for each reference, as described in [my previous post](https://smallcultfollowing.com/babysteps/
  /blog/2017/02/21/non-lexical-lifetimes-using-liveness-and-location/));
...
_reserved_. We can then do the next part of the check just as before,
...
the future”, [described in the internals thread](https://internals.rust-lang.org/t/accepting-nested-method-calls-with-an-mut-self-receiver/4588). The basic
...
[NLL rules](https://smallcultfollowing.com/babysteps/
/blog/2017/02/21/non-lexical-lifetimes-using-liveness-and-location/) that I presented previously. This is because it
allows us to keep the rule that when a reference is _live_ at some
...
have to make the [NLL rules](https://smallcultfollowing.com/babysteps/
/blog/2017/02/21/non-lexical-lifetimes-using-liveness-and-location/) that I outlined later incorporate a
...
was to define a type `RefMut<'r, 'w, T>` [1](#fn:1) which stands in for a
kind of “richer” `&mut` type.[2](#fn:2) In particular, it has two
lifetimes:
...
[NLL post](https://smallcultfollowing.com/babysteps/
/blog/2017/02/21/non-lexical-lifetimes-using-liveness-and-location/). But the `'w` lifetime only needs to include those
...
[Ralf Jung](https://www.ralfj.de/blog/). As you may know, Ralf is working on a
formalization of Rust as part of the [RustBelt project](http://plv.mpi-sws.org/rustbelt/) (if you’re
interested, there is video available of a
[great introduction to this work](https://air.mozilla.org/rust-paris-meetup-35-2017-01-19/) which Ralf gave at the Rust
...
that borrow has been activated – fits borrowck quite naturally.[3](#fn:3)
...
second out of a series of three posts that lay out a complete proposal[4](#fn:4):

- [the core NLL system](https://smallcultfollowing.com/babysteps/
  /blog/2017/02/21/non-lexical-lifetimes-using-liveness-and-location/), covered in the previous post;
- nested method calls, this post;
- incorporating dropck, still to come.

**Comments?** Let’s use [this internals thread for comments](https://internals.rust-lang.org/t/blog-post-nested-method-calls-via-two-phase-borrowing/4886).

### Footnotes

* * *

1. arielb1 called it `Ref2Φ<'immut, 'mutbl, T>`, but I’m going to take the liberty of renaming it. [↩︎](#fnref:1)

2. arielb1 also proposed to unify `&T` into this type, but that introduces complications because `&T` are `Copy` but `&mut` are not, so i’m leaving that out too. [↩︎](#fnref:2)

3. In more traditional compiler terminology,
...
forwards data-flow analysis. [↩︎](#fnref:3)

4. Presuming I’m not overlooking something. =)
\[ref2\]: [https://internals.rust-lang.org/t/accepting-nested-method-calls-with-an-mut-self-receiver/4588/24?u=nikomatsakis](https://internals.rust-lang.org/t/accepting-nested-method-calls-with-an-mut-self-receiver/4588/24?u=nikomatsakis)
\[wil\]: https://smallcultfollowing.com/babysteps/
/blog/2017/02/21/non-lexical-lifetimes-using-liveness-and-location/#step-0-what-is-a-lifetime [↩︎](#fnref:4)
