...
url: https://smallcultfollowing.com/babysteps/blog/2019/12/23/async-interview-3-carl-lerche/?utm_source=atom_feed
published: "2019-12-23T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2019/12/23/async-interview-3-carl-lerche/
---

# Async Interview #3: Carl Lerche

Hello! For the latest [async interview](http://smallcultfollowing.com/babysteps/blog/2019/12/09/async-interview-2-cramertj/), I spoke with Carl Lerche
( [carllerche](https://github.com/carllerche/)). Among many other crates[1](#fn:1), Carl is perhaps best
known as one of the key authors behind [tokio](https://github.com/tokio-rs/tokio) and [mio](https://github.com/tokio-rs/mio). These two
crates are quite widely used through the async ecosystem. Carl and I
spoke on December 3rd.

### Video

You can watch the [video](https://youtu.be/xpk0y8tfszE) on YouTube. I’ve also embedded a copy here
...
We started with the [mio](https://github.com/tokio-rs/mio) crate. [mio](https://github.com/tokio-rs/mio) is meant to be the “lightest
...
The [tokio](https://github.com/tokio-rs/tokio) crate builds on [mio](https://github.com/tokio-rs/mio). It wraps the epoll interface and
exposes it via the [`Future`](https://doc.rust-lang.org/std/future/trait.Future.html) abstraction from `std`. It also offers
other things that people commonly need, such as timers.

Finally, bulding atop tokio you find [tower](https://crates.io/crates/tower), which exposes a
“request-response” abstraction called [`Service`](https://docs.rs/tower/0.3.0/tower/trait.Service.html). [tower](https://crates.io/crates/tower) is similar
to things like [finagle](https://twitter.github.io/finagle/) or [rack](https://rack.github.io/). This is then used by libraries
like [hyper](https://crates.io/crates/hyper) and [tonic](https://crates.io/crates/tonic), which implement protocol servers (http for
[hyper](https://crates.io/crates/hyper), gRPC for [tonic](https://crates.io/crates/tonic)). These protocol servers internally use the
[tower](https://crates.io/crates/tower) abstractions as well, so you can tell hyper to execute any
[`Service`](https://docs.rs/tower/0.3.0/tower/trait.Service.html).

One challenge is that it is not yet clear how to adapt tower’s
[`Service`](https://docs.rs/tower/0.3.0/tower/trait.Service.html) trait to `std::Future`. It would really benefit from
support of async functions in traits, in particular, which [is\
difficult for a lot of reasons](http://smallcultfollowing.com/babysteps/blog/2019/10/26/async-fn-in-traits-are-hard/). The current plan is to adopt
[`Pin`](https://doc.rust-lang.org/std/pin/struct.Pin.html) and to require boxing and `dyn Future` values if you wish to
...
hyper, you find web frameworks, such as [warp](https://crates.io/crates/warp) – and (finally) within
...
(e.g., [Fuchsia](https://fuchsia.googlesource.com/)) as well as different trade-offs (e.g., [async-std](https://async.rs/)).
...
As you may recall, cramertj and I [discussed the `Stream` trait](http://smallcultfollowing.com/babysteps/blog/2019/12/10/async-interview-2-cramertj-part-2/)
...
Carl’s motivation for a stable [`Stream`](https://docs.rs/futures/0.3.1/futures/stream/trait.Stream.html) is in part that he would like
to issue a stable tokio release, ideally in Q3 of 2020, and [`Stream`](https://docs.rs/futures/0.3.1/futures/stream/trait.Stream.html)
would be a part of that. If there is no [`Stream`](https://docs.rs/futures/0.3.1/futures/stream/trait.Stream.html) trait in the standard
libary, that complicates things.

One thing we _didn’t_ discuss, but which I personally would like to
understand better, is what sort of libraries and infrastructure might
benefit from a stabilized [`Stream`](https://docs.rs/futures/0.3.1/futures/stream/trait.Stream.html). For example, “data libraries” like
hyper mostly want a trait like [`AsyncRead`](https://docs.rs/futures/0.3.1/futures/io/trait.AsyncRead.html) to be stabilized.

## About async read

Next we discussed the [`AsyncRead`](https://docs.rs/futures/0.3.1/futures/io/trait.AsyncRead.html) trait a little, though not in great
...
example [proposed](https://github.com/tokio-rs/tokio/pull/1744#issuecomment-553575438) using a concrete wrapper struct instead, and adding
...
[proc-macro-hack](https://crates.io/crates/proc-macro-hack) to support these, so perhaps a good thing to focus
...
\[loom\]: [https://crates.io/crates/loom](https://crates.io/crates/loom) [↩︎](#fnref:1)
