...
url: https://smallcultfollowing.com/babysteps/blog/2017/10/21/chalk-meets-slg/?utm_source=atom_feed
published: "2017-10-21T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2017/10/21/chalk-meets-slg/
---
...
adapting the [SLG algorithm](https://link.springer.com/chapter/10.1007/3-540-48159-1_12), which is used in the
[XSB Prolog engine](http://xsb.sourceforge.net/). I recently
[opened a PR that adds this SLG-based solver as an alternative](https://github.com/rust-lang-nursery/chalk/pull/59),
...
[the current solver that I described in my previous post](http://smallcultfollowing.com/babysteps/blog/2017/09/12/tabling-handling-cyclic-queries-in-chalk/).
...
based on this paper
[“Efficient Top-Down Computation of Queries Under the Well-formed Semantics” by Chen, Swift, and Warren (JLP ‘95)](https://ac.els-cdn.com/0743106694000285/1-s2.0-0743106694000285-main.pdf?_tid=f8beb358-b642-11e7-b052-00000aacb35f&acdnat=1508578621_12290e1834d94c48d36219f58be6e87f),
...
[**guarantee termination**](#guaranteed-termination) [1](#fn:1). Unlike a
traditional Prolog solver, however, it natively incorporates
[**tabling**](#tabling) and has a strong story for
[**negative reasoning**](#negative-reasoning-and-the-well-founded-semantics). In the rest of the post, I will go into
...
will find **all possible answers** to any query[2](#fn:2). Moreover, like
...
“ambiguous” result (this is running on [my PR](https://github.com/rust-lang-nursery/chalk/pull/59)):
...
existential variable in the query (i.e., `T`) is `Lemon`.[3](#fn:3)
...
rustc could give a more precise error message here regarding _which_
...
an infinite set of answers. Of course we can have `T = Vinegar` and `T = Lemon`. And we can have `T = HotSauce<Vinegar>` and `T = HotSauce<Lemon>`. But we can also have `T = HotSauce<HotSauce<Lemon>>`.
Or, for the real hot-sauce enthusiast[4](#fn:4), we might have:
...
**approximate** the answer by introducing variables.[5](#fn:5) In this
...
The original answer is an _instance_ of this – that is, we can
substitute `U = HotSauce<HotSauce<Lemon>>` to recover it.

Now, when we introduce variables into answers like this, we lose some
precision. We can now only say that `exists<U> { T = HotSauce<HotSauce<U>> }` **might** be an answer, we can’t say for
sure. It’s a kind of “ambiguous” answer[6](#fn:6).

So let’s see it in action. If I invoke the SLG solver using a maximum
size of 3, I get the following:[7](#fn:7)
...
one[8](#fn:8). So e.g. trying to solve
...
time. Tabling came up in my [previous post](http://smallcultfollowing.com/babysteps/blog/2017/09/12/tabling-handling-cyclic-queries-in-chalk/), too, where I
...
The [paper about SLG that I pointed you at earlier](https://link.springer.com/chapter/10.1007/3-540-48159-1_12) describes an
...
query you are doing. This extension is not _too_ difficult, and we
...
This would assert that there is **no type** `T` for which `Vec<T>: Foo` is implemented. In the SLG solver, this is handled by creating a
...
not (because `T: !Foo` is false). Alternatively, we could say that `T: Bar` holds, but then `T: Foo` does not (because `T: !Bar` is
...
- Fallback clauses: If you normalize something like `<Vec<u32> as IntoIterator>::Item`,
  ...
  or the unnormalized form `<Vec<u32> as IntoIterator>::Item`. This is not _wrong_,
...
- [Efficient Top-Down Computation of Queries Under the Well-formed Semantics](https://ac.els-cdn.com/0743106694000285/1-s2.0-0743106694000285-main.pdf?_tid=f8beb358-b642-11e7-b052-00000aacb35f&acdnat=1508578621_12290e1834d94c48d36219f58be6e87f)
  - Chen, Swift, and Warren; JLP ‘95.
  - The specific solution strategy for SLG that I used.
- [A New Formulation of Tabled resolution With Delay](https://link.springer.com/chapter/10.1007/3-540-48159-1_12)
  - Swift; EPIA ‘99
...
- [OLD Resolution with Tabulation](https://www.researchgate.net/publication/220986525_OLD_Resolution_with_Tabulation)
  - Tamaki and Sato 86
  - Describes approximating subgoals.
- \[Radial Restraint\]\[“radial restraint”\]
  - Grosof and Swift; 2013
  - Describes approximating answers.
- [Scoping constructs in logic programming: Implementation problems and their solution](http://www.sciencedirect.com/science/article/pii/074310669500037K)
...
1. True confessions: I have never (personally) managed to make a non-trivial Prolog program terminate. I understand it can be done. Just not by me. [↩︎](#fnref:1)

2. Assuming termination. More on that later. [↩︎](#fnref:2)

3. Some might say that lemons are not, in fact, sweet. Well fooey. I’m not rewriting this blog post now, dang it. [↩︎](#fnref:3)

4. Try [this stuff](https://store.davesgourmet.com/ProductDetails.asp?ProductCode=DAIN), it’s for real. [↩︎](#fnref:4)

5. This technique is called \[“radial restraint”\] by its authors.
\[“radial restraint”\]: [http://www3.cs.stonybrook.edu/~tswift/webpapers/aaai-13.pdf](http://www3.cs.stonybrook.edu/~tswift/webpapers/aaai-13.pdf) [↩︎](#fnref:5)

6. In terms of the well-formed semantics that we’ll discuss later, its truth value is considered “unknown”. [↩︎](#fnref:6)

7. Actually, in the course of writing this blog post, I found I sometimes only see 5 answers, so YMMV. Some kind of bug I suppose. (Update: fixed it.) [↩︎](#fnref:7)

8. This technique is called \[“subgoal abstraction”\] by its authors.
\[“subgoal abstraction”\]: [http://www3.cs.stonybrook.edu/~tswift/webpapers/tocl-14.pdf](http://www3.cs.stonybrook.edu/~tswift/webpapers/tocl-14.pdf) [↩︎](#fnref:8)
