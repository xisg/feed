...
url: https://smallcultfollowing.com/babysteps/blog/2017/02/01/unsafe-code-and-shared-references/?utm_source=atom_feed
published: "2017-02-01T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2017/02/01/unsafe-code-and-shared-references/
---

# Unsafe code and shared references

In a previous post, I talked about a [proposed approach to drafting the\
unsafe code guidelines](https://smallcultfollowing.com/babysteps/
/blog/2017/01/22/assigning-blame-to-unsafe-code/). Specifically, I want to the approach of having
...
probably the right _formal_ way to express it is not as code but
...
roughly work (perhaps using an operationally-based model like [promises](https://github.com/nikomatsakis/rust-memory-model/issues/32) as a starting
point).

### How to use shared references wrong

In Rust, a shared reference is more than a pointer. It’s also a kind
of _promise_ to the type system. Specifically, when you create a
shared reference, the data that it refers to (“referent”) is
considered _borrowed_, which means that it is supposed to be
**immutable** (except for under an `UnsafeCell`) and **valid** so long
as the reference is in use. When you’re writing _safe Rust_, of
...
of these properties. For now, I’m going to focus on _mutating_
...
this can be justified if we say that the program encounters _undefined_
_behavior_, but how can we make that more precise?

### Instrumenting to detect failures

Earlier we mentioned that the key property of a shared reference is
that the borrowed memory will remain both _immutable_ and _valid_ for
...
`increment()` would dynamically fail as it attempted to execute `*q += 1`, because the memory was “read-locked”.
...
integer. Rather, when something gets borrowed, we have to freeze _all_
_the memory reachable from the point of borrow_. That turns out to be
...
of _eagerly locking_ all the memory that is reachable through a
reference, we might instead declare that the _compiler should not be_
_able to observe any writes_. Under this model, modifying the referent
...
_create_ a shared reference `r`, we can also read the current value of
...
Now, when we _read_ from a shared reference `r` that refers to the
memory address `m`, we can check that `LAST_WRITE[m] <= TIME_STAMP[r]`, which tells us that the memory has not been written
...
One of the premises of the [Tootsie Pop model](https://smallcultfollowing.com/babysteps/
/blog/2016/05/27/the-tootsie-pop-model-for-unsafe-code/) is that we can
leverage the fact that Rust separates _safe_ from _unsafe_ code to
...
[my previous post about trusting types](https://smallcultfollowing.com/babysteps/
/blog/2016/09/12/thoughts-on-trusting-types-and-unsafe-code/). Let’s assume that the
...
In this code, the author has loaded `*i` _before_ calling
...
previous post on [observational equivalence](https://smallcultfollowing.com/babysteps/
/blog/2016/10/02/observational-equivalence-and-unsafe-code/) – I would like to
...
moves the `let v = *i` line down to _after_ `increment()`, an
...
has not expired (in particular, even in a [NLL world](https://smallcultfollowing.com/babysteps/
/blog/2016/04/27/non-lexical-lifetimes-introduction/), its lifetime
...
_safe enough_ for the compiler to do the optimizations we would like
to do.
...
memory models (e.g., [this one](https://github.com/nikomatsakis/rust-memory-model/issues/30)), which also have to
...
not that familiar
with. [Robert Krebbers’s thesis formalizing the C standard](https://github.com/nikomatsakis/rust-memory-model/issues/33) is
...
detail). Projects like [CompCert](http://compcert.inria.fr) also leap to mind.)
