...
url: https://smallcultfollowing.com/babysteps/blog/2018/02/01/in-rust-ordinary-vectors-are-values/?utm_source=atom_feed
published: "2018-02-01T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2018/02/01/in-rust-ordinary-vectors-are-values/
---
...
- If the vector is not _actually_ aliased, and you _know_ that it’s
  ...
  Rust-based persistent collection library called [`DVec`](https://docs.rs/dogged/0.2.0/dogged/struct.DVec.html) which does
...
- There are tons of other designs for persistent collections, some of
  ...
  [this paper](https://www.lri.fr/~filliatr/ftp/publis/puf-wml07.pdf) has a design oriented specifically towards
...
If you’ve ever seen one of my talks on Rust[2](#fn:2), you’ll know that
...
vector library called [dogged](https://crates.io/crates/dogged) [3](#fn:3). Dogged offers a vector type
called [`DVec`](https://docs.rs/dogged/0.2.0/dogged/struct.DVec.html), which is based on the
[persistent vectors offered by Clojure](http://hypirion.com/musings/understanding-persistent-vector-pt-1). But if you look at
the methods that [`DVec`](https://docs.rs/dogged/0.2.0/dogged/struct.DVec.html) offers, you’ll see they’re kind of the
...
Nonetheless, a `DVec` _is_ a persistent data structure. Under the
hood, a `DVec` is implemented as a [trie](https://en.wikipedia.org/wiki/Trie). It contains an [`Arc`](https://doc.rust-lang.org/std/sync/struct.Arc.html)
...
(As an aside, [`Arc::make_mut`](https://doc.rust-lang.org/std/sync/struct.Arc.html#method.make_mut) is a **really cool** method. It
...
recall how persistent data structures tend to work, this is _perfect_
for updating a tree as you walk. It lets you avoid cloning in the case
where your collection is not yet aliased.)

### But persistent collections _are_ different
...
a paper entitled [“Linear Types Can Change The World!”](http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.55.5439&rep=rep1&type=pdf) in
...
[ena unification library](https://crates.io/crates/ena) (which is based on a vector under the
hood) to act in either [persistent mode](https://docs.rs/ena/0.8.0/src/ena/unify/mod.rs.html#188) (using `DVec`) or
[imperative mode](https://docs.rs/ena/0.8.0/src/ena/unify/mod.rs.html#185) (using `Vec`). Basically the idea is to be generic
...
interest in persistent collections for Rust _specifically_. That is,
while simultaneous sharing and mutation can be a risky pattern, it is
sometimes a necessary and _dang useful_ one, and Rust currently makes
it kind of unergonomic. **I do think we should do things to improve**
**this situation, and I have some specific thoughts** [4](#fn:4), but I
think that persistent vs imperative collections are kind of a
non-sequitor here. Put another way, Rust already _has_ persistent
...
1. As it happens, the SLG solver that I wrote about before seems like it would really like to use persistent collections. [↩︎](#fnref:1)

2. If you haven’t, I thought \[this one\] went pretty well.
\[this one\]: [https://www.sics.se/nicholas-matsakis](https://www.sics.se/nicholas-matsakis) [↩︎](#fnref:2)

3. In English, if you are “dogged” in pursuing your goals, you are persistent. [↩︎](#fnref:3)

4. Specific thoughts that will have to wait until the next blog post. Time to get my daughter up and ready for school! [↩︎](#fnref:4)
