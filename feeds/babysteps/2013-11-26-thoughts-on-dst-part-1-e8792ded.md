...
url: https://smallcultfollowing.com/babysteps/blog/2013/11/26/thoughts-on-dst-1/?utm_source=atom_feed
published: "2013-11-26T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2013/11/26/thoughts-on-dst-1/
---
...
[originated on Reddit](http://www.reddit.com/r/rust/comments/1r082g/meetingweekly20131119_static_linking_wildcards/). The basic idea is to have the builtin
vector notation `[T]` be used for _fixed-length vectors_, whereas
...
a pointer, length, _and_ capacity (hence `sizeof::<~[T]>()` would be 3
...
`exists N. [T, ..N]` – this is called an _existential type_ and it
...
`~[T]`, and `*[T]`) are all _fat pointers_, meaning that they are
...
latter is `((data, length), refcount)`. Still, we are _coercing_ the
...
_directly_ behind a `*` pointer, it couldn’t even be instantiated with
...
Thanks to inherited mutability, what would _work_ is to implement `Message`
for `~Message`:
...
self (modulo [issue 10672](https://github.com/mozilla/rust/issues/10672)).
