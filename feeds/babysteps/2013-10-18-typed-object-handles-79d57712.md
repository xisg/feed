...
url: https://smallcultfollowing.com/babysteps/blog/2013/10/18/typed-object-handles/?utm_source=atom_feed
published: "2013-10-18T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2013/10/18/typed-object-handles/
---

# Typed object handles

Yesterday [Dmitry Lomov](https://twitter.com/mulambda) and I had a discussion about the typed objects
API. Much of the discussion revolved around the specific issue of
_handles_. In this post I will summarize the issues we discussed and
review the various design options.

I’ll begin with a summary of what handles are and how they are used in
current APIs; if this is familiar to you ( _cough_ [Dave Herman](https://twitter.com/littlecalculist) _cough_)
...
of _movable pointer_ that can be used to point into arbitrary data
...
_Note:_ the [wiki](http://wiki.ecmascript.org/doku.php?id=harmony:typed_objects) specifies that handles are created using `new Handle(PointType, aLine, "from")` instead of `PointType.handle(aLine, "from")`; my branch differs from this, the distinction is unimportant
...
we can _move_ a handle to point somewhere else, such as the `to`
coordinate:
...
One of the primary uses for handles is to serve as an _out pointer_ –
you may recall the idea from this
[previous post about integrating typed objects with PJS](https://smallcultfollowing.com/babysteps/
/blog/2013/05/29/integrating-binary-data-and-pjs), but in
...
`build` API provide an _out pointer_ – that is, a pointer into the
...
_Side note on optimization:_ You might object that writing the
function using `Handle.set` _still_ allocates a temporary object (the
...
This is _almost_ the definition we’ve arrived at with PJS. The one
difference is that we found that in some cases, in particular when you
have an array of _scalar data_, using an out pointer is awkward.
...
pointer _optional_. The idea is that we examine the return value from
...
This definition will _almost_ work, but there are two important
...
me to write generic, uniform code that works _no matter what type of_
_array is being built_.

_Note:_ The fact that handles behave differently from typed objects
...
_or_ an out pointer, everything probably works out, and you can still
...
pointer _only allows_ the `func` callback access to the memory for the
...
pointer to element `22`, I _actually_ have access to the entire array.
...
_Note:_ I will argue later that in the _subtle points_ section that
...
by _anyone_, including the callback. This can be potentially
...
too different and too similar. That is, they behave similarly _enough_
...
reason for this is that it lets handles serve as _revocable_
...
One example use case for this that [dherman](https://twitter.com/littlecalculist) and I have tossed
...
this new typed object will be created _opaque_, meaning that it does
...
entire buffer. It is also useful to be able to _take it away again_.
...
**Avoiding allocation _is_ important.** It is tempting to say that
allocation can be made free. After all, we’re all using highly
optimized generational collectors, right? (ok, almost;
[go, terrence and jonco, go!](https://bugzilla.mozilla.org/show_bug.cgi?id=764882)). But I think this is ultimately
...
_lazilly_, meaning on first allocation. If we make handles
...
better parallel speedups](http://en.wikipedia.org/wiki/Amdahl%27s_law).
