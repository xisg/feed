...
url: https://smallcultfollowing.com/babysteps/blog/2012/04/23/on-types-and-type-schemes/?utm_source=atom_feed
published: "2012-04-23T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2012/04/23/on-types-and-type-schemes/
---

# On types and type schemes

After my recent dalliance in
[Matters of a Truly Trivial Nature](https://smallcultfollowing.com/babysteps/
/blog/2012/04/15/syntax-matters-dot-dot-dot/), I’d like to return to
...
indicates that the function type is _parameterized_ by the region `r`.
...
_monotypes_, meaning types that refer to exactly one thing. Now, it
may not be known precisely what that thing _is_, but there must be a
...
accommodate this concept, ML introduced the idea of a _type scheme_,
also called a polytype. (at least by [Wikipedia](http://en.wikipedia.org/wiki/Hindley%E2%80%93Milner), I’ve never
heard the term before. but it seems logical.)

A type scheme is basically a type along with a set of _bound type_
_variables_. A bound type variable is one that is defined within the
scheme itself. So, if you have the type scheme `fn<T>(T) -> T`, then
the variable `T` is said to be _bound_ in this scheme. In a scheme
like `fn<T>(T, U) -> T`, the variable `T` is bound, but the variable
`U` is called _free_, as it is not defined in the scheme. Note that
...
`C=fn(&r)`. Here, the region variable `r` is _not_ bound but rather
free. So in this case, is `A` a subtype of `C`? I would argue that
the answer _should be_ yes: after all, if you _instantiate_ `A` with
...
We can still handle this case, but it requires a _region variable_ as well.
...
Actually, I think we definitely _could_ (not yet sure if we _should_).
...
a blog post Real Soon Now, but if you’re curious about _that_ you may
also want to read my [mailing list post](https://mail.mozilla.org/pipermail/rust-dev/2011-December/001036.html) on the topic as it is
