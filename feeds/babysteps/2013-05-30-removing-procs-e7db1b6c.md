...
url: https://smallcultfollowing.com/babysteps/blog/2013/05/30/removing-procs/?utm_source=atom_feed
published: "2013-05-30T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2013/05/30/removing-procs/
---
...
it references them; as I [argued before](http://smallcultfollowing.com/babysteps/blog/2013/05/14/procedures/), I find this potentially
confusing). This task then pushes the result of `computation()` onto
the vector and sends the vector back.

Interestingly, it has to send back a _clone_ of the vector, even
...
This closure, `body`, defines the task body. _We_ know that the closure
...
that has [already been requested for other reasons](https://github.com/mozilla/rust/issues/3678#issuecomment-16092916) and which
I plan to do.
...
of `once fn` s. The _main_ use of those was task bodies and futures,
...
and `&mut` borrowed pointers, but it is a _very_ sweet sugar that we
