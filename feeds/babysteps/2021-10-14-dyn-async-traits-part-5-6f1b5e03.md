...
url: https://smallcultfollowing.com/babysteps/blog/2021/10/14/dyn-async-traits-part-5/?utm_source=atom_feed
published: "2021-10-14T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2021/10/14/dyn-async-traits-part-5/
---
...
**Just to be crystal clear:** Nothing in this post is intended to describe an “ideal end state” for async functions in traits. I still want to get to the point where one can write `async fn` in a trait without any further annotation and have the trait be “fully capable” (support both static dispatch and dyn mode while adhering to the tenets of zero-cost abstractions[1](#fn:1)). But there are some significant questions there, and to find the best answers for those questions, we need to enable more exploration, which is the point of this post.
...
- Given a `T: AsyncIter`, we can coerce it into some form of `DynAsyncIter` that uses virtual dispatch. In this case, the type doesn’t reveal the specific `T` or the specific types of the futures.
  ...
  - I wrote `DynAsyncIter`, and not `dyn AsyncIter` on purpose — we are going to create our own type that acts _like_ a `dyn` type, but which manages the adaptations needed for async.
...
You could write the code I’m showing here by hand, but the better route would be to package it up as a kind of decorator (e.g., `#[async_trait_v2]` [2](#fn:2)).
...
Anyway, I know people have done some pretty clever tricks, so I’d be curious to know if I’m missing something and there _is_ a way to build this vtable on Rust today. Regardless, it seems like extending `const` and a few other things to support this case is a relatively light lift, if we wanted to do that.
...
1. In the words of Bjarne Stroustroup, “What you don’t use, you don’t pay for. And further: What you do use, you couldn’t hand code any better.” [↩︎](#fnref:1)

2. Egads, I need a snazzier name than that! [↩︎](#fnref:2)
