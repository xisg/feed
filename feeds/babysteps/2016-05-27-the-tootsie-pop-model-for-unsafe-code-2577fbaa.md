...
url: https://smallcultfollowing.com/babysteps/blog/2016/05/27/the-tootsie-pop-model-for-unsafe-code/?utm_source=atom_feed
published: "2016-05-27T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2016/05/27/the-tootsie-pop-model-for-unsafe-code/
---

# The 'Tootsie Pop' model for unsafe code

In my [previous post](http://smallcultfollowing.com/babysteps/blog/2016/05/23/unsafe-abstractions/), I spent some time talking about the idea of
...
at guidelines which are “human friendly”. Even if we _have_ published
...
abstraction is kind of like a [Tootsie Pop](https://en.wikipedia.org/wiki/Tootsie_Pop). There is a gooey candy
...
working on making that model more precise as part of the [Rust Belt](http://plv.mpi-sws.org/rustbelt/)
...
[type-based alias analysis](http://www.drdobbs.com/cpp/type-based-alias-analysis/184404273); we would not).
...
By the way, I have been [collecting these sorts of examples](https://github.com/nikomatsakis/rust-memory-model/) in a
...
`split_at_mut` method that I covered in [the previous post](http://smallcultfollowing.com/babysteps/blog/2016/05/23/unsafe-abstractions/):
...
at a safe implementation (also in the [previous post](http://smallcultfollowing.com/babysteps/blog/2016/05/23/unsafe-abstractions/)). The only
...
normal Rust rules. When optimizing, then, the compiler will _not_
...
think that we can model this using the [aliasing metadata](http://llvm.org/docs/LangRef.html#noalias-and-alias-scope-metadata) that LLVM
...
perform. For example, under this model, _manually_ inlining a fn body
...
am talking about _inlining by the user_. Inlining by the compiler is
...
employ any [type-based alias analysis](http://www.drdobbs.com/cpp/type-based-alias-analysis/184404273), though).
...
**might be nicer if we could have a way to _declare_ that boundary**
...
[this thread on the Rust internals forum](http://internals.rust-lang.org/t/tootsie-pop-model-for-unsafe-code/3522).
