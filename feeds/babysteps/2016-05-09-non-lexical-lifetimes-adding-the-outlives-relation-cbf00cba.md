...
url: https://smallcultfollowing.com/babysteps/blog/2016/05/09/non-lexical-lifetimes-adding-the-outlives-relation/?utm_source=atom_feed
published: "2016-05-09T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2016/05/09/non-lexical-lifetimes-adding-the-outlives-relation/
---

# Non-lexical lifetimes: adding the outlives relation

This is the third post in my
[series on non-lexical lifetimes](http://smallcultfollowing.com/babysteps/blog/categories/nll/). Here I want to dive into
...
continuous lifetimes proposed as part of [RFC 396](https://github.com/rust-lang/rfcs/pull/396).
...
some better intuition for what a _named lifetime parameter_ represents,
...
Note that, for this fragment to type-check, `'m1` must _exclude_ the
...
At this point, it’s worth revisiting [RFC 396](https://github.com/rust-lang/rfcs/pull/396). RFC 396 was based on
...
Here the lifetime `'m1` would be a set containing _at least_ {D, E, I}, because the value in
...
In my [previous post](http://smallcultfollowing.com/babysteps/blog/2016/05/04/non-lexical-lifetimes-based-on-liveness/), I defined a lifetime as simply a set of
...
basically means that `'a` corresponds to something _at least as long_
_as_ `'b` (note that the outlives relation, like many other relations
...
In English, you can approximate a mutable reference of type `&'a mut T` to a mutable reference of type `&'b mut T` so long as the new
...
account the _point P in the control-flow graph where it must hold_. So
...
an explanation below of how it works listed under _appendix_):
...
this in turn implies the lifetime `'m1` would be `{A/3, A/4, B/*, C/3, C/4, D/0}`. Note that this includes `C/3`, precisely where we _would_
...
that `'a` must outlive `'b` _at the point P_. We can define this
relation as follows:
...
to support _gaps_ in lifetimes. In all the examples in this post, the
...
_paths_ through the control-flow graph, in which case the points after
...
interact as well with the notion of _fragments_ that I talked about in
[the previous post](http://smallcultfollowing.com/babysteps/blog/2016/05/04/non-lexical-lifetimes-based-on-liveness/) (though one can use variants of SSA that
