...
url: https://smallcultfollowing.com/babysteps/blog/2016/02/19/parallel-iterators-part-1-foundations/?utm_source=atom_feed
published: "2016-02-19T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2016/02/19/parallel-iterators-part-1-foundations/
---

# Parallel Iterators Part 1: Foundations

Since [giving a talk about Rayon at the Bay Area Rust meetup](https://air.mozilla.org/bay-area-rust-meetup-january-2016/),
I’ve been working off and on on the support for _parallel_
_iterators_. The basic idea of a parallel iterator is that I should be
...
Rust’s _sequential_ iterators work. The basic idea is that iterators
are lazy, in the sense that constructing an iterator chain does not
actually _do_ anything until you “execute” that iterator, either with
a `for` loop or with a method like `sum`. In the example above, that
means that the chain `vec1.iter().zip(...).map(...)` are all
operations that just build up a iterator, without actually _doing_
anything. Only when we call `sum` do we start actually doing work.

In sequential iterators, the key to this is
[the `Iterator` trait](http://doc.rust-lang.org/std/iter/trait.Iterator.html). This trait is actually very simple; it
...
_slice_ is a subportion of a vector or array.) But the `iter()` method
...
[`std::slice::Iter`](http://doc.rust-lang.org/std/iter/trait.Iterator.html), though it’s implemented somewhat
...
_lifetime_ of the slice, meaning the span of the code where that
...
about `zip`? The [zip iterator](http://doc.rust-lang.org/std/iter/trait.Iterator.html#method.zip) is an adapter that takes two
...
fashion (notice I said _sequential_; we’ll have to adjust this later
...
straightforward. The [actual `sum` method](http://doc.rust-lang.org/std/iter/trait.Iterator.html#method.sum) is designed to work over any
...
elements. However, converting a _reference_ to a vector (e.g.,
\`\`&Vec`) gives back *references* to the elements `&T `. Similarly, converting a borrowed slice like `&\[T\] ` into an iterator also gives back references to the elements (`&T `). We can implement ` IntoIterator `for`&\[T\]\` like so:
...
_happens_ until we call `sum`. That is, you might expect that calling
...
fact, no vector of pairs is _ever_ created (unless you ask for one by
calling `collect`). Thus iteration can be described as _lazy_, since
...
_precisely_ what it is going to do. The compiler will then generate a
