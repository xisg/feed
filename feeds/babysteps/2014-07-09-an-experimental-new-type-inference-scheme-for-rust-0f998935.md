...
url: https://smallcultfollowing.com/babysteps/blog/2014/07/09/an-experimental-new-type-inference-scheme-for-rust/?utm_source=atom_feed
published: "2014-07-09T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2014/07/09/an-experimental-new-type-inference-scheme-for-rust/
---
...
_Note:_ Changing the type inference scheme doesn’t really mean much to
...
when either `$0` or `$X` _is_ set to some specific type `T`, we can go
...
about for us compiler engineers. Every type variable is either _set_
– in which case its type is known precisely – or _unset_ – in which
...
_one-way matching_ as part of trait resolution. One-way matching is
basically a way of saying: is there any substitution `S` such that `T <: S(U)` (whereas normal matching searches for a substitution applied
...
in `T` or `U` which are partially constrained, we only know _bounds_
...
actually tells us that the type variable _is_ `int`, since in Rust’s
...
amounts to precisely the same thing as this _new_ inference scheme.
...
_subtype_ of the function type `fn(&'b T) -> &'b uint`. While these
two function types look similar, they are quite different: the former
accepts a reference with _any_ lifetime but the latter accepts only a
