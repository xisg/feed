...
url: https://smallcultfollowing.com/babysteps/blog/2011/12/16/implementing-unique-closures/?utm_source=atom_feed
published: "2011-12-16T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2011/12/16/implementing-unique-closures/
---
...
Now, this closure is going to result in a bound\_data that is _itself_ generic!
...
one set of code that works for _any value of `T`_. In other words, we know that
...
There are two changes of note. First, the `closure_td` represents the _entire_
_closure_ and not just the bound data. Second, the set of bound type descriptors
is stored at a _statically known offset_, regardless of what data is closed over.
What’s more, while it may not be obvious at first, the offset of the bound\_data
