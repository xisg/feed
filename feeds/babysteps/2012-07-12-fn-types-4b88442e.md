---
title: Fn types
url: https://smallcultfollowing.com/babysteps/blog/2012/07/12/fn-types/?utm_source=atom_feed
published: "2012-07-12T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2012/07/12/fn-types/
---
...
type `@fn(S) -> T` or `~fn(S) -> T` can be “borrowed” into a `&fn(S) -> T`. (Today, there is a subtyping relationship)
...
Borrowed closures in their most explicit form are written `&r/fn(S) -> T`. They are like `fn()` today but they are associated with a region
...
Including an explicit region also _enables_ a lot of very useful
...
Unique closures in their most explicit form are written like `~fn:K(S) -> T`, where `K` is a set of bounds like those found on type
...
The representation of function pointers will remain a pair `(code, env)` of pointers. This is not as pure as my prior proposals but has
...
we’d _still_ need the `owned` kind, or else we’d need to be able to
...
### Goals that _might_ be achieved by this proposal

**Moving within closures.** Actually I only _think_ this can be done
...
while some upvars are uninitialized. I _think_ regions give us the
...
### Goals that are _not_ achieved by this proposal
