...
url: https://smallcultfollowing.com/babysteps/blog/2018/01/31/an-on-demand-slg-solver-for-chalk/?utm_source=atom_feed
published: "2018-01-31T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2018/01/31/an-on-demand-slg-solver-for-chalk/
---
...
[“Efficient top-down computation of queries under the well-founded semantics”, by W. Chen, T. Swift, and D. Warren](http://www.sciencedirect.com/science/article/pii/0743106694000285). It
...
one, with a lot of influence from [MiniKanren](http://minikanren.org/). I think it’s getting
...
As part of the [PR](https://github.com/rust-lang-nursery/chalk/pull/76), I wrote up a [README](https://github.com/nikomatsakis/chalk-ndm/blob/64964db637c1ea63ecb0234326f9b57b3a9e55cb/src/solve/slg/on_demand/README.md) that tries to walk
...
type. A _forest_ stores a collection of _tables_ as well as a
_stack_. Each _table_ represents the stored results of a particular
query that is being performed, as well as the various _strands_, which
...
Now imagine that we want to find answers for the query `exists<T> { Rc<T>: Debug }`. The first step would be to u-canonicalize this query; this
...
it with a set of _initial strands_. A “strand” is kind of like a
...
(i.e., a “domain goal”) is determined by looking for _clauses_ in the
...
**What is a strand?** Let’s talk a bit more about what a strand _is_. In the code, a strand
...
For each strand, we also optionally store a _selected subgoal_. This
...
of _answers_ for our table, calling it answer A0 (it is the first
...
at this point (remember, we only do enough work to generate _one_
...
created thus far _and_ the strands that will let us try to produce
...
_internally_ has to go through quite a few answers in order to produce
...
Under the setup described here, one of these queries – let’s say `T: Bar` – gets chosen somewhat arbitrary to begin producing answers
...
open-ended impls – and prefer not to select them first. I _suspect_ a few
...
[I’ve created an internals thread](https://internals.rust-lang.org/t/blog-post-an-on-demand-slg-solver-for-chalk/6676), please leave comments
...
1. The existing implementation could do better here, but the ingredients are there. [↩︎](#fnref:1)
