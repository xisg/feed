...
url: https://smallcultfollowing.com/babysteps/blog/2020/04/09/libraryification/?utm_source=atom_feed
published: "2020-04-09T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2020/04/09/libraryification/
---
...
that “layer on” to rustc. I am thinking of projects like [Prusti](https://www.pm.inf.ethz.ch/research/prusti.html) or
Facebook’s [MIRAI](https://github.com/facebookexperimental/MIRAI), or even the venerable [Clippy](https://github.com/rust-lang/rust-clippy). All of these
...
[Prusti](https://www.pm.inf.ethz.ch/research/prusti.html), for example, lets you add pre- and post-conditions to your
...
initially for other languages to analyze Rust. [Prusti](https://www.pm.inf.ethz.ch/research/prusti.html), for example,
is adapting an existing project called [Viper](https://www.pm.inf.ethz.ch/research/viper.html), which was built to
analyze languages like C# or Java. However, actually analyzing
programs written in C# or Java _in practice_ is often quite difficult,
...
of aliasing information that the borrow check gathers. [Prusti](https://www.pm.inf.ethz.ch/research/prusti.html), for
...
to construct MIR. They should then be able to access the [MIR](https://rustc-dev-guide.rust-lang.org/mir/index.html) and the
...
turn invoke other queries, and thus execution begins from the _end_
_state_ that we want to reach (“give me an executable”) and works its
way _backwards_ to the first few steps (“parse this file”). This winds
up [fitting quite nicely with incremental computation](https://blog.rust-lang.org/2016/09/08/incremental.html) as
well as parallel execution. (If you’d like to learn more about this, I
gave a talk at [PLISS](https://pliss.org/) that is [available on YouTube](https://www.youtube.com/watch?v=N6b44kMS6OM).)
...
computation, using the [salsa](https://github.com/salsa-rs/salsa) library.

### On-demand analysis is a good fit for analysis tools

On-demand analysis is not only a good fit for IDEs: it’d be a great
fit for tools like [Prusti](https://www.pm.inf.ethz.ch/research/prusti.html). If we had a reasonably stable API, tools
like [Prusti](https://www.pm.inf.ethz.ch/research/prusti.html) could use on-demand analysis to ask for just the results
...
results that [Prusti](https://www.pm.inf.ethz.ch/research/prusti.html) computed when doing its analysis).
...
to refactor the compiler into a set of _independent libraries_, all
...
its trait system. And, of course, work is [also underway](https://blog.rust-lang.org/inside-rust/2020/03/28/traits-sprint-1.html) to integrate
chalk into rustc.
...
the [`RustIrDatabase`](http://rust-lang.github.io/chalk/chalk_solve/trait.RustIrDatabase.html), that allows it to query for specific bits of
information about the Rust source (e.g., [“tell me about this\
impl”](http://rust-lang.github.io/chalk/chalk_solve/trait.RustIrDatabase.html#tymethod.impl_datum)), and doesn’t require a full AST or lots of
...
unit tests](https://github.com/rust-lang/chalk/blob/73a74be3bc1d0cdef3f76fa529a112a0d8367ddb/tests/test/impls.rs#L9-L22) in a simplified form of Rust syntax.
...
then starts to look sort of like the [.NET Roslyn compiler](https://github.com/dotnet/roslyn) – we are
exporting the tools to help people analyze and understand Rust code
for themselves. So, for example, [Prusti](https://www.pm.inf.ethz.ch/research/prusti.html) could invoke rustc’s borrow
