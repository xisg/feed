...
url: https://smallcultfollowing.com/babysteps/blog/2020/02/11/async-interview-6-eliza-weisman/?utm_source=atom_feed
published: "2020-02-11T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2020/02/11/async-interview-6-eliza-weisman/
---

# Async Interview #6: Eliza Weisman

Hello! For the latest [async interview](http://smallcultfollowing.com/babysteps/blog/2019/11/22/announcing-the-async-interviews/), I spoke with Eliza Weisman
( [hawkw](https://github.com/hawkw/), [mycoliza on twitter](https://twitter.com/mycoliza)). Eliza first came to my attention as the author of the
[tracing](https://crates.io/crates/tracing) crate, which is a nifty crate for doing application level
tracing. However, she is also a core maintainer of tokio, and she
works at Buoyant on the [linkerd](https://linkerd.io/) system. [linkerd](https://linkerd.io/) is one of a small
...
You can watch the [video](https://youtu.be/bCf9K28TqVQ) on YouTube. I’ve also embedded a copy here
...
has reached the point where there is nothing _critically missing_.
...
We talked some about what [tokio](https://tokio.rs/) needs from async Rust. As Eliza
...
- async functions in traits would be great, but [they’re hard](http://smallcultfollowing.com/babysteps/blog/2019/10/26/async-fn-in-traits-are-hard/)
...
semver tells you _when a breaking change has been made_, of course,
but it doesn’t tell _whether a breaking change will be made in the_
_future_ – or how long we plan to do backports, and the like.
...
[maintain a stable interface for the next 5 years](https://tokio.rs/blog/2019-11-tokio-0-2/#tokio-1-0-in-q3-2020-with-lts-support), and so if
...
really useful. My go to example for this is the [http](https://crates.io/crates/http) crate, which
...
representing the details of a HTTP request. Maybe the [http](https://crates.io/crates/http) crate is
...
We talked a fair amount about the [tracing](https://crates.io/crates/tracing) library. Tracing is one of
...
crates for collecting _scoped, structured, and contextual diagnostic_
_information_ in Rust programs. One of the simplest use cases is to
...
The [tracing](https://crates.io/crates/tracing) library got its start as a structured replacement for
various hacky layers atop the `log` crate that were in use for
debugging [linkerd](https://linkerd.io/). Like many async applications, debugging a
[linkerd](https://linkerd.io/) session involves correlating a lot of events that may be
taking place at distinct times – or even distinct _machines_ – but
are still part of one conceptual “thread” of control.

[tracing](https://crates.io/crates/tracing) is actually a “front-end” built atop the “tracing-core”
...
collectevents and log them later. Crates like [tracing-timing](https://crates.io/crates/tracing-timing) record
...
With the tracing crate, the goal is that libraries can _enrich_ the
...
interoperable with the [log](https://crates.io/crates/log) crate (there is also a “logger” that
...
infrastructure. (rayon uses the same trick; the [rayon-core](https://crates.io/crates/rayon-core) crate
...
As [explained in the tracing README](https://github.com/tokio-rs/tracing#in-asynchronous-code), the problem is that if an
...
described here](https://boats.gitlab.io/blog/post/poll-drop/)).
...
which [didn’t integrate well with rust borrows](http://aturon.github.io/tech/2018/04/24/async-borrowing/).
