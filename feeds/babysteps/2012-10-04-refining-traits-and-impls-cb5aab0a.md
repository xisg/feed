...
url: https://smallcultfollowing.com/babysteps/blog/2012/10/04/refining-traits-slash-impls/?utm_source=atom_feed
published: "2012-10-04T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2012/10/04/refining-traits-slash-impls/
---
...
( _An aside:_ I just love saying that phrase for some perverse reason.
...
had relevance in the modern era. The [Swiss Einzahlungschein](https://www.postfinance.ch/en/biz/prod/pay/debsolution/inpay/apply.html) truly
...
- _Coherent and unambiguous:_ There is at most one impl of a given trait for a given type.
- _Safe linking:_ Linking in a new crate cannot cause a static error
  nor violate any of the above properties.
- _Overloading- and backtracking-free:_ Given an expression
...
- _Termination:_ Compilation always terminates.
...
_Aside from the coherence requirement, the main restriction is that a_
_given type can only implement or extend a given trait once (and hence_
_with one set of type parameters)._
...
[“Type classes: an exploration of the design space”](http://research.microsoft.com/en-us/um/people/simonpj/papers/type-class-design-space/), Jones et
al. discuss several uses for multiparameter type classes which they
find compelling. If I am not mistaken, _all of those examples_ are
...
1. You can only provide one implementation of any particular trait for
   ...
   In other words, the following set of impls _would be illegal_:
...
2. The rules I gave permit an impl such as `impl<A> Foo: Bar<A>`. In
   ...
   `A`. I don’t _think_ this has any ill effects, but it does make me
...
[these papers](http://research.microsoft.com/en-us/um/people/simonpj/papers/fd-chr/) to be sure, since they apparently discuss
the matter in more detail.

**UPDATE**: I discuss this analogy to Haskell further in the [next post](https://smallcultfollowing.com/babysteps/
/blog/2012/10/04/a-postscript-on-traits-and-impls/).

**UPDATE**: I am pretty sure that the termination condition is insufficient. More to come.

### What if I _want_ overloading?
...
current crate and (2) there are _no other impls of the trait `Bar`_.
...
of those patterns _is_ compatible with restriction #1 and the second
...
backtracking—even though unification succeeded, we don’t _know_ that
the impl applies.
...
[list of type class extensions in the Haskell’s user manual](http://www.haskell.org/ghc/docs/7.0.1/html/users_guide/type-class-extensions.html), I
