...
url: https://smallcultfollowing.com/babysteps/blog/2014/04/01/value-types-in-javascript/?utm_source=atom_feed
published: "2014-04-01T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2014/04/01/value-types-in-javascript/
---
...
[Brendan’s slides](http://www.slideshare.net/BrendanEich/value-objects) where he discusses value objects. This post
...
JavaScript has long had a division between _primitive values_ and
_objects_. These two things are fundamentally rather different.
Primitive values have no identity and no prototype. They are not
_allocated_, just used. They are also immutable. Consider an integer
like `var x = 1` – if I write `x += 1`, I haven’t _incremented_ the
...
_identity_. If I execute the same expression twice, I get two
_different objects_. This is why `{} === {}` evaluates to false
...
circumstances. What _is_ unfortunate is that JavaScript makes the set
...
places where a _value_ is fundamentally what you want and not an
_object_ – this makes programs easier to read and write and also
...
example of a type that really _wants_ to be a value. The fact that JS
...
_objects_ and thus when we compare with `===` we are comparing
for _object identity_ rather than representing the _same color_.
...
`rect2` to the _same color object_. So now if some other piece of code
...
1. Primitives are _immutable_. You can’t rewrite the contents of
   a string, for example, you have to generate a new one.
2. Primitives do not have _identity_. `===` compares the _value_, not
   the _pointer_. Two strings, for example, are equal if they contain
   the same _characters_, regardless of where those characters are
   stored.
...
2. Primitives do not have _prototypes_, so if you evaluate
   ...
   - In particular, if you access a primitive value from another realm
     ...
     try to invoke a method on it, it will get wrapped in the _local_
     _wrapper_ for the current realm, and not the wrapper from the
...
Therefore, I think we should focus on value _objects_. A value object
...
users first define a custom _value struct type_ or _value array type_
...
1. A user-defined struct or array can be made into a value type via a
   “valueType()” transformer like `var Point = new StructType({x: uint8, y: uint8}).valueType();`
...
creates a _copy_ (if the owner is not a value type). Hence `p` is not
...
Rule 2.2 also has another important implication. _Without_ rule 2.2,
...
– we want it to be _invisible_ when copies occur, at least if there
...
with them, however, is that they are always keyed on _object_
_identity_. This means that if you wanted to have, say, a map keyed by
...
tied to the _type_ but rather to the instance. I have avoided this
