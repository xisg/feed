...
url: https://smallcultfollowing.com/babysteps/blog/2017/07/11/non-lexical-lifetimes-draft-rfc-and-prototype-available/?utm_source=atom_feed
published: "2017-07-11T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2017/07/11/non-lexical-lifetimes-draft-rfc-and-prototype-available/
---
...
- The draft RFC is [available in a GitHub repository](https://github.com/nikomatsakis/nll-rfc/):
...
- There is a [working prototype as well](https://github.com/nikomatsakis/nll/):
...
in my [introductory blog post;](https://smallcultfollowing.com/babysteps/
/blog/2016/04/27/non-lexical-lifetimes-introduction/) but that alone isn’t enough to
...
When combined with [RFC 2025](https://github.com/rust-lang/rfcs/pull/2025), this means that we will accept
...
refers to. You can workaround this problem by forcing a _move_ of
`data` (e.g., by writing `{data}.split_first_mut()`), but you
shouldn’t have to. (This issue has been filed for some time as
[#10520](https://github.com/rust-lang/rust/issues/10520), which also lists some other workarounds.)

### Draft RFC

The Draft RFC is almost complete. I’ve created
[a GitHub repository](https://github.com/nikomatsakis/nll-rfc/) containing the text. I’ve also opened
...
[working prototype of these ideas](https://github.com/nikomatsakis/nll/). The prototype takes as
...
example, the [`borrowck-walk-linked-list.nll`](https://github.com/nikomatsakis/nll/blob/724156e86236052fb6c483e2359d99c47dd29dc7/test/borrowck-walk-linked-list.nll) test covers the
...
_very_ concise form because I was too lazy to make it more
user-friendly:
...
up. Perhaps surprisingly, the reference can have a _bigger_ lifetime
...
[in other tests](https://github.com/nikomatsakis/nll/blob/724156e86236052fb6c483e2359d99c47dd29dc7/test/borrowck-read-struct-containing-shared-ref-whose-referent-is-borrowed.nll#L11); these identify errors that the borrow checker
...
This was discussed on [an internals thread](https://internals.rust-lang.org/t/borrow-the-full-stable-name-in-closures-for-ergonomics/5387); as I
[commented there](https://internals.rust-lang.org/t/borrow-the-full-stable-name-in-closures-for-ergonomics/5387/11?u=nikomatsakis), I’d like to fix this by making the closure
...
[this comment on an internals thread](https://internals.rust-lang.org/t/partially-borrowed-moved-struct-types/5392/2) for my current thoughts. (A
...
latter, when combined with [associated type constructors](https://github.com/rust-lang/rfcs/pull/1598), might
...
case of futures especially, [the `?Move` RFC](https://github.com/rust-lang/rfcs/pull/1858) proposes another
