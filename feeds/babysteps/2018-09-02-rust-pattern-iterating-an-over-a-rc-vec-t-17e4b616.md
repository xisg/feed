...
url: https://smallcultfollowing.com/babysteps/blog/2018/09/02/rust-pattern-iterating-an-over-a-rc-vec-t/?utm_source=atom_feed
published: "2018-09-02T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2018/09/02/rust-pattern-iterating-an-over-a-rc-vec-t/
---
...
ownership of an `Rc<Vec<u32>>` and it wants to return an `impl Iterator<Item = u32>` [1](#fn:1) that iterates over that vector.
...
`Rc<Vec<u32>>` – that is, a reference counted[2](#fn:2) vector of
...
[`data.into_iter()`](https://doc.rust-lang.org/std/vec/struct.Vec.html#method.into_iter) and be done with it ( [try it on\
play](https://play.rust-lang.org/?gist=e5474c80b2f7fa290917b1bf3f522c30&version=stable&mode=debug&edition=2015)).
...
[`data.iter()`](https://doc.rust-lang.org/std/primitive.slice.html#method.iter) gives us back references ( `&u32`) and [the\
`cloned()` adapter](https://doc.rust-lang.org/std/iter/trait.Iterator.html#method.cloned) then “clones” them to give us back a `u32`
( [try it on play](https://play.rust-lang.org/?gist=e5474c80b2f7fa290917b1bf3f522c30&version=stable&mode=debug&edition=2015)).

But we have a `Rc<Vec<u32>>`, so what can we do? We can’t invoke
[`into_iter`](https://doc.rust-lang.org/std/vec/struct.Vec.html#method.into_iter), since that requires **complete** ownership
...
If you [try that on playground](https://play.rust-lang.org/?gist=dbf25e623505ebbb9a118b9155107fbc&version=stable&mode=debug&edition=2015), you’ll find you get this error:
...
_exactly_ what the problem is, but it’s pretty hard to understand.
(I’ve filed [#53882](https://github.com/rust-lang/rust/issues/53882) to improve it, though I’m not yet sure what I
...
syntactic structure of the code.[3](#fn:3) The borrow checker then comes after
...
( [playground](https://play.rust-lang.org/?gist=2fc90fb310e8fac9298d7c34a67e9a21&version=stable&mode=debug&edition=2015)):
...
**vector _is_ passing back to the caller as part of the iterator.**
...
strings ( [playground](https://play.rust-lang.org/?gist=ab0595b0cdbacd30a9d19493281fca52&version=stable&mode=debug&edition=2015)):
...
Rust. I’d like to transition to [a Polonius-based system](https://smallcultfollowing.com/babysteps/
...
could that work? Well, the problem fundamentally is that we _want_ a
...
concrete ideas for how this would work[4](#fn:4). But it’s an interesting
thought.
...
1. This just means it wants to return “some iterator that yields up `u32` values”. [↩︎](#fnref:1)

2. Also worth nothing: in Rust, reference counted data is typically immutable. [↩︎](#fnref:2)

3. In other words, lifetime inference doesn’t affect execution order. This is crucial – for example, it is the reason we can move to [NLL](https://rust-lang.github.io/rfcs/2094-nll.html) without breaking backwards compatibility. [↩︎](#fnref:3)
...
\[out pointers\]: [https://internals.rust-lang.org/t/thoughts-about-additional-built-in-pointer-types/959](https://internals.rust-lang.org/t/thoughts-about-additional-built-in-pointer-types/959) [↩︎](#fnref:4)
