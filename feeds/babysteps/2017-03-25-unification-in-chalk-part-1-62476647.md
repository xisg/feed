...
url: https://smallcultfollowing.com/babysteps/blog/2017/03/25/unification-in-chalk-part-1/?utm_source=atom_feed
published: "2017-03-25T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2017/03/25/unification-in-chalk-part-1/
---

# Unification in Chalk, part 1

So in [my first post](https://smallcultfollowing.com/babysteps/
/blog/2017/01/26/lowering-rust-traits-to-logic/) on [chalk](https://github.com/nikomatsakis/chalk/), I mentioned that unification and
...
equal. In particular, we don’t yet have a simple constraint like `?T = String`. To arrive at that, we have to do **unification**.
...
- a bunch of **type variables** like `?T`. We often call these
  **existential type variables** because, when you look at things in a
  logical setting, they arise from asking questions like `exists ?T. (Option<String> = Option<?T>)` – i.e., does there exist a type
...
had one variable ( `?T`) and one unification problem ( `Option<?T> = Option<String>`). We start an initial state like this:
...
like `?X = T` for some type `T`, we can’t just **immediately** add `?X => T` to our substitution. We have to first check and make sure that
...
table, which uses [Tarjan’s Union-Find algorithm](https://en.wikipedia.org/wiki/Disjoint-set_data_structure) (there are a
[number of implementations](https://crates.io/search?q=union%20find) available on crates.io); the set
...
[Robinson](http://dl.acm.org/citation.cfm?id=321253), I believe, and it forms the heart of
[Hindley-Milner type inference](https://en.wikipedia.org/wiki/Hindley%E2%80%93Milner_type_system) (used in ML, Haskell, and Rust as
...
1. Later on, probably not in this post, we’ll see universal type variables (i.e., `forall !T`); if you’re interested in reading up on how they interact with inference, I recommend \[“A Proof Procedure for the Logic of Hereditary Harrop Formulas”, by Gopalan Nadathur\]\[pphhf\], which has a very concrete explanation.
\[pphhf\]: [http://dl.acm.org/citation.cfm?id=868380](http://dl.acm.org/citation.cfm?id=868380) [↩︎](#fnref:1)
