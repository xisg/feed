...
url: https://smallcultfollowing.com/babysteps/blog/2016/09/24/intersection-impls/?utm_source=atom_feed
published: "2016-09-24T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2016/09/24/intersection-impls/
---
...
in [RFC 1210](https://github.com/rust-lang/rfcs/pull/1210). The idea of specialization is to improve Rust’s
existing coherence rules to allow for overlap between impls, so long
as one of the overlapping impls can be considered _more_
_specific_. Specialization is hotly desired because it can enable
powerful optimizations, but also because it is an important component
for [modeling object-oriented designs](http://aturon.github.io/blog/2015/09/18/reuse/).
...
These two traits are clearly _related_. In fact, `Clone` is a
_supertrait_ of `Copy`, which means that every type that is copyable
must also be cloneable.

For better or worse, supertraits in Rust work a bit differently than
_superclasses_ from OO languages. In particular, the two traits are
...
could be used _any_ `Copy` type. It would be nice if we could add a
...
![](/images/Troymcclure.png)

_Hi! I’m the language feature coherence! You may remember me from_
_previous essays like [Little Orphan Impls](http://smallcultfollowing.com/babysteps/blog/2015/01/14/little-orphan-impls/) or [RFC 1023](https://github.com/rust-lang/rfcs/pull/1023)._
...
The problem is that types like `Option<T>` are _sometimes_ `Copy` and
...
what if we adopted the existing [specialization RFC](https://github.com/rust-lang/rfcs/pull/1210)? After
...
those implementations is _more specific_. Maybe that applies here?
...
[RFC #1210](https://github.com/rust-lang/rfcs/pull/1210). If the compiler ever had to choose between them, it
...
quite right. After all, the blanket impl cannot be used for _any_ type
...
[RFC #1210](https://github.com/rust-lang/rfcs/pull/1210) and hence adding a blanket impl now would _still_ be
...
that covers _precisely_ that intersection, and hence which subsets
...
is, if we add the blanket impl, we _also_ have to add the intersection
...
[nrc described on the tracking issue](https://github.com/rust-lang/rust/issues/31844#issuecomment-247867693): sometimes there is a
_theoretical_ intersection between impls, but that intersection is
...
is much like the existing [`Display`](https://doc.rust-lang.org/std/fmt/trait.Display.html) trait, except that it
...
[RFC 1023](https://github.com/rust-lang/rfcs/pull/1023), impls A and B are considered to _potentially_ overlap,
...
crate. This is because [RFC 1023](https://github.com/rust-lang/rfcs/pull/1023) permits the use of “negative
reasoning” **if either the trait or the type is under local**
**control**. That is, if the `RichDisplay` and the `Widget` type were
defined in the _same_ crate, then impls A and B could co-exist,
...
- they interact poorly with semver, and specifically the limitations
  on negative logic imposed by [RFC 1023](https://github.com/rust-lang/rfcs/pull/1023).

My conclusion then is that intersection impls may well be _part_ of
...
[thread on Rust’s internals forum](https://internals.rust-lang.org/t/blog-post-intersection-impls/4129) instead.
