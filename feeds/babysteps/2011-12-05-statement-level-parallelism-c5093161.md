...
url: https://smallcultfollowing.com/babysteps/blog/2011/12/05/statement-level-parallelism/?utm_source=atom_feed
published: "2011-12-05T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2011/12/05/statement-level-parallelism/
---
...
[_parallel blocks_](http://harmonic-lang.org/tutorial-2/4-parallel-control-flow-usi.html), which are blocks (in the Smalltalk and
Ruby sense) that may execute in parallel with each other _and with the_
_function that created them_. This is a slight twist on the typical
...
data and you want to give each piece out to a worker which then _owns_
...
reflection to allow parallel blocks (which were written `{| x -> ... |}` instead of `{ x-> ... }`) to simply execute in parallel.
...
pools would allow us to allocate _mutable_ data on the stack in the
