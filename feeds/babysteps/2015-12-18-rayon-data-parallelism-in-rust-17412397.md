...
url: https://smallcultfollowing.com/babysteps/blog/2015/12/18/rayon-data-parallelism-in-rust/?utm_source=atom_feed
published: "2015-12-18T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2015/12/18/rayon-data-parallelism-in-rust/
---

# Rayon: data parallelism in Rust

Over the last week or so, I’ve been working on an update to
[Rayon](https://github.com/nikomatsakis/rayon/), my experimental library for **data parallelism** in
...
it will _potentially_ execute them in parallel. Once they have both
...
The fact that the two closures _potentially_ run in parallel is key:
...
[crossbeam’s scoped threads](https://github.com/aturon/crossbeam/blob/master/src/scoped.rs). Whereas in crossbeam,
...
**of the main points of this post is to advocate for _potential_**
**_parallelism_ as the basis for Rust data parallelism libraries**, in
contrast to the _guaranteed concurrency_ that we have seen thus far.
...
inherently _parallel_. (Note though that it is perfectly reasonable to
use types like `Mutex`, `AtomicU32`, etc from within a `join` call –
you just don’t want one closure to _block_ waiting for the other.)
...
[parallel version of quicksort](https://github.com/nikomatsakis/rayon/blob/22f04aee0e12b31e029ec669299802d6e2f86bf6/src/test.rs#L6-L28) like so:
...
[actual code](https://github.com/nikomatsakis/rayon/blob/22f04aee0e12b31e029ec669299802d6e2f86bf6/src/api.rs#L23-L64) works somewhat differently; for example, it allows
...
_are_ idle threads available, then we get parallelism.
...
can get pretty decent speedups even today, but it does take a _bit_
more tuning than I would like. For example, with a
[tweaked version of quicksort](https://github.com/nikomatsakis/rayon/blob/22f04aee0e12b31e029ec669299802d6e2f86bf6/demo/quicksort/src/main.rs#L47-L60), I see the following
[parallel speedups](https://en.wikipedia.org/wiki/Speedup#Speedup_in_latency) on my 4-core Macbook Pro (hence, 4x is
basically the best you could expect):

Array LengthSpeedup 1K 0.95x  32K 2.19x  64K 3.09x  128K 3.52x  512K 3.84x  1024K 4.01x

The change that I made from the original version is to introduce
_sequential fallback_. Basically, we just check if we have a small
...
see from [the demo code](https://github.com/nikomatsakis/rayon/blob/22f04aee0e12b31e029ec669299802d6e2f86bf6/demo/quicksort/src/main.rs#L47-L60). (If you’re curious, I explain the idea
...
Array LengthSpeedup 1K 0.41x  32K 2.05x  64K 2.42x  128K 2.75x  512K 3.02x  1024K 3.10x
...
all, the expectation is that most tasks will _never_ be stolen,
...
- Many work-stealing implementations use heuristics to try and decide
  ...
  [Lazy Scheduling](http://dl.acm.org/citation.cfm?id=2629643) work by Tzannes et al. tries to avoid pushing a
...
- And of course good ol’ fashioned optimization would help. I’ve never
  even _looked_ at the generated LLVM bitcode or assembly for `join`,
...
_Side note:_ This example may seem artificial, but in fact this is an
...
[non-atomic reference-counted type](http://doc.rust-lang.org/std/rc/struct.Rc.html) called `Rc`. Because `Rc` uses
...
is because `&mut` data is supposed to be _uniquely_ accessed, and
hence if you had two closures, they would both have access to the same
“unique” data. Which of course makes it not so unique.

(In fact, this was one of the [great epiphanies for me](http://smallcultfollowing.com/babysteps/blog/2013/06/11/on-the-connection-between-memory-management-and-data-race-freedom/) in
...
[pretty simple wrapper](https://github.com/nikomatsakis/rayon/tree/22f04aee0e12b31e029ec669299802d6e2f86bf6/src/par_iter) around `join`. At the moment, it’s
more of a proof of concept than anything else. But what’s really nifty
about it is that it does not require _any_ unsafe code related to
parallelism – that is, it just builds on `join`, which encapsulates
all of the unsafety. (To be clear, there _is_ a small amount of unsafe
code related to [managing uninitialized memory](https://github.com/nikomatsakis/rayon/blob/22f04aee0e12b31e029ec669299802d6e2f86bf6/src/par_iter/collect.rs#L27-L47) when
collecting into a vector. But this has nothing to do with
_parallelism_; you’ll find similar code in `Vec`. This code is also
...
[has the following core members](https://github.com/nikomatsakis/rayon/blob/22f04aee0e12b31e029ec669299802d6e2f86bf6/src/par_iter/mod.rs#L30-L35):
...
The [`ParallelIteratorState` trait](https://github.com/nikomatsakis/rayon/blob/22f04aee0e12b31e029ec669299802d6e2f86bf6/src/par_iter/mod.rs#L80-L90) represents some
...
- implement [`len`](https://github.com/nikomatsakis/rayon/blob/22f04aee0e12b31e029ec669299802d6e2f86bf6/src/par_iter/slice.rs#L30-L36) by just returning the length of the slice,
- implement [`split_at`](https://github.com/nikomatsakis/rayon/blob/22f04aee0e12b31e029ec669299802d6e2f86bf6/src/par_iter/slice.rs#L38-L41) by splitting the slice into two subslices,
- and implement [`for_each`](https://github.com/nikomatsakis/rayon/blob/22f04aee0e12b31e029ec669299802d6e2f86bf6/src/par_iter/slice.rs#L43-L49) by iterating over the array and
...
[collect into a vector](https://github.com/nikomatsakis/rayon/blob/22f04aee0e12b31e029ec669299802d6e2f86bf6/src/par_iter/collect.rs#L27-L47) or to
[reduce a stream of values into one](https://github.com/nikomatsakis/rayon/blob/22f04aee0e12b31e029ec669299802d6e2f86bf6/src/par_iter/reduce.rs#L20-L42).
...
If you look at some of my [earlier](http://smallcultfollowing.com/babysteps/blog/2013/06/11/data-parallelism-in-rust/) [designs](http://smallcultfollowing.com/babysteps/blog/2012/06/11/hotpar/),
...
users, but also simple for _implementors_ – that is, it doesn’t
...
- [“INHTWAMA”](http://smallcultfollowing.com/babysteps/blog/2012/11/18/imagine-never-hearing-the-phrase-aliasable/), which was the decision to make `&mut` references
  be non-aliasable and to remove `const` (read-only, but not
  immutable) references. This basically meant that Rust authors were
  now writing data-race-free code _by default_.
- [Improved Send traits](https://github.com/rust-lang/rfcs/blob/master/text/0458-send-improvements.md), or RFC 458, which modified the `Send`
  trait to permit borrowed references. Prior to this RFC, which was
  authored by [Joshua Yanovski](https://github.com/pythonesque), we had the constraint that for
...
the [trick that I used in the demo code](https://github.com/nikomatsakis/rayon/blob/22f04aee0e12b31e029ec669299802d6e2f86bf6/demo/quicksort/src/main.rs#L47-L60).
