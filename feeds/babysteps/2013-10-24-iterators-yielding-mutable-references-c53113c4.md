...
url: https://smallcultfollowing.com/babysteps/blog/2013/10/24/iterators-yielding-mutable-references/?utm_source=atom_feed
published: "2013-10-24T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2013/10/24/iterators-yielding-mutable-references/
---

# Iterators yielding mutable references

There is a [known bug](https://github.com/mozilla/rust/issues/8624) with the borrowck rules that causes it to
...
_interface_ for iterating over `&mut` values, it is not possible to
_implement_ such iterators without an unsafe block.
...
same iterator trait to handling mutable references like `&mut int`. But let’s take it step by step.
...
trait. The interface below _looks_ reasonable, but we will see that
...
interface without an unsafe block (modulo [#8624](https://github.com/mozilla/rust/issues/8624)).
...
must be the _only way to mutate the memory it references_. This rule
ensures [memory safety](http://static.rust-lang.org/doc/0.6/tutorial-borrowed-ptr.html#borrowing-unique-boxes) and can also serve other purposes, such
as [preventing data races](http://smallcultfollowing.com/babysteps/blog/2013/06/11/data-parallelism-in-rust/) (I have in the past waxed
philosophical about how
[those two classes of errors are two sides of the same coin](http://smallcultfollowing.com/babysteps/blog/2013/06/11/on-the-connection-between-memory-management-and-data-race-freedom/)).
...
access the _iterator itself_. That means that someone could write:
...
will, once I push the fix for [#8624](https://github.com/mozilla/rust/issues/8624)). More specifically, there
...
for any given `VecMutIterator` instance, there isn’t a _single_ lifetime
...
showed you is unsafe and couldn’t be implemented. But we _do_ want to
...
the _user_ of the interface. It does mean that implementing iterators
...
returned exactly once (e.g., [Java’s `ListIterator`](http://docs.oracle.com/javase/6/docs/api/java/util/ListIterator.html), or the
...
one _can_ have infinite iterators, but one can also choose not
...
Here you can see that the lifetime of the result is _always_ `'n`.
...
over _any_ iterator, but must instead always handle the `MutIterator`
...
[higher-kinded types](http://en.wikipedia.org/wiki/Kind_%28type_theory%29). We could redefine the `Iterator` trait so
...
where the parameters and result types are _types_, not _values_.
...
_probably allow_ `VecMutIterator` as well. But the `VecMutIterator`
...
**Safety.** To be honest, I am not _that_ concerned about the safety
...
write generic code that operates over _all_ iterators, but the mutable
...
_But_ traits like `RandomAccessIterator` cannot be supported,
...
an exception: Tell me dear reader, what do _you_ think we should do?
