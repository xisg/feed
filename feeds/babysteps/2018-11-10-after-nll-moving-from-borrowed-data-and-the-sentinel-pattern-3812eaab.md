...
url: https://smallcultfollowing.com/babysteps/blog/2018/11/10/after-nll-moving-from-borrowed-data-and-the-sentinel-pattern/?utm_source=atom_feed
published: "2018-11-10T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2018/11/10/after-nll-moving-from-borrowed-data-and-the-sentinel-pattern/
---
...
from borrowed data and the _Sentinel Pattern_ that can be used to enable
them.

# The problem

Sometimes when we have `&mut` access to a struct, we have a need to
_temporarily_ take ownership of some of its fields. Usually what happens
...
Now, if we try to [run this code](https://play.rust-lang.org/?version=stable&mode=debug&edition=2015&gist=896551436908a5a7b8b76d5f5ace54af),
...
The problem here is that we need to _take ownership_ of `self.chain`,
...
though: can we be _really sure_ that we are going to replace
...
is possible for this function call to _panic_, at least in principle. If
...
some sort of `Mutex` or something else, so even if _our thread_ panics,
other threads might observe the hole.

To return to our Lego analogy[1](#fn:1), it is as if – after we removed some
...
In fact, there _is_ a way to move out from an `&mut` pointer – you can
use [the function `std::mem::replace`](https://doc.rust-lang.org/std/mem/fn.replace.html) [2](#fn:2). `replace`
...
`self.chain` and _immediately_ put a replacement there.
...
temporary, dummy value in that spot. I call this a _sentinel_ value –
...
To return to our Lego analogy[3](#fn:3), it’s as if, when we
...
that? Actually, in this _particular_ example, we can be fairly sure…
...
reads `self.counter` and creates that many new links[4](#fn:4) in the
chain:
...
Now I would get a bit nervous. I _think_ nobody ever observes this empty
chain, but how can I be _sure_? At some point, you would like to test
this hypothesis.
...
`Option<Chain>`, with the invariant that `self.chain` should _always_ be
...
There is a crate called [`take_mut`](https://crates.io/crates/take_mut) on crates.io that offers a
...
about [“unsafe composability”](https://smallcultfollowing.com/babysteps/
/blog/2016/10/02/observational-equivalence-and-unsafe-code/) that worry me a bit, which I’ll
discuss at the end.
...
The [`take`](https://docs.rs/take_mut/0.2.2/take_mut/fn.take.html) function works like so: first, it uses unsafe code to
...
library is that it will _abort the entire process_. That is, unlike with
...
_aborting the process does not compose with some other unsafe_
_abstractions you might want_. Imagine, for example, that you had
...
[observational equivalence](https://smallcultfollowing.com/babysteps/
/blog/2016/10/02/observational-equivalence-and-unsafe-code/).

### My recommenation

I would advise you to use some variant of the sentinel pattern. I
personally prefer to use a “signaling sentinel”[5](#fn:5) like `Option`
if it would be a bug for other code to read the field, unless the range
of code where the value is taken is very simple. So, in our _original_
...
It’s a _bit_ annoying to use `Option` for this because it’s so explicit.
...
`T` values within, you can use the [`drain`](https://doc.rust-lang.org/std/vec/struct.Vec.html#method.drain) iterator method. The
only real difference from [`drain`](https://doc.rust-lang.org/std/vec/struct.Vec.html#method.drain) vs [`into_iter`](https://doc.rust-lang.org/std/iter/trait.IntoIterator.html#tymethod.into_iter) is that [`drain`](https://doc.rust-lang.org/std/vec/struct.Vec.html#method.drain)
...
between writing a `fn(self) -> Self` sort of signature vs `fn(&mut self)`, you might adopt the latter, as it gives your callers more
flexibility. But this is a bit subtle; it would make a good topic for
the [Rust API guidelines](https://rust-lang-nursery.github.io/api-guidelines/), but I didn’t find it there.)

### Discussion

If you’d like to discuss something in this post, there is a [dedicated\
thread on the users.rust-lang.org site](https://users.rust-lang.org/t/blog-post-series-after-nll-whats-next-for-borrowing-and-lifetimes/21864).
...
guaranteed to be replaced _before the function returns or panics_. The
...
function doesn’t panic _now_, but does that mean you promise never to
make it panic in the future?[6](#fn:6)
...
There are however some other cases where _similar_ sorts of “permission
...
this interpretation that makes methods like [`Mutex::get_mut`](https://doc.rust-lang.org/std/sync/struct.Mutex.html#method.get_mut) sound.

If we were to move in this direction, we might look to languages like
[Mezzo](http://gallium.inria.fr/~fpottier/publis/bpp-mezzo-journal.pdf) for inspiration, which encode this notion of “permissions in, permissons
out” more directly[7](#fn:7). I’m definitely interested in
...
>
> — Alexis Beingessner (@Gankro) [November 10, 2018](https://twitter.com/Gankro/status/1061364298449108992?ref_src=twsrc%5Etfw)
...
1. I really like this Lego analogy. You’ll just have to bear with me. [↩︎](#fnref:1)

2. `std::mem::replace` is a super useful function in all kinds of scenarios; worth having in your toolbox. [↩︎](#fnref:2)

3. OK, maybe I’m taking this analogy too far. Sorry. I need help. [↩︎](#fnref:3)

4. I bet you were wondering what that `counter` field was for – gotta admire that \[Chekhov’s Gun\] action. [↩︎](#fnref:4)

5. i.e., some sort of sentinel where a panic occurs if the memory is observed [↩︎](#fnref:5)

6. It occurs to me that we now have a corpus of crates at various versions. It would be interesting to see how common it is to make something panic which did not used to, as well sa to make other sorts of changes. [↩︎](#fnref:6)

7. Also related: \[fractional permissions\] and a whole host of other things.
\[fractional permissions\]: [https://pdfs.semanticscholar.org/f744/e6fe7b8d9f92205d3a407e0446369c5f02bd.pdf](https://pdfs.semanticscholar.org/f744/e6fe7b8d9f92205d3a407e0446369c5f02bd.pdf) [↩︎](#fnref:7)
