...
url: https://smallcultfollowing.com/babysteps/blog/2013/11/27/thoughts-on-dst-3/?utm_source=atom_feed
published: "2013-11-27T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2013/11/27/thoughts-on-dst-3/
---

# Thoughts on DST, Part 3

After posting [part 2](https://smallcultfollowing.com/babysteps/
/blog/2013/11/26/thoughts-on-dst-2/) of my DST series, I realized that I had
...
(which I will dub _statically sized typed_ (SST) from here on
out). Let me first explain the problem and then show how my first
thoughts on how it might be addressed.

This is part 3 of a series:

1. [Examining DST.](https://smallcultfollowing.com/babysteps/
   /blog/2013/11/26/thoughts-on-dst-1/)
2. [Examining an alternative formulation of the current system.](https://smallcultfollowing.com/babysteps/
   /blog/2013/11/26/thoughts-on-dst-2/)
3. …and then a twist.
4. [Further complications.](https://smallcultfollowing.com/babysteps/
   /blog/2013/12/02/thoughts-on-dst-4/)
...
has type `exists N. RC<[int, ..N]>` (written long-hand). _Typing_ this
dereference is relatively straightforward, but when you think about
the _actual code_ that we generate, things get more complicated.
...
of course is that it doesn’t actually _matter_ which version we call
...
parameters as _erased_. If a type parameter `T` is marked erased, the
...
run. And if `T` is _not_ erased, then when `RC<[~int]>` is dropped,
we won’t know whether to run the destructor for `RC<[~int, ..5]>` or
`RC<[~int, ..6]>` etc. _And_ – of course – it’s wildly wasteful to have
...
instantiation would be an unsized type `[U]` (that is, `exists N. [U, ..N]`). The compiler knew it could always find the length for
any instance of `[U]` and thus could generate drop glue and so on.

So perhaps the solution is not to have _erased_, which says “code
generation knows _nothing_ about `T`, but rather some sort of partial
...
SST approach, but we’ll have to find the correct variation on _erased_
