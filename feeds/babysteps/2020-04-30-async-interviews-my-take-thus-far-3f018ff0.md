...
url: https://smallcultfollowing.com/babysteps/blog/2020/04/30/async-interviews-my-take-thus-far/?utm_source=atom_feed
published: "2020-04-30T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2020/04/30/async-interviews-my-take-thus-far/
---

# Async interviews: my take thus far

The point of the [async interview](https://smallcultfollowing.com/babysteps/
/blog/2019/11/22/announcing-the-async-interviews/) series, in the end, was to help
...
Most of the focus of [wg-async-foundations](https://github.com/rust-lang/wg-async-foundations) recently has been on
polish work on the compiler, and we’ve made quite a lot of
progress. Diagnostics have [notably](https://github.com/rust-lang/rust/pull/64895) [improved](https://github.com/rust-lang/rust/pull/65345), and we’ve been
working on [inserting](https://github.com/rust-lang/rust/pull/70906) [helpful](https://github.com/rust-lang/rust/pull/68212) [suggestions](https://github.com/rust-lang/rust/pull/71174), [fixing compiler\
bugs](https://github.com/rust-lang/rust/pull/68884), and [improving efficiency](https://github.com/rust-lang/rust/pull/69837). One thing I’m especially excited
about is that we [no longer rely on thread-local storage in the `async fn` transformation](https://github.com/rust-lang/rust/pull/69033), which means that async-await is now
compatible with `#[no_std]` environments and hence embedded
development.

I want to give a 👏 “shout-out” 👏 to 👏 [tmandry](https://github.com/tmandry) 👏 for leading this
...
some [tips for how to get involved](https://github.com/rust-lang/wg-async-foundations#getting-involved).

I think it’s also a good idea to be looking a bit more broadly. On
Zulip, for example, [LucioFranco suggested](https://zulip-archive.rust-lang.org/187312wgasyncfoundations/81944meeting20200428.html#195598667) that we could add a
...
lint](https://github.com/rust-lang/rust-clippy/issues/4226) already, though it’s specific to `MutexGuard`; maybe this should
...
[`task::block_on`](http://smallcultfollowing.com/babysteps/blog/2020/03/10/async-interview-7-withoutboats/#block_on-in-the-std-library). YoshuaWuyts recently proposed adding some simple
constructors, like [`future::{pending, ready}`](https://github.com/rust-lang/rust/pull/70834) which I think could
...
Adding `async fn main`, as [boats proposed](http://smallcultfollowing.com/babysteps/blog/2020/03/10/async-interview-7-withoutboats/#async-fn-main), might be such an example
(and I rather like the idea of `#[test]` that [XAMPRocky proposed on\
internals](https://users.rust-lang.org/t/async-interviews/35167/17?u=nikomatsakis)).

Another change I think makes sense is to support [async destructors](http://smallcultfollowing.com/babysteps/blog/2020/03/10/async-interview-7-withoutboats/#next-step-async-destructors),
and I would go further and adopt find some solution to the concerns
about RAII and async that [Eliza Weisman raised](http://smallcultfollowing.com/babysteps/blog/2020/02/11/async-interview-6-eliza-weisman/#raii-and-async-fn-doesnt-always-play-well). In particular, I
...
The first step, I think, is to do [as Carl Lerche proposed](http://smallcultfollowing.com/babysteps/blog/2019/12/23/async-interview-3-carl-lerche/#what-should-we-do-next-stabilize-stream) and add
the [`Stream`](https://docs.rs/futures/0.3.4/futures/stream/trait.Stream.html) trait into the standard library. Ideally, it would be
added in _exactly_ the form that it takes in futures 0.3.4, so that we
...
cramertj, in particular, [raised a concern](http://smallcultfollowing.com/babysteps/blog/2019/12/10/async-interview-2-cramertj-part-2/) about the desire for
...
of `Iterator`. It has a [`poll_next`](https://docs.rs/futures/0.3.4/futures/stream/trait.Stream.html#tymethod.poll_next) method that tries to fetch the
...
themselves (though they _can_ yield references into the collection
...
streams will require not only [GATs](http://smallcultfollowing.com/babysteps/blog/2019/12/10/async-interview-2-cramertj-part-2/#the-natural-way-to-write-attached-streams-is-with-gats), but experience with GATs. This
...
had it right when they said that the [guiding vision](http://smallcultfollowing.com/babysteps/blog/2020/03/10/async-interview-7-withoutboats/#vision-for-async) for Async I/O in
...
or another. But I think that in cases where we see _orthogonal_
_problems_ – problems that are not really related to being synchronous
...
it for both sync and async applications, well, synchronously[1](#fn:1).

### Supporting interoperability: adding async read and write traits

I also think we should add [`AsyncRead`](https://docs.rs/futures/0.3.4/futures/io/trait.AsyncRead.html) and [`AsyncWrite`](https://docs.rs/futures/0.3.4/futures/io/trait.AsyncWrite.html) to the
...
In contrast to [`Stream`](https://docs.rs/futures/0.3.4/futures/stream/trait.Stream.html), I do expect this to be controversial, for a
few reasons. But much like [`Stream`](https://docs.rs/futures/0.3.4/futures/stream/trait.Stream.html), I still think it’s the right
...
its synchronous counterpart [`Read`](https://doc.rust-lang.org/std/io/trait.Read.html), the [`AsyncRead`](https://docs.rs/futures/0.3.4/futures/io/trait.AsyncRead.html) trait must be
given a buffer where the data will be written. And, just like
[`Read`](https://doc.rust-lang.org/std/io/trait.Read.html), the trait currently requires that this buffer must be zeroed
...
that](http://smallcultfollowing.com/babysteps/blog/2020/01/20/async-interview-5-steven-fackler/). The idea that we discussed in our async interview is
...
### Second concern about async read: io\_uring
...
[`epoll`](https://en.wikipedia.org/wiki/Epoll)-like interfaces. In these interfaces, you get a callback
...
gives a [good intro](https://boats.gitlab.io/blog/post/iou/) in their blog post introducing the [`iou`](https://github.com/withoutboats/iou)
library.
...
and experimenting with the [`AsyncRead`](https://docs.rs/futures/0.3.4/futures/io/trait.AsyncRead.html) and [`AsyncWrite`](https://docs.rs/futures/0.3.4/futures/io/trait.AsyncWrite.html). It’s time
...
doing so. As [boats said](http://smallcultfollowing.com/babysteps/blog/2020/03/10/async-interview-7-withoutboats/#supporting-generators-iterators-and-async-generators-streams), writing `Stream` implementations is a
...
- `impl Trait` everywhere – finishing up the `impl Trait` saga will
  ...
  nice, such as Tower’s [`Service`](https://docs.rs/tower/0.3.0/tower/trait.Service.html) trait;
...
- Extend the stdlib with mutexes, channels, [`task::block_on`](http://smallcultfollowing.com/babysteps/blog/2020/03/10/async-interview-7-withoutboats/#block_on-in-the-std-library), and other small utilities.
...
There are a few things I’d be particularly interested to [get feedback](https://users.rust-lang.org/t/async-interviews/35167/) on:
...
Thus far, [tmandry](https://github.com/tmandry) has been doing a great job at organizing the
...
involved](https://github.com/rust-lang/wg-async-foundations#getting-involved)!)
...
maintaining the [async book](https://rust-lang.github.io/async-book/index.html).

To do those tasks, we’re going to need [more than coders](http://smallcultfollowing.com/babysteps/blog/2019/04/15/more-than-coders/). We need to
...
on Zulip and say hello, and monitor the [Inside Rust](https://blog.rust-lang.org/inside-rust/), as I expect
we’ll be posting updates there from time to time.

### Comments?

As always, please leave comments in the [async interviews thread](https://users.rust-lang.org/t/async-interviews/35167/)
