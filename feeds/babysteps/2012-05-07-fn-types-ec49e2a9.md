---
title: Fn types
url: https://smallcultfollowing.com/babysteps/blog/2012/05/07/fn-types/?utm_source=atom_feed
published: "2012-05-07T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2012/05/07/fn-types/
---

# Fn types

As you loyal readers know, I am on a quest to make the Rust type
system more _orthogonal_ with respect to the kind of pointer in use,
...
- The unique closure ( `fn~`) uses a unique pointer to store its
  environment. All data must be _sendable_, which basically means
...
- The stack closure ( `fn&`) uses a reference to store its environment.
  ...
  stack frame. Copying a stack closure is _very_ cheap. Because
...
therefore must basically always be referenced by pointer ( `@fn(S) -> T`, `~fn(S) -> T`, `&fn(S) -> T`).
...
_always_ copies out of the stack frame. So something like:
...
closure must be copyable; but if data is _moved_ into the closure (for
...
have a type of `fn(S) -> T`. You would then write `vec::iter(v, &foo)` to apply the top-level function `foo` to each item in the
