...
url: https://smallcultfollowing.com/babysteps/blog/2016/09/12/thoughts-on-trusting-types-and-unsafe-code/?utm_source=atom_feed
published: "2016-09-12T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2016/09/12/thoughts-on-trusting-types-and-unsafe-code/
---
...
[Tootsie Pop Model](http://smallcultfollowing.com/babysteps/blog/2016/05/27/the-tootsie-pop-model-for-unsafe-code/) (TPM) blog post, one of the _key_ examples
...
[unchecked-get](http://smallcultfollowing.com/babysteps/blog/2016/08/18/tootsie-pop-followup/) use case. I’ve been pondering various ways to address
...
be ( [at least currently](http://smallcultfollowing.com/babysteps/blog/2016/04/27/non-lexical-lifetimes-introduction/)) **yes**. Remember that the purpose of this
...
[non-lexical lifetimes](http://smallcultfollowing.com/babysteps/blog/2016/04/27/non-lexical-lifetimes-introduction/) approach, that would mean the lifetime
...
fact the _programmer_ knows that `condition1()` and `condition3()` are
...
The idea here is that lifetimes are _sometimes_ significant to the
...
evidence that the _user_ believes the lifetime of `q` to enclose
...
[“Asserting-Conflicting Access” model](https://github.com/nikomatsakis/rust-memory-model/issues/26) (ACA), which was proposed
...
is free to pick any valid lifetime, so long as it would still _type_
_check_ (not “borrow check” – i.e., it only has to ensure that no data
...
implications for the [unchecked-get](http://smallcultfollowing.com/babysteps/blog/2016/08/18/tootsie-pop-followup/) example.)

Nonetheless, I have concerns about this formulation because it seems
to assume that the logic for unsafe code _can_ be expressed in terms
...
In terms of their model, the _conflicting access_ would be (2) and the
_asserting access_ would be (1). But I might be misunderstanding how
this whole thing works.

#### Trust lifetimes at safe fn boundaries

Nonetheless, perhaps we can do something _similar_ to the ACA model
...
reference _any time during its execution_. I have been (separately)
...
**it easy to optimize Rust code aggressively, there _is_ something**
...
interprocedural analysis, it seems clear that we _need_ the caller to
...
comments on [the internals thread](https://internals.rust-lang.org/t/blog-post-thoughts-on-trusting-types-and-unsafe-code/4059) I’m about to start. =)

Also, I’m collecting unsafe-related posts into the [unsafe category](http://smallcultfollowing.com/babysteps/blog/categories/unsafe/).
