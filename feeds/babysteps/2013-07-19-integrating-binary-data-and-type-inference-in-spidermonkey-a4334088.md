...
url: https://smallcultfollowing.com/babysteps/blog/2013/07/19/integrating-binary-data-and-type-inference-in-spidermonkey/?utm_source=atom_feed
published: "2013-07-19T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2013/07/19/integrating-binary-data-and-type-inference-in-spidermonkey/
---
...
Nikhil Marathe has prepared an [initial implementation](https://bugzilla.mozilla.org/show_bug.cgi?id=578700&sourceid=Mozilla-search), but
...
Let’s cover some terminology. In binary data, users create _type_
_descriptors_ as follows:
...
are called _metatypes_ because they do not themselves represent types
...
actually _two_ property accesses, one after the other. In other
...
Here, the first access, `line.end`, returns a new, _derived instance_
...
When the JIT processes the first property access, `var tmp = line.end`, it will observe from the type set of `line` that `end` is a
...
In the case where the access to the complex property _is_ the end
...
could occur if `line` is _sometimes_ a normal JS object and
_sometimes_ a binary data instance, or if it may be one of many kinds
...
For simplicity, I didn’t discuss _handles_, which are effectively
binary data pointers. A handle lets you get a pointer to a subpiece of
another binary data object. This is _similar_ to the derived objects I
...
For example, the [plan for PJS](https://smallcultfollowing.com/babysteps/
/blog/2013/05/29/integrating-binary-data-and-pjs/) is to use handles when
