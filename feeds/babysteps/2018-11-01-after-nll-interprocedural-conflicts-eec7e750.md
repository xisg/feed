...
url: https://smallcultfollowing.com/babysteps/blog/2018/11/01/after-nll-interprocedural-conflicts/?utm_source=atom_feed
published: "2018-11-01T00:00:00Z"
feed: babysteps
guid: https://smallcultfollowing.com/babysteps/blog/2018/11/01/after-nll-interprocedural-conflicts/
---
...
[Precise Closure Capture Clauses](https://smallcultfollowing.com/babysteps/
/blog/2018/04/24/rust-pattern-precise-closure-capture-clauses/). But the problem is more
general.
...
Unfortunately, [this code is going to yield a compilation error](https://play.rust-lang.org/?version=beta&mode=debug&edition=2018&gist=ef84f42cd30a8c110e6d5ce4eceac5df).
...
What this message is trying to tell you[1](#fn:1) is that:
...
The _simplest_ way to fix this problem is to modify `check_widgets`
...
[This code compiles fine](https://play.rust-lang.org/?version=beta&mode=debug&edition=2018&gist=6512551bf58cd66917895a588f3643dc), since the compiler now sees access to
...
those fields don’t factor _nicely_. For example, if we return to our
...
method on `Sender` only requires `&self`](https://doc.rust-lang.org/std/sync/mpsc/struct.Sender.html#method.send), so a shared borrow is
...
to the iterator itself, just to a _reference to the iterator_. So
...
fact, we’ve accepted [RFC #2229](https://github.com/rust-lang/rfcs/pull/2229), which proposes to change the closure
...
1. This message is actually a bit confusing. [↩︎](#fnref:1)
