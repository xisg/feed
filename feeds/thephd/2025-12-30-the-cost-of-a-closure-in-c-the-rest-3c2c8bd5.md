...
feed: thephd
guid: https://thephd.dev/The%20Cost%20of%20a%20Closure%20II
---
...
If you want an introduction to what is going on, there’s a gentle description with some 10,000 foot overview in [the previous article](https://thephd.dev/the-cost-of-a-closure-in-c-c2y). Additionally, if you’d like to learn more about specific kinds of Closures as they exist in C and/or C++, you can read [a much older article](https://thephd.dev/lambdas-nested-functions-block-expressions-oh-my) or read the entire introduction in [this work-in-progress C proposal](https://thephd.dev/_vendor/future_cxx/papers/C%20-%20Functions%20with%20Data%20-%20Closures%20in%20C.html). The much older article is a much gentler introduction; the work-in-progress C proposal goes through a lot of the technical and design nitty-gritty and why things work or do not work very well.
...
As always, the implementations of my benchmarks are publicly available[1](https://thephd.dev/the-cost-of-a-closure-in-c-c2y-followup/#fn:idk-benchmarks-closures).

# Experimental Setup

The only thing that changed from the last time we did this was to use 150 repetitions of the whole 100,000+ sample iterations benchmarks rather than just 50 or 100 repetitions. You can find the full, detailed explanation [at the bottom of this article](https://thephd.dev/the-cost-of-a-closure-in-c-c2y-followup/#methodology).
...
![See the paragraph for a text description of this bar chart image.](https://thephd.dev/assets/img/2025/12/manorboy2-appleclang17-linear.png)

![See the paragraph for a text description of this bar chart image.](https://thephd.dev/assets/img/2025/12/manorboy2-gcc15-linear.png)

[For the vision-impaired, a text description is available.](https://thephd.dev/assets/img/2025/12/manorboy2-gcc15-linear.png.txt)
...
![See the paragraph for a text description of this bar chart image.](https://thephd.dev/assets/img/2025/12/manorboy2-appleclang17-logarithmic.png)

![See the paragraph for a text description of this bar chart image.](https://thephd.dev/assets/img/2025/12/manorboy2-gcc15-logarithmic.png)

[For the vision-impaired, a text description is available.](https://thephd.dev/assets/img/2025/12/manorboy2-gcc15-logarithmic.png.txt)
...
![See the paragraph for a text description of this bar chart image.](https://thephd.dev/assets/img/2025/12/manorboy2-appleclang17-linear-focused.png)

![See the paragraph for a text description of this bar chart image.](https://thephd.dev/assets/img/2025/12/manorboy2-gcc15-linear-focused.png)

[For the vision-impaired, a text description is available.](https://thephd.dev/assets/img/2025/12/manorboy2-gcc15-linear-focused.png.txt)
...
It goes to show that having what the Closures WIP ISO C proposal asks for both C++-style Lambdas and C-style “Capture Functions” (nested functions that do not have the design, ABI issues, and Implementation Baggage of regular GNU Nested Functions)[2](https://thephd.dev/the-cost-of-a-closure-in-c-c2y-followup/#fn:capture-functions) **along with** a Wide Function Pointer type would be better than trying to figure out a magic `static` or magic `thread_local` style of implementation.

We are not sure what to think of the Local Functions and Function Literals proposals[3](https://thephd.dev/the-cost-of-a-closure-in-c-c2y-followup/#fn:local-literal-functions), because neither of them try to allow you to access local variables. Which is 90%[4](https://thephd.dev/the-cost-of-a-closure-in-c-c2y-followup/#fn:stat-90) of the reason anyone uses Nested Functions to begin with!
...
- Lambdas (and the proposed Capture Functions[2](https://thephd.dev/the-cost-of-a-closure-in-c-c2y-followup/#fn:capture-functions)) are the best for performance, so long as perfect information is retained.
...
- Normal Functions: regular C functions which add an extra argument to the function call in order to pass more data. Somewhat similar in representation to rewriting qsort to qsort\_r/qsort\_s to pass a user data pointer.
...
- Lambdas (std::function\_ref): The same as above, but rather than using a function template to handle each uniquely-typed lambda like a precious baby bird, it instead erases the lambda behind a `std::function_ref<int(void)>`. This allows the recursive function to retain exactly one signature.
...
- C++03 shared\_ptr (Rosetta Code): A C++ class using `std::enable_shared_from_this` and `std::shared_ptr` with a virtual function call to invoke the “right” function call during recursion.
...
1. See: [https://github.com/soasis/idk/tree/main/benchmarks/closures](https://github.com/soasis/idk/tree/main/benchmarks/closures). [↩](https://thephd.dev/the-cost-of-a-closure-in-c-c2y-followup/#fnref:idk-benchmarks-closures)

2. See “Captures Functions: Rehydrated Nested Functions” from [“Functions with Data - Closures in C”](https://thephd.dev/_vendor/future_cxx/papers/C%20-%20Functions%20with%20Data%20-%20Closures%20in%20C.html#design-capture.functions). [↩](https://thephd.dev/the-cost-of-a-closure-in-c-c2y-followup/#fnref:capture-functions) [↩2](https://thephd.dev/the-cost-of-a-closure-in-c-c2y-followup/#fnref:capture-functions:1)

3. See “N3678 - Local functions” and “N3679 - Function literals”, [https://www.open-std.org/JTC1/SC22/WG14/www/docs/n3678.pdf](https://www.open-std.org/JTC1/SC22/WG14/www/docs/n3678.pdf) and [https://www.open-std.org/JTC1/SC22/WG14/www/docs/n3679.pdf](https://www.open-std.org/JTC1/SC22/WG14/www/docs/n3678.pdfhttps://www.open-std.org/JTC1/SC22/WG14/www/docs/n3679.pdf) [↩](https://thephd.dev/the-cost-of-a-closure-in-c-c2y-followup/#fnref:local-literal-functions)

4. This is not a hard or scientific statistic. We simply catalogued a codebase that used GNU Nested Functions – of the thousands of uses, the overwhelming supermajority accessed variables contextually. A proposal that solves 10% of a codebases existing uses seems worthless. [↩](https://thephd.dev/the-cost-of-a-closure-in-c-c2y-followup/#fnref:stat-90)

5. See “Access the Context of Nested Functions”, [https://www.open-std.org/JTC1/SC22/WG14/www/docs/n3654.pdf](https://www.open-std.org/JTC1/SC22/WG14/www/docs/n3654.pdf). [↩](https://thephd.dev/the-cost-of-a-closure-in-c-c2y-followup/#fnref:n3654)
