...
url: https://andrewkelley.me/post/quest-build-ultimate-music-player.html
published: "2014-04-22T17:39:04Z"
feed: andrewkelley
guid: https://andrewkelley.me/post/quest-build-ultimate-music-player.html
---
...
02. [I <3 Amarok 1.4](#amarok14)
03. [A Short Explanation of Loudness Compensation](#loudness-intro)
04. [Shortcomings of Amarok 1.4](#amarok14-shortcomings)
05. [My Laundry List of Music Player Features](#laundry)
06. ["PartyBeat"](#partybeat)
07. [Fumbling Around with Technology](#tech)
08. [Growing Pains](#growing)
09. [Rejecting MPD](#rejecting-mpd)
10. [Building a Music Player Backend](#building-a-backend)
11. [Packaging](#packaging)
12. [Contributing to libav](#libav-contributions)
...
[ReplayGain plugin](http://kde-apps.org/content/show.php?content=26073).
Here's a screenshot:
...
on the release blog articles for Amarok _2.0_, which rebuilt the player from scratch and
...
[2.0](http://amarok.kde.org/en/releases/2.0) [2.1](http://amarok.kde.org/en/releases/2.1) [2.2](http://amarok.kde.org/en/releases/2.2)

Even now, 4 years later, the project is at version 2.8 and the release name is titled
["Return To The Origin"](http://amarok.kde.org/en/releases/2.8):
...
Here is a song called _The Happiest Days of Our Lives_ by Pink Floyd, mastered in 1979:

![](http://s3.amazonaws.com/superjoe/blog-files/quest-build-ultimate-music-player/pink-floyd-waveform.png)

Here is a song called _Saying Sorry_ by Hawthorne Heights, mastered in 2006:
...
_decrease_ the volume of the loud song. In order to do this, we
introduce an amount called **pre-gain**. All songs are turned down by this
amount, which gives us the **headroom** we need to turn the quieter ones
back up.
...
The
[ReplayGain 1.0 Specification](http://wiki.hydrogenaudio.org/index.php?title=ReplayGain_1.0_specification#Clipping_prevention)
outlines this in more detail.

In 2010, the European Broadcasting Union introduced a new standard called
[R128](https://tech.ebu.ch/loudness). This standard outlines a
strategy for analyzing media and determining how loud it is. There is a
[motion](http://wiki.hydrogenaudio.org/index.php?title=Talk:ReplayGain_2.0_specification)
...
the set of songs which you can decode and play back _must_ be the same set of songs
...
At the same time, it should determine the _true_ duration of the file and use that
...
[VLC](http://www.videolan.org/vlc/index.html) to play it? That must be true
...
For example, I could type "jonsi ik" and match the song _Boy Lilikoi_ by Jónsi.
...
queued. So any song would be _possible_ to be queued, but songs that have not been
queued recently are much more likely to be queued. _Queue date_ is chosen rather than
_play date_ because if a song is queued and the user skips the song, this should still
...
[fun little project](https://github.com/royvandewater/partybeat)
...
[Trontastic JQuery UI theme](http://jqueryui.com/themeroller/#!zThemeParams=).

This project used [Django](https://www.djangoproject.com/) and
[xmms2](https://xmms2.org/wiki/Main_Page) and had a bug-ridden, barren, and clunky web-based user interface. It was so lacking compared to my usual Amarok 1.4 experience,
...
I tinkered a bit with [Qt](http://qt-project.org/) and the
[Phonon](http://phonon.kde.org/) framework, but I didn't get as far as
having a web interface controlling it.

Eventually I stumbled upon [Music Player Daemon](http://www.musicpd.org/).
...
[the protocol](http://www.musicpd.org/doc/protocol/)
...
[12-day hiking trip](http://andrewkelley.me/post/jmt.html) followed immediately
...
[socket.io](http://socket.io/) was getting a
lot of hype, and it seemed like the perfect fit for my design. Also I had just given
[Coffee-Script](http://coffeescript.org/) a real chance after snubbing it
initially. So I ported over the proxy/file server to
[Node.js](http://nodejs.org/) and got a prototype working:
...
I named it **Groove Basin**, after the
[Sonic the Hedgehog 3 Azure Lake Remix](http://s3k.ocremix.org/php/media.php) by
[Rayza](https://soundcloud.com/rayza).
As homage to the original project, I picked a JQuery UI theme for the UI, except this time
I chose
[Dot Luv](http://jqueryui.com/themeroller/#!zThemeParams=).
...
[Last.fm](http://last.fm) scrobbling required that the server send scrobbles
...
rather than shadowing it. For example if you `path = require('path')` and
then later innocently use `path` as the name of a local variable, you're
in for a wild ride:
...
not reuse temporary variables, so you end up with `_len`, `_len2`,
`_len3` and so on:
...
`return` statement at the end. This can have surprising side effects.
In our example code, Coffee-Script decided to put the output of `console.log`
into an array. This is a
[controversial feature](https://github.com/jashkenas/coffee-script/issues/2477),
...
[cleanest organization of callback code](js-callback-organization.html).
...
instead I switched the codebase over to [coco](https://github.com/satyr/coco).
This project
[solved some of Coffee-Script's problems](https://github.com/satyr/coco/wiki/wtfcs)
including all the ones I listed above, and
[satyr](https://github.com/satyr) seemed to have a better understanding of
compiler design than
[jashkenas](https://github.com/jashkenas)
given that coco ran [twice as fast](https://github.com/satyr/coco/wiki/improvements#size--speed).

coco lasted about a year before I removed it. satyr started taking coco in a
pretty wild direction, adding things like `c = a-b` compiling to
`c = aB` rather than `c = a - b`. The syntax became so
...
package managers, including the [Raspberry Pi](http://www.raspberrypi.org/).
It had been around
...
ReplayGain scanning makes the set of songs you can _play_ different from the
set of songs you can _scan_. This leads to an inconsistent and rather unpleasant
...
MPD is controlled via the [MPD Protocol](http://musicpd.org/doc/protocol/) that can be used to control
...
the server accepts the message and then only notifies _other_ clients that the volume changed.
...
[Josh](https://github.com/thejoshwolfe) discovered that MPD apparently makes two
...
2. It somehow is so inefficient at updating sticker information that it causes audio
...
- Per-playlist-item gain adjustment so that perfect loudness compensation can be
   implemented.
...
- Support transcoding audio into another format so a player can implement, for example,
   HTTP streaming.

- Give raw access to decoded audio buffers just in case a player wants to do something
...
[libav](http://www.libav.org/) from
[ffmpeg](http://www.ffmpeg.org/) (here are two sides to the story:
[libav side](http://libav.org/about.html),
[ffmpeg side](http://blog.pkh.me/p/13-the-ffmpeg-libav-situation.html)), I went
with libav simply because it is what is in the
[Debian](http://www.debian.org/) and
[Ubuntu](http://www.ubuntu.com/) package managers, and one
...
[pretty solid API](https://github.com/andrewrk/libgroove/blob/3.0.7/groove/groove.h).
How it works:
...
A **sink** is a metaphor of a real-life sink that you would find
...
- **playback sink** \- opens a sound device and
   sends the decoded audio to it. This sink fills up with _events_
   that signal when the sink has started playing the next track, or when
   a buffer underflow occurs.

- **encoder sink** \- encodes the audio buffers it receives
...
- **loudness scanner sink** \- uses the EBU R 128 standard to
...
To do this I built a native add-on node module called [groove](https://www.npmjs.org/package/groove).
It uses [libuv](https://github.com/joyent/libuv) and [v8](https://code.google.com/p/v8/)
to interface between C and Node.js. I wrote the majority of this code at
[Hacker School](https://www.hackerschool.com/), an experience which I highly recommend.
...
Next, I made a [libgroove PPA](https://launchpad.net/~andrewrk/+archive/libgroove/)
...
Then I [joined](http://lists.alioth.debian.org/pipermail/pkg-multimedia-maintainers/2014-February/036583.html) the [Debian multimedia packaging team](https://wiki.debian.org/DebianMultimedia).
...
[libebur128 Debian package](https://packages.debian.org/source/jessie/libebur128) is ready to be installed
from [testing](http://www.debian.org/releases/testing/), and a
[libgroove Debian package](https://packages.debian.org/source/experimental/libgroove)
can be installed from experimental. Once the
[libav10 transition](https://release.debian.org/transitions/html/libav10.html) is complete,
libgroove can be submitted to
[unstable](http://www.debian.org/releases/sid/), where it will move into
...
installation process could be just an `apt-get install` away.

## Contributions to libav

While working on libgroove several issues came up which led me to contribute code to
[libav](http://libav.org). The first thing I noticed is that if you asked
...
[Xiph.org](http://xiph.org/) Foundation
[recommends](http://wiki.xiph.org/index.php/MIME_Types_and_File_Extensions)
...
so the
[compromise](http://git.libav.org/?p=libav.git;a=commitdiff;h=b0c2c097e422b9e10a7d856582f8321d28af821e)
...
[broken, glitchy playback if you seek to the beginning](https://bugzilla.libav.org/show_bug.cgi?id=567).
...
[a patch](http://git.libav.org/?p=libav.git;a=commitdiff;h=0c082565965258dca143767cc6cb25e38b6e9ea3)
...
[ported](http://git.libav.org/?p=libav.git;a=commitdiff;h=ba21499648bbffc5518a41dc01a51449b9871088)
...
3 years, 6 months from `git init` and Groove Basin is still under active development.
...
- Lazy multi-core
   [EBU R128 loudness scanning](http://tech.ebu.ch/loudness)
...
[live demo](http://demo.groovebasin.com/) site.
...
Groove Basin still has lots of [issues](https://github.com/andrewrk/groovebasin/issues)
...
running on a [Raspberry Pi](http://www.raspberrypi.org/). It's mostly straightforward
but there are enough "gotchas" here and there that I think it could be a useful article.

_(update 2014-June-19)_ I have now written this article:
[Turn Your Raspberry Pi into a Music Player Server](raspberry-pi-music-player-server.html)

Feel free to star or watch the
[Groove Basin GitHub repository](https://github.com/andrewrk/groovebasin)
