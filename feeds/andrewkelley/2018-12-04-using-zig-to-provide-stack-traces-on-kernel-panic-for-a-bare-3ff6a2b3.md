...
feed: andrewkelley
guid: https://andrewkelley.me/post/zig-stack-traces-kernel-panic-bare-bones-os.html
---

# Using Zig to Provide Stack Traces on Kernel Panic for a Bare Bones Operating System

Last week, I reached an exciting milestone in my career as a programmer.
[For the first time in my life, I ran code directly on hardware](https://www.youtube.com/watch?v=gihdpLtHi9Q), with no Operating System sitting between the bare metal and my code.
...
So what's the next step? Bootloader? File system? _Graphics?!_
...
[stack trace](https://ziglang.org/download/0.3.0/release-notes.html#stack-traces),
like this:
...
_freestanding_. The equivalent code in freestanding actually just hangs
the CPU:
...
When run, you'll see we never get to the "got here" message, but also
we don't get any kind of error message printed or anything.
...
[root source file](https://ziglang.org/documentation/master/#Root-Source-File)
alongside `pub fn main`. But if you do not provide this
function, [the default](https://github.com/ziglang/zig/blob/0.3.0/std/special/panic.zig)
is used:
...
After a [back and forth on a bug report](https://bugs.llvm.org/show_bug.cgi?id=39862),
...
I [submitted a patch upstream](https://reviews.llvm.org/D55276),
...
in freestanding mode thanks to Zig's _lazy top level declaration analysis_.
...
and then looking at their _addresses_, we can find out where in memory the symbols
defined in the linker script are.

For `.debug_abbrev`, `.debug_str`, and `.debug_ranges`, I had to
set the offset to 0 to
[workaround](https://bugs.llvm.org/show_bug.cgi?id=39862#c6)
...
That last line is coming from the startup assembly code, which has no
source mapping. But we can see that it's correct, by looking at
the disassembly of the kernel:
...
Here we take advantage of [inline for](https://ziglang.org/documentation/master/#inline-for)
as well as [@embedFile](https://ziglang.org/documentation/master/#embedFile), and all the sudden
we can print lines of code from our own source files. `printLineFromBuffer` [left as an exercise for the reader](https://github.com/andrewrk/clashos/blob/bf8e57ac220715d0698ab910d337ea590c4b4e33/src/debug.zig#L133-L152).
...
And now all of the
[protections that Zig offers against undefined behavior](https://ziglang.org/documentation/master/#Undefined-Behavior)
...
[Hello World x86 Kernel](https://github.com/andrewrk/HellOS), which comes with a
[video of me live coding it](https://www.youtube.com/watch?v=yUge-ujPxzQ).
