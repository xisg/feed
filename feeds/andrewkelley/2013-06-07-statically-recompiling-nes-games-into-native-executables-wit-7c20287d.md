...
url: https://andrewkelley.me/post/jamulator.html
published: "2013-06-07T08:48:00Z"
feed: andrewkelley
guid: https://andrewkelley.me/post/jamulator.html
---
...
[LLVM](https://llvm.org/), I felt like I finally had worthwhile idea for a project.
...
02. [Discovering LLVM](#discovering-llvm)
03. [NES Crash Course](#nes-crash-course)
04. [Creating a Custom ABI for Testing Purposes](#custom-abi)
05. [Assembly](#assembly)
06. [Disassembly](#disassembly)
07. [Code Generation](#code-generation)
08. [Optimization](#optimization)
09. [Layout of an NES ROM File](#rom-layout)
10. [Challenge: Runtime Address Checks](#runtime-address-checks)
11. [Challenge: Parallel Systems Running Simultaneously](#parallel-systems)
12. [Challenge: Handling Interrupts](#interrupts)
13. [Challenge: Detecting a Jump Table](#detect-jump-table)
14. [Challenge: Indirect Jumps](#indirect-jumps)
15. [Challenge: Dirty Assembly Tricks](#dirty-assembly-tricks)
16. [Video: Playing Super Mario Brothers 1 on Native Machine Code](#video)
17. [Unsolved Challenge: Mappers](#mappers)
18. [Community Support](#community-support)
19. [Conclusion](#conclusion)

## Discovering LLVM

I became giddy with excitement when I discovered [LLVM](https://llvm.org/).
Generate code in [LLVM IR format](https://llvm.org/docs/LangRef.html),
...
[emscripten](http://emscripten.org) provides a JavaScript backend.

To seal the deal, the LLVM project offers [clang](http://clang.llvm.org/), a
C, C++, Objective C, and Objective C++ _front-end_ for LLVM.
...
Right, so default gcc doesn't have the nice things that [C99](https://en.wikipedia.org/wiki/C99) brings.
...
LLVM has [a much better explanation](http://clang.llvm.org/diagnostics.html)
...
This is exciting because clang is merely a _front-end_ to LLVM. What this means is that
...
**_statically_ recompile NES games into native binaries!**
...
StartEndDescription$0000$07ffRAM. Unless they use a mapper, NES games only get 2KB of RAM!$2000$2007
...
[pretty complicated](http://www.dwedit.org/files/hello.asm).
...
First things first - this project should be able to _assemble_ our
...
to _tokenize_ the source, and then a
[parser](https://en.wikipedia.org/wiki/Parser#Programming_languages)
to turn the tokens into an
[Abstract Syntax Tree](https://en.wikipedia.org/wiki/Abstract_syntax_tree).
From there we can process the AST and turn it into the 32KB binary payload.

In Go land, this is straightforward thanks to [nex](https://github.com/blynn/nex),
a lexer, and [go tool yacc](https://golang.org/cmd/yacc/), a parser that
is bundled with Go.
...
I decided not to use this technique, however, since the goal of this project is _static_
...
2. Replace the `.db` statements at `$fffa` and `$fffb`
    with a `.dw` statement which references an `NMI_Routine` label.

3. Calculate the address that the `.db` statements at `$fffa` and `$fffb`
...
mark other locations as instructions:
 ...
 Mark the jump target address _and_ the next address as an instruction.
...
The instructions that start with "B" are all _conditional branch_ instructions.
...
possible branch address _and_ the next address as instructions.
...
It is _possible_ for assembly programmers to use `JSR` and then
...
As seen in this table, there are 2 types of `JMP` instructions: absolute and indirect:
 ...
 is known statically - the address is _hard-coded_.
...
Our code generation code will generate an LLVM _bitcode_ module.
...
This means we can integrate cleanly using [cgo](https://golang.org/cmd/cgo/).
In fact, [Andrew Wilkins](http://awilkins.id.au/) maintains a convenient Go module called
[gollvm](https://github.com/axw/gollvm) which gives us seamless integration.

At any time we can debug the LLVM module we are generating by calling `module.Dump()`
which prints the [LLVM IR code](https://llvm.org/docs/LangRef.html)
...
Here is a full description of all the status bits:
 ...
 Normally, this bit would toggle _decimal mode_ on and off.
...
With the help of [gollvm](https://github.com/axw/gollvm)
we will:

1. Create a LLVM module.
2. Declare our dependency on `putchar` and `exit`.
3. Create the `main` function and allocate stack variables for the
    registers.

4. Perform one pass over the AST to identify labeled data
    and create global variables. We save the index of label name
    to global variable in a map.

5. Perform a second pass over the AST to generate the basic blocks
...
LLVM comes with state of the art optimization techniques, in the form of _passes_
...
bitcode:
 ...
 Description
 [Constant Propagation](https://llvm.org/docs/Passes.html#constprop-simple-constant-propagation)
 Looks for instructions involving only constants and replaces them with a
 constant value instead of an instruction.
 [Combine Redundant Instructions](https://llvm.org/docs/Passes.html#instcombine-combine-redundant-instructions)
 ...
 all the redundancy.
 [Global Value Numbering](https://llvm.org/docs/Passes.html#gvn-global-value-numbering)
 Eliminates partially and fully redundant instructions, and delete redundant load instructions.
 [Control Flow Graph Simplification](https://llvm.org/docs/Passes.html#simplifycfg-simplify-the-cfg)
...
LLVM offers [many more available passes](https://llvm.org/docs/Passes.html),
...
but we're actually generating highly _optimized_ code.
...
format, originally started by the [iNES emulator](http://fms.komkon.org/iNES/).
...
2. The 32 KB of assembled code which gets loaded into $8000 - $ffff on bootup.
    If there is a mapper, there might be more of this program data, but we'll talk
    about that later.

3. 8KB of graphics data, known as CHR-ROM.
...
[Nesdev Wiki](http://wiki.nesdev.com/w/index.php/INES) or the
[official specification](http://fms.komkon.org/EMUL8/NES.html).

The first "real" ROM I tried to recompile is this demo ROM made by
[Chris Covell](http://www.chrismcovell.com/) in 1998 called
[Zelda Title Screen Simulator](https://www.castledragmire.com/hynes/reference/resource/index.html).
...
[In retrospect](https://github.com/andrewrk/jamulator/issues/1),
...
This is an _indirect_ instruction. It tells the CPU to:
...
Because values in memory addresses $0000 and $0001 could be _anything_,
...
[Fergulator](https://github.com/scottferg/Fergulator) \- an
...
_One of the systems must be emulated._
...
[small C runtime](https://github.com/andrewrk/jamulator/tree/c9b4de0424d4dcc623f594be4a165685874713fa/runtime),
...
Reset
 ...
 [vertical blank](https://en.wikipedia.org/wiki/Vertical_blanking_interval)
 begins.
...
defined by the _interrupt vector table_, as seen in code listings above:
...
including
[Super Mario Brothers 3](http://sonicepoch.com/sm3mix/disassembly.html#DynJump) and Pac-Man.
...
_sabotages_ the next instruction, causing the Y = $04 to not happen. Instead,
...
where the program _jumps into the middle of an instruction_.
...
These issues _must_ be resolved if we want a playable game.
...
This strategy ensures that NES games will run _correctly_,
...
[fascinating optimization articles](http://www.emulators.com/docs/nx25_nostradamus.htm),
the invaluable
...
Thanks especially to Ulfalizer, sherlock\_, Bisqwit, Movax12, and
[scottferg](https://github.com/scottferg/) for answering
my questions, even when they seemed stupid.
The [Nesdev Forums](http://forums.nesdev.com/) are nice as well.
...
[fix a bug](https://github.com/scottferg/Fergulator/commit/feb9b16d785f281f04f7295452d065f856bfc727)
in his emulator, which fixed support for
[Maniac Mansion](https://en.wikipedia.org/wiki/Maniac_Mansion).

I also filed an
[llvm feature request](https://llvm.org/bugs/show_bug.cgi?id=15873)
...
[V8](https://code.google.com/p/v8/), but they do not
work by statically compiling the script. Instead they use
[Just-In-Time compilation](https://en.wikipedia.org/wiki/Just-in-time_compilation), along with some advanced techniques, to achieve great speeds.
