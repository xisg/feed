---
title: 'От баз данных до инструментов для ИИ‑экосистем: проекты, которые получили гранты Yandex Open Source'
url: https://habr.com/ru/companies/yandex/articles/1040282/?utm_source=habrahabr&utm_medium=rss&utm_campaign=corporate_blog
published: "2026-05-28T09:01:26Z"
feed: yandex
guid: https://habr.com/ru/companies/yandex/articles/1040282/
---

# От баз данных до инструментов для ИИ‑экосистем: проекты, которые получили гранты Yandex Open Source

![](https://habrastorage.org/getpro/habr/upload_files/42c/170/d75/42c170d75ce9457aaee3c2d5a33ba789.png)

Развивать собственный технологический проект в одиночку или небольшой командой — это всегда вызов. Нужно не просто написать работающий код, но и продумать архитектуру, закрыть инфраструктурные боли, настроить CI/CD и при этом не выгореть. Тем ценнее видеть, как крутые разработки получают заслуженную поддержку, помогающую им выйти на новый уровень.

В этом году мы провели Yandex Open Source при поддержке платформы для разработчиков [SourceCraft](https://sourcecraft.dev/). А ещё мы увеличили призовой фонд с 12 до 18 победителей. Мы принимали заявки по трём трекам: обработка и хранение данных, разработка, искусственный интеллект.

[Читать далее](https://habr.com/ru/articles/1040282/?utm_source=habrahabr&utm_medium=rss&utm_campaign=corporate_blog#habracut)
