...
url: https://yosefk.com/blog/advantages-of-incompetent-management.html
...
Of course you could say that this is a _badly_ run company, and to avoid arguing what that means, let’s stick to the
...
show, and ends once the budget increase is approved (or _isn’t_ approved; but it never really hurts to try.) But if you
...
mean, shrinking the cost has got to be helping with these?.. Well, sure it’s helping, but it’s not helping _you,_ because
you don’t bring any revenue _by yourself,_ unlike cost, which you very much do incur all by yourself. The math with
_you_ is, **we tell you to do something if the cost is below a threshold**. If you won’t commit to doing it
...
impact on the firm’s “revenue - cost” doesn’t trickle down to your “cost < threshold,” because you have no revenue [1](#fn1).
...
resources, and then _you_ won’t be able to. In fact, a mature, well-run CI system will measure everyone’s resource
...
people learn to never optimize their code - unless _they_ want to deploy something new themselves, and only after asking
...
mistake their corporation for their tribe, thinking their coworkers share more of their genes than they do [2](#fn2). But **if there’s a reliable & scalable way for**
...
to do, they’re still always going to do _something_, and then it’ll be too late.”
...
If we now revisit the situation from the employer’s point of view, it is no longer _trivially_ wasteful, because
...
need, and yet you almost certainly need _now,_ because actually productive activities are hopelessly intertwined with
this stuff.
...
trying. And of course when people lie, they lie first and foremost to themselves - we’re all natural-born [Method actors](https://en.wikipedia.org/wiki/Method_acting) \- so if you spot a decoy and try to cancel the work on that
...
_something_ \- often a big something.
...
_in their own work_ for the same reasons mentioned above, but they might still be increasing _others’_ efficiency,
...
detectors](https://yosefk.com/blog/checkedthreads-bug-free-shared-memory-parallelism.html), etc.
...
With performance, you deal in resources to begin with, and you have _some_ number speaking of resource savings by
...
software, which can't be patched to work around hardware bugs, like a CPU, [will face more pressure to be correct](https://yosefk.com/blog/the-habitat-of-hardware-bugs.html) than something like a
...
where it’s “easy” for software developers to add workarounds to this code [3](#fn3). If you’re a software developer, you could try an industry with high reliability
...
“What is to be done?” is [a pamphlet by Lenin](https://en.wikipedia.org/wiki/What_Is_to_Be_Done%3F), who proposed
...
observed not-so-good alternatives. I was brought up with natural immunity to [the Soviet strain](https://en.wikipedia.org/wiki/The_Internationale#Anthem_of_the_Soviet_Union) of this Panglossian
...
Western strain](https://paulgraham.com/good.html):
...
> if anything can. Follow it and it will take you through everything you need to do. [4](#fn4)
...
_could_, and they would be better off if they did.
...
opposite thought is depressing? Well, the thought of [faster code tending to be uglier](https://yosefk.com/blog/efficiency-is-fundamentally-at-odds-with-elegance.html) isn’t
...
think you have to find a solution to an organizational problem just because you happen to observe it [5](#fn5). From an individual’s POV, the environment is nearly
...
the rise and fall of Creo](https://news.ycombinator.com/item?id=40893945), a company that they say did start out combining the best of both worlds, then regressed to the
...
create](https://danluu.com/algorithms-interviews/), problems equivalent to basic phone screen questions on the job, for reasons related to the above.

_Thanks to Dan Luu and Tim Pote for reviewing a draft of this post._

* * *

1. If you’re a general manager of a business unit, aka a P&L (profit & loss) unit, then of course you
   _do_ have revenue, and things are different. For the purpose of our current discussion, I treat a BU as a separate
   company, and the discussion applies to its employees, not its GM who for our purposes can be treated as a CEO. [↩︎](#fnref1)

2. Either that, or it’s a genetic defect in people, or something about group selection. It is a scientific fact
   ...
   failure to do so, and no other mechanism nor information encoded in any other form is of any consequence. [↩︎](#fnref2)

3. It’s also easy for a device driver programmer to beat up a hardware device designer, especially when sneaking
   from behind, but it’s been made artificially hard by various legal mechanisms. [↩︎](#fnref3)

4. I did notice that it says “good for users” and not “good for employees” or from other points of view. But you
...
5. The virus is bad enough to actually make you think, “since I don’t know how to solve this problem, I probably
   don’t really understand it - my analysis of my observations must be incorrect,” as if thinking that the discrete knapsack
   problem is NP-complete is a symptom of not understanding the discrete knapsack problem. [↩︎](#fnref5)
